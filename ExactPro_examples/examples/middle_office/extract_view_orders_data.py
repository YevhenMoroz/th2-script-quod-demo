...
from win_gui_modules.middle_office_wrappers import ViewOrderExtractionDetails
...
def execute(report_id):
    ...
    case_name = "Extract view order details example"
    ...
    try:
        ...
        extract_request.set_view_orders_filter({"Symbol": "VETO"})
        extract_request.extract_length("middleOffice.viewOrdersCount")

        order_details = extract_request.add_order_details()
        order_details.set_order_number(1)
        order_details.add_extraction_detail(ExtractionDetail("middleOffice.avgPrice", "AvgPrice"))
        order_details.add_extraction_detail(ExtractionDetail("middleOffice.orderId", "Order ID"))
