...
#from test_cases.QAP_T2964 import TestCase
...
def execute(report_id):
    ...
    try:
        ...
        allocations_details.add_allocation_param({"Security Account": "MOClientSA1", "Alloc Qty": qtyh})
        allocations_details.add_allocation_param({"Security Account": "MOClientSA2", "Alloc Qty": qtyh})
        ...
        # Checkpoint5 creation
        checkpoint_response5 = Stubs.verifier.createCheckpoint(bca.create_checkpoint_request(case_id))
        checkpoint_id5 = checkpoint_response5.checkpoint
        ...
        verifier.compare_values("Order Status", "Accepted", request_allocate[block_order_status.name])
        verifier.compare_values("Order Match Status", "Matched", request_allocate[block_order_match_status.name])
        verifier.compare_values("Order Summary Status", "", request_allocate[block_order_summary_status.name])
        verifier.verify()
        ...
        verifier.set_event_name("Checking allocate blotter")
        verifier.compare_values("Allocation Status", "Canceled", request_allocate_blotter[allocate_status.name])
        verifier.compare_values("Allocation Account ID", "MOClientSA1", request_allocate_blotter[allocate_account_id.name])
        verifier.compare_values("Allocation Match Status", "Unmatched", request_allocate_blotter[allocate_status_match_status.name])
        verifier.verify()
        ...
        extract_request = AllocationsExtractionDetails(base=base_request)
        extract_request.set_block_filter({"Order ID": block_order_id})
        extract_request.set_allocations_filter({"Account ID": "MOClientSA1", "Status": "Affirmed"})
        allocate_status = ExtractionDetail("middleOffice.status", "Status")
        allocate_account_id = ExtractionDetail("middleOffice.account_id", "Account ID")
        allocate_status_match_status = ExtractionDetail("middleOffice.match_status", "Match Status")
        order_details = extract_request.add_order_details()
        ...
        extract_request = AllocationsExtractionDetails(base=base_request)
        extract_request.set_block_filter({"Order ID": block_order_id})
        extract_request.set_allocations_filter({"Account ID": "MOClientSA2", "Status": "Affirmed"})
        allocate_status = ExtractionDetail("middleOffice.status", "Status")
        allocate_account_id = ExtractionDetail("middleOffice.account_id", "Account ID")
        allocate_status_match_status = ExtractionDetail("middleOffice.match_status", "Match Status")
        order_details = extract_request.add_order_details()
