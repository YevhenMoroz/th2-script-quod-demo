...
class TestCase:
    ...
    def __init__(self, report_id):
        ...
        self.case_id = bca.create_event('QAP_T2955', report_id)
    ...
    def send_md_subscribe(self, ask_px, bid_px):
    ...
    def extract_prices(self):
        ...
        extract_table_request.set_extraction_id("extrId0")
        ...
        extract_table_request.set_ask_extraction_field(ExtractionDetail("rateTile.AskFwdPts", "Pts"))
        extract_table_request.set_bid_extraction_field(ExtractionDetail("rateTile.BidFwdPts", "Pts"))
        data_table = call(self.cp_service.extractRatesTileTableValues, extract_table_request.build())

        return str(float(data_table["rateTile.AskFwdPts"]) * 0.0001 + float(data_table["rateTile.AskSpot"]))[:8], \
               str(float(data_table["rateTile.BidFwdPts"]) * 0.0001 + float(data_table["rateTile.BidSpot"]))[:8],

    # Extract Large values and pips from GUI and verify calculation
    def verify_prices(self, exp_ask_large, exp_bid_large, exp_ask_pip, exp_bid_pip):
        extract_request = ExtractRatesTileValues(details=self.base_details)
        extract_request.set_extraction_id("extrId1")
        extract_request.extract_ask_large_value("rateTile.askLargeValue")
        extract_request.extract_bid_large_value("rateTile.bidLargeValue")
        extract_request.extract_ask_pips("rateTile.askPips")
        extract_request.extract_bid_pips("rateTile.bidPips")
        data_tile = call(self.cp_service.extractRateTileValues, extract_request.build())
        print(exp_ask_large, exp_bid_large, exp_ask_pip, exp_bid_pip)

        verifier = Verifier(self.case_id)
        verifier.set_event_name("Check Large values and pips")
        verifier.compare_values("rateTile.askLargeValue", exp_ask_large, data_tile['rateTile.askLargeValue'])
        verifier.compare_values("rateTile.bidLargeValue", exp_bid_large, data_tile['rateTile.bidLargeValue'])
        verifier.compare_values("rateTile.askPips", exp_ask_pip, data_tile['rateTile.askPips'])
        verifier.compare_values("rateTile.bidPips", exp_bid_pip, data_tile['rateTile.bidPips'])
        verifier.verify()
    ...
    def execute(self):
        ...
        try:
            ...
            # Step 3
            self.verify_prices(ask[:5], bid[:5], ask[5:], bid[5:])
            # Step 4
            market_data_params = self.send_md_subscribe(ask, bid)
