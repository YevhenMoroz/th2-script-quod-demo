...
def test_run(parent_id=None, version=None, mode=None):
    if mode == 'Regression':
        report_id = bca.create_event(f"Algo_TWAP" if version is None else f"Algo_TWAP | {version}", parent_id)
    else:
    ...
    try:
        ...
        print(f'Algo_TWAP StartTime is {datetime.utcnow()}')
        ...
        print(f'Algo_TWAP EndTime is {datetime.utcnow()}, duration is {timedelta(seconds=end_time-start_time)}')
    except Exception:
