...
def test_run(parent_id=None, version=None, skip_ssh=False, only_ssh=False):
    report_id = bca.create_event(f"Basket Analysis" if version is None else f"Basket Analysis | {version}",
                                 parent_id)
    seconds, nanos = timestamps()  # Store case start time
    configuration = ComponentConfiguration("BasketTrading")
    data_set = configuration.data_set
    try:
        tests = os.listdir(f'{root_path(ignore_cwd=True)}/test_cases/eq/Basket')
        for test in tests:
            if test.find("QAP_T") == 0:
                class_ = getattr(importlib.import_module(f"test_cases.eq.Basket.{test[:-3]}"), test[:-3])
                ssh_test = check_ssh(f'{root_path(ignore_cwd=True)}/test_cases/eq/Basket/{test}')
...
if __name__ == '__main__':
    test_run(version="5.1.182.195", skip_ssh=False)
    Stubs.factory.close()
