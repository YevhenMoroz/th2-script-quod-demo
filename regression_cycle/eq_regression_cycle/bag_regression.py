...
def test_run(parent_id=None, version=None, skip_ssh=False, only_ssh=False):
    report_id = bca.create_event(f"Bag" if version is None else f"Bag Analysis | {version}", parent_id)
    seconds, nanos = timestamps()  # Store case start time
    configuration = ComponentConfiguration("Bag")
    data_set = configuration.data_set

    try:
        tests = os.listdir(f'{root_path(ignore_cwd=True)}/test_cases/eq/Bag')
        for test in tests:
            class_ = getattr(importlib.import_module(f"test_cases.eq.Bag.{test[:-3]}"), test[:-3])
            ssh_test = check_ssh(f'{root_path(ignore_cwd=True)}/test_cases/eq/Bag/{test}')
