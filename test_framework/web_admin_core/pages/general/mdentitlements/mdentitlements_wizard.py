...
class EntitlementsWizard(CommonPage):
    ...
    def click_on_close(self):
        self.find_by_xpath(MDEntitlementsConstants.CLOSE_WIZARD_XPATH).click()

    def click_on_save_changes(self):
        self.find_by_xpath(MDEntitlementsConstants.SAVE_CHANGES_BUTTON_XPATH).click()

    def click_on_revert_changes(self):
        self.find_by_xpath(MDEntitlementsConstants.REVERT_CHANGES_XPATH).click()
    ...
    def is_both_desk_and_location_can_be_filled_message_displayed(self):
        return 'Both Desk and Location can not be filled' in self.find_by_xpath(
            MDEntitlementsConstants.BOTH_DESK_AND_LOCATION_CAN_NOT_BE_FILLED_MESSAGE_XPATH).text
