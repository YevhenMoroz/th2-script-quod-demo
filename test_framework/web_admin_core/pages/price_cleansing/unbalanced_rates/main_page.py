...
class MainPage(CommonPage):
    ...
    def click_download_csv_button_and_get_content(self):
        self.clear_download_directory()
    ...
    def set_enrich_empty_side_of_book_filter(self, confirmation: bool):
        self.find_by_xpath(Constants.MainPage.Filters.ENRICH_EMPTY_SIDE_OF_BOOK).click()
        if confirmation:
            self.find_by_xpath(Constants.MainPage.Filters.SELECT_OPTION_TRUE).click()
        else:
            self.find_by_xpath(Constants.MainPage.Filters.SELECT_OPTION_FALSE).click()
