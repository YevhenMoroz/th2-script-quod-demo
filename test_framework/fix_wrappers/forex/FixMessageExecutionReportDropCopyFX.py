...
class FixMessageExecutionReportDropCopyFX(FixMessageExecutionReport):
    ...
    def set_params_from_trade(self, trade_request, status: Status = Status.Fill, response=None):

        if status is Status.Pending:
            self.__set_pending_new_sell(trade_request)
        elif status is Status.New:
            self.__set_new_sell(trade_request)
        elif status is Status.Fill:
            self.__set_fill_sell(trade_request, response)
        elif status is Status.PartialFill:
            self.__set_partial_fill_sell(trade_request)
        elif status is Status.Cancel:
            self.__set_cancel_sell(trade_request)
        elif status is Status.Reject:
            self.__set_reject_sell(trade_request)
        else:
    ...
    # CHECKED
    def __set_fill_sell(self, trade_request, response=None):
