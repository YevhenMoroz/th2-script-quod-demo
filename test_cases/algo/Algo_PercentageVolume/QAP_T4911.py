...
class QAP_T4911(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.child1_ltq_qty = AlgoFormulasManager.get_pov_child_qty(self.pct, self.ltq, self.qty)
        self.child2_ltq_qty = AlgoFormulasManager.get_pov_child_qty(self.pct, self.ltq_2, self.qty - self.child1_ltq_qty)
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_trade = rule_manager.add_NewOrdSingleExecutionReportTradeByOrdQty(self.fix_env1.buy_side, self.account, self.ex_destination_1, 1, 1, self.child1_ltq_qty, self.child1_ltq_qty, 3)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_1, True)
        self.rule_list = [ocr_rule, nos_dma_rule, nos_trade]
        # endregion
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child1_ltq_qty, Price=self.price, Instrument='*', TimeInForce=self.tif_day))
        ...
        self.fix_verifier_buy.check_fix_message(self.fill_dma_order_1_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport Fill DMA 1')
        #endregion
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for POV", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid, MDEntrySize=self.ltq_2)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child2_ltq_qty, Price=self.price, Instrument='*'))
