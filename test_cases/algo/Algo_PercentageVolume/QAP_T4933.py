...
class QAP_T4933(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.child_1_qty = AlgoFormulasManager.get_pov_child_qty(self.pct, 1000, self.qty)
        self.child_1_price = self.price_bid - self.tick_size
        self.child_2_qty = self.qty - self.child_1_qty
        self.child_2_price = self.price
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_dma1_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.child_1_price)
        nos_dma2_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.child_2_price)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_1, True)

        self.rule_list = [nos_dma1_rule, nos_dma2_rule, ocr_rule]
        # endregion
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price, MDEntrySize='1000')
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child_1_qty, Price=self.child_1_price, Instrument='*', TimeInForce=self.tif_day))
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child_2_qty, Price=self.child_2_price, Instrument='*', TimeInForce=self.tif_day))
