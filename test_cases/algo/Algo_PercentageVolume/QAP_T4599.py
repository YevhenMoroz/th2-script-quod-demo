...
class QAP_T4599(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.qty = 300
        self.price = 25
        self.qty_child_passive_1 = 23
        self.price_child_passive_1 = 20
        self.qty_child_passive_2 = 12
        self.price_child_passive_2 = 19.995
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_ioc_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.price_child_passive_1)
        nos_ioc_rule_2 = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.price_child_passive_2)
        ocr_rule = rule_manager.add_OCR(self.fix_env1.buy_side)
        self.rule_list = [ocr_rule, nos_ioc_rule, nos_ioc_rule_2]
        # endregion
        ...
        self.reactive_child_order.change_parameters(dict(Account=self.account, OrderQty=self.qty_child_passive_1, Price=self.price_child_passive_1, Instrument='*', ExDestination=self.ex_destination_1))
        self.fix_verifier_buy.check_fix_message(self.reactive_child_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Passive Child 1')
        ...
        self.fix_verifier_buy.check_fix_message(pending_reactive_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew  Passive Child 1')
        ...
        self.fix_verifier_buy.check_fix_message(new_reactive_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New  Passive Child 1')

        # endregion

        # region Check Passive child order
        self.fix_verifier_buy.set_case_id(bca.create_event("Passive child order 2", self.test_id))

        self.reactive_child_order = FixMessageNewOrderSingleAlgo().set_DMA_RB_params()
        self.reactive_child_order.change_parameters(dict(Account=self.account, OrderQty=self.qty_child_passive_2, Price=self.price_child_passive_2, Instrument='*', ExDestination=self.ex_destination_1))
        self.fix_verifier_buy.check_fix_message(self.reactive_child_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Passive Child 2')

        pending_reactive_child_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.reactive_child_order, self.gateway_side_buy, self.status_pending)
        self.fix_verifier_buy.check_fix_message(pending_reactive_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew  Passive Child 2')

        new_reactive_child_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.reactive_child_order, self.gateway_side_buy, self.status_new)
        self.fix_verifier_buy.check_fix_message(new_reactive_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New  Passive Child 2')
    ...
    def run_post_conditions(self):
        # region cancel Order
        self.case_id_cancel = bca.create_event("Fill Algo Order", self.test_id)
        ...
        self.fix_verifier_buy.check_fix_message(cancel_reactive_child_order_2, self.key_params, self.ToQuod, "Buy Side ExecReport Passive Child")
