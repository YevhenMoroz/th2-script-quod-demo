...
class QAP_T5039(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=1000) #self.open_qty
        ...
        # region Send NewOrderSingle (35=D) for POV order 1
        case_id_1 = bca.create_event("Create POV Order 1", self.test_id)
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=143)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=0, MDEntrySize=0)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par)
        # endregion
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for POV", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid, MDEntrySize=self.ltq_qty)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child_open_qty, Price=self.price, Instrument='*', TimeInForce=self.tif_day))
        ...
        # region Send NewOrderSingle (35=D) for POV order 2
        ...
        case_id_1 = bca.create_event("Create POV Order 2", self.test_id)
        self.fix_verifier_sell_2.set_case_id(case_id_1)

        self.POV_order_2 = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_POV_params()
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=186)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=0, MDEntrySize=0)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell_2.check_fix_message(self.POV_order_2, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        pending_POV_order_2_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.POV_order_2, self.gateway_side_sell, self.status_pending)
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child_open_qty, Price=self.price, Instrument='*'))
    ...
    def run_post_conditions(self):
        # region Cancel Algo Order 2
        case_id_3 = bca.create_event("Cancel Algo Order 2", self.test_id)
        self.fix_verifier_sell_2.set_case_id(case_id_3)

        cancel_request_pov_order = FixMessageOrderCancelRequest(self.POV_order_2)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_pov_order, case_id_3)
        self.fix_verifier_sell_2.check_fix_message(cancel_request_pov_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
        # endregion
        ...
        self.fix_verifier_buy_2.check_fix_message(cancel_dma_order_2, self.key_params, self.ToQuod, "Buy Side ExecReport cancel Child DMA 2")
        # endregion

        # region Cancel Algo Order 1
        self.fix_verifier_buy_3 = FixVerifier(self.fix_env1.buy_side, self.test_id)
        self.fix_verifier_sell_3 = FixVerifier(self.fix_env1.sell_side, self.test_id)
        time.sleep(5)

        case_id_4 = bca.create_event("Cancel Algo Order 1", self.test_id)
        self.fix_verifier_sell_2.set_case_id(case_id_4)
