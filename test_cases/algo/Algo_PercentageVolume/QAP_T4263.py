...
class QAP_T4263(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.rule_list = [ocr_rule, nos_dma_pass_rule, ocrr_rule]
        # endregion
        ...
        end_time = (now + timedelta(minutes=9)).strftime("%Y%m%d-%H:%M:%S")
        end_time_mod = (now + timedelta(minutes=5)).strftime("%Y%m%d-%H:%M:%S")
        ...
        self.POV_order.add_fields_into_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='Aggressivity', StrategyParameterType=1, StrategyParameterValue=self.aggressivity_pass),
                                                                                dict(StrategyParameterName='BookParticipation', StrategyParameterType=13, StrategyParameterValue=self.book_participation),
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child_qty_1, Price=self.price_pass, Instrument='*', TimeInForce=self.tif_day))
        ...
        self.new_dma_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_1, self.gateway_side_buy, self.status_new)
        ...
        self.POV_order_replace_params.add_fields_into_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='EndDate', StrategyParameterType=19, StrategyParameterValue=end_time_mod)])
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child_qty_1, Price=self.price_pass, Instrument='*', TimeInForce=self.tif_day))
        ...
        self.cancel_dma_order_2 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_2, self.gateway_side_buy, self.status_cancel)
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_request_POV_order.change_parameters(dict(NoParty='*', NoStrategyParameters='*', TargetStrategy='*', SettlType='*'))
        ...
        # region check sequnece child orders
        self.fix_verifier_buy.set_case_id(bca.create_event('Check child orders sequence', self.test_id))

        self.fix_verifier_buy.check_fix_message_sequence([self.pending_dma_order_params, self.new_dma_order_params, self.cancel_dma_order_1,
                                                          self.pending_dma_order_2_params, self.new_dma_order_2_params, self.cancel_dma_order_2],
                                                         [None, None, None, None, None, None], self.ToQuod)
        # endregion
