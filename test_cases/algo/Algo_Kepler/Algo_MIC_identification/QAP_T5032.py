...
class QAP_T5032(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # region instrument
        self.instrument = self.data_set.get_fix_instrument_by_name("instrument_32")
        # endregion
        ...
        # region venue param
        self.ex_destination_xlis = self.data_set.get_mic_by_name("mic_39")
        self.client = self.data_set.get_client_by_name("client_4")
        self.account_xlis = self.data_set.get_account_by_name("account_9")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account_xlis, self.ex_destination_xlis, self.price)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account_xlis, self.ex_destination_xlis, True)
        ...
        self.Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Instrument=self.instrument, Price=self.price, ExDestination=self.ex_destination_xlis, DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        self.dma_order.change_parameters(dict(Account=self.account_xlis, ExDestination=self.ex_destination_xlis, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument))
