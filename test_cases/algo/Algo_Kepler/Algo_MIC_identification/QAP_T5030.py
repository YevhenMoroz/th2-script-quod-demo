...
class QAP_T5030(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # region instrument
        self.instrument = self.data_set.get_fix_instrument_by_name("instrument_34")
        # endregion
        ...
        # region venue param
        self.ex_destination_xdub = self.data_set.get_mic_by_name("mic_40")
        self.client = self.data_set.get_client_by_name("client_4")
        self.account_xdub = self.data_set.get_account_by_name("account_9")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account_xdub, self.ex_destination_xdub, self.price)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account_xdub, self.ex_destination_xdub, True)
        ...
        self.Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Instrument=self.instrument, Price=self.price, ExDestination=self.ex_destination_xdub, DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        self.dma_order.change_parameters(dict(Account=self.account_xdub, ExDestination=self.ex_destination_xdub, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument))
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel first DMA 1 order")
        # endregion
