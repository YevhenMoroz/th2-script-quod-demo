...
class QAP_T4728(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # region instrument
        self.instrument = self.data_set.get_fix_instrument_by_name("instrument_40")
        # endregion
        ...
        self.account_bats = self.data_set.get_account_by_name("account_12")
        self.listing_id_chix = self.data_set.get_listing_id_by_name("listing_60")
        self.listing_id_bats = self.data_set.get_listing_id_by_name("listing_61")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_chix, True)
        ...
        market_data_snap_shot_chix.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=0, MDEntrySize=0)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_chix)
        ...
        market_data_snap_shot_bats.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=0, MDEntrySize=0)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_bats)
        ...
        case_id_1 = bca.create_event("Create SORPING STL GTD Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_1)

        self.SORPING_STL_GTD_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_Multiple_Emulation_Kepler_params()
        self.SORPING_STL_GTD_order.add_ClordId((os.path.basename(__file__)[:-3]))
        self.SORPING_STL_GTD_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_gtd, OrdType=self.order_type_stop_lmt, Instrument=self.instrument)).add_tag(dict(ExpireDate=self.ExpireDate, StopPx=self.stop_price))

        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTD_order, case_id_1)

        time.sleep(3)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_GTD_order, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new_SORPING_STL_GTD_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_order, self.gateway_side_sell, self.status_pending)
        self.fix_verifier_sell.check_fix_message(er_pending_new_SORPING_STL_GTD_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport PendingNew')

        er_new_SORPING_STL_GTD_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_order, self.gateway_side_sell, self.status_new)
        self.fix_verifier_sell.check_fix_message(er_new_SORPING_STL_GTD_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport New')
        # endregion
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_chix, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd, OrdType=self.order_type_stop_lmt)).add_tag(dict(ExpireDate=self.ExpireDate, StopPx=self.stop_price))
    ...
    #     cancel_request_SORPING_STL_GTD_order = FixMessageOrderCancelRequest(self.SORPING_STL_GTD_order)
    #
    #     self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_GTD_order, case_id_4)
    #     self.fix_verifier_sell.check_fix_message(cancel_request_SORPING_STL_GTD_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
    #
    ...
    #
    #     er_cancel_SORPING_STL_GTD_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_order, self.gateway_side_sell, self.status_cancel)
    #     self.fix_verifier_sell.check_fix_message(er_cancel_SORPING_STL_GTD_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
    #     # endregion
