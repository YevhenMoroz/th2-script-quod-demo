...
class QAP_T4737(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.ExpireDate = (now + timedelta(days=100)).strftime("%Y%m%d")
        self.ExpireTime = (now + timedelta(days=100)).strftime("%Y%m%d-%H:%M:%S.000")
        self.ChildExpireTime = (now + timedelta(days=2)).strftime("%Y%m%d-%H:%M:%S.000")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_GTD_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_gtd)).add_tag(dict(ExpireDate=self.ExpireDate, ExpireTime=self.ExpireTime))
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit6, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd)).add_tag(dict(ExpireTime=self.ChildExpireTime))
