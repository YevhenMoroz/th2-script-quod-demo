...
class QAP_T4827(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_eliminate_SORPING_STL_GTD_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_Iceberg_order, self.gateway_side_sell, self.status_eliminate)
        er_eliminate_SORPING_STL_GTD_Iceberg_order_params.add_tag(dict(Text='Iceberg synthesisation forbidden by algorithm parameters - child order blocked by internal validation'))
        self.fix_verifier_sell.check_fix_message(er_eliminate_SORPING_STL_GTD_Iceberg_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Eliminate')
        # endregion
