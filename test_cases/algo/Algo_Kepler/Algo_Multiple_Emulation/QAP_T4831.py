...
class QAP_T4831(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_STL_Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt, ClientAlgoPolicyID=self.algopolicy)).add_tag(dict(StopPx=self.stop_price, DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        er_eliminate_SORPING_STL_Iceberg_order_params.add_tag(dict(Text='Iceberg synthesisation forbidden by algorithm parameters - child order blocked by internal validation'))
