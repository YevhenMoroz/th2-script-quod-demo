...
class QAP_T4836(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_STL_MinQty_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_Multiple_Emulation_Kepler_params()
        self.SORPING_STL_MinQty_order.add_ClordId((os.path.basename(__file__)[:-3]))
        self.SORPING_STL_MinQty_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt, ClientAlgoPolicyID=self.algopolicy)).add_tag(dict(StopPx=self.stop_price, MinQty=self.min_qty))

        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_MinQty_order, case_id_1)

        time.sleep(3)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_MinQty_order, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new_SORPING_STL_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_MinQty_order, self.gateway_side_sell, self.status_pending)
        self.fix_verifier_sell.check_fix_message(er_pending_new_SORPING_STL_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport PendingNew')

        er_new_SORPING_STL_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_MinQty_order, self.gateway_side_sell, self.status_new)
        self.fix_verifier_sell.check_fix_message(er_new_SORPING_STL_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport New')
        # endregion
        ...
        self.dma_order.add_tag(dict(StopPx=self.stop_price, MinQty=self.min_qty))
    ...
    def run_post_conditions(self):
        ...
        cancel_request_SORPING_STL_MinQty_order = FixMessageOrderCancelRequest(self.SORPING_STL_MinQty_order)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_MinQty_order, case_id_2)
        self.fix_verifier_sell.check_fix_message(cancel_request_SORPING_STL_MinQty_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel Passive child DMA 2 order")
        # endregion

        er_cancel_SORPING_STL_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_MinQty_order, self.gateway_side_sell, self.status_cancel)
        self.fix_verifier_sell.check_fix_message(er_cancel_SORPING_STL_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
        # endregion
