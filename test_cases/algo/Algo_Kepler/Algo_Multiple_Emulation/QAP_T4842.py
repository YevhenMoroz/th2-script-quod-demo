...
class QAP_T4842(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_Iceberg_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_Multiple_Emulation_Kepler_params()
        self.SORPING_Iceberg_order.add_ClordId((os.path.basename(__file__)[:-3]))
        self.SORPING_Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, ClientAlgoPolicyID=self.algopolicy)).add_tag(dict(DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        er_eliminate_SORPING_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_Iceberg_order, self.gateway_side_sell, self.status_eliminate)
        er_eliminate_SORPING_Iceberg_order_params.add_tag(dict(Text='Iceberg synthesisation forbidden by algorithm parameters - child order blocked by internal validation'))
        self.fix_verifier_sell.check_fix_message(er_eliminate_SORPING_Iceberg_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Eliminate')
        # endregion
