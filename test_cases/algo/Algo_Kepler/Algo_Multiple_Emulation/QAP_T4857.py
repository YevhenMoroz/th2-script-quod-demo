...
class QAP_T4857(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        case_id_1 = bca.create_event("Create SORPING STL GTC Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_1)

        self.SORPING_STL_GTC_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_Multiple_Emulation_Kepler_params()
        self.SORPING_STL_GTC_order.add_ClordId((os.path.basename(__file__)[:-3]))
        self.SORPING_STL_GTC_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_gtc, OrdType=self.order_type_stop_lmt)).add_tag(dict(StopPx=self.stop_price))

        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTC_order, case_id_1)

        time.sleep(3)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_GTC_order, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new_SORPING_STL_GTC_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTC_order, self.gateway_side_sell, self.status_pending)
        self.fix_verifier_sell.check_fix_message(er_pending_new_SORPING_STL_GTC_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport PendingNew')

        er_new_SORPING_STL_GTC_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTC_order, self.gateway_side_sell, self.status_new)
        self.fix_verifier_sell.check_fix_message(er_new_SORPING_STL_GTC_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport New')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        cancel_request_SORPING_STL_GTC_order = FixMessageOrderCancelRequest(self.SORPING_STL_GTC_order)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_GTC_order, case_id_4)
        self.fix_verifier_sell.check_fix_message(cancel_request_SORPING_STL_GTC_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
        ...
        er_cancel_SORPING_STL_GTC_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTC_order, self.gateway_side_sell, self.status_cancel)
        self.fix_verifier_sell.check_fix_message(er_cancel_SORPING_STL_GTC_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
        # endregion
