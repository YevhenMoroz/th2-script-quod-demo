...
class QAP_T4833(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_STL_GTD_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_gtd, OrdType=self.order_type_stop_lmt, ClientAlgoPolicyID=self.algopolicy)).add_tag(dict(ExpireDate=self.ExpireDate, StopPx=self.stop_price))
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit6, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd, OrdType=self.order_type_stop_lmt)).add_tag(dict(ExpireDate=self.ExpireDate, StopPx=self.stop_price))
