...
class QAP_T4819(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.key_params_NOS_child_with_stoppx = self.data_set.get_verifier_key_parameters_by_name("verifier_key_parameters_NOS_child_with_stoppx")
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit6, self.price)
        nos_2_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit6, self.price)
        ...
        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTC_Iceberg_MinQty_order, case_id_1)

        time.sleep(3)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_GTC_Iceberg_MinQty_order, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new_SORPING_STL_GTC_Iceberg_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTC_Iceberg_MinQty_order, self.gateway_side_sell, self.status_pending)
        self.fix_verifier_sell.check_fix_message(er_pending_new_SORPING_STL_GTC_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport PendingNew')

        er_new_SORPING_STL_GTC_Iceberg_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTC_Iceberg_MinQty_order, self.gateway_side_sell, self.status_new)
        self.fix_verifier_sell.check_fix_message(er_new_SORPING_STL_GTC_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport New')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_1_order, key_parameters=self.key_params_NOS_child_with_stoppx, message_name='Buy side NewOrderSingle Child DMA 1 order')
        ...
        # region Modify parent SORPING STL GTC MinQty Iceberg Order
        case_id_2 = bca.create_event("Replace SORPING STL GTC MinQty Iceberg Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)

        self.SORPING_STL_GTC_Iceberg_MinQty_order_replace_params = FixMessageOrderCancelReplaceRequestAlgo(self.SORPING_STL_GTC_Iceberg_MinQty_order)
        self.SORPING_STL_GTC_Iceberg_MinQty_order_replace_params.change_parameters(dict(StopPx=self.inc_stop_price))
        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTC_Iceberg_MinQty_order_replace_params, case_id_2)
        ...
        self.dma_2_order.add_tag(dict(MinQty=self.min_qty, StopPx=self.inc_stop_price, ExpireDate=self.ExpireDate))
        self.fix_verifier_buy.check_fix_message(self.dma_2_order, key_parameters=self.key_params_NOS_child_with_stoppx, message_name='Buy side NewOrderSingle Child DMA 1 order')
    ...
    def run_post_conditions(self):
        ...
        cancel_request_SORPING_STL_GTC_Iceberg_MinQty_order = FixMessageOrderCancelRequest(self.SORPING_STL_GTC_Iceberg_MinQty_order)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_GTC_Iceberg_MinQty_order, case_id_4)
