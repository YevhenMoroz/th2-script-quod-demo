...
class QAP_T4852(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_STL_Iceberg_MinQty_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt)).add_tag(dict(MinQty=self.min_qty, StopPx=self.stop_price, DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit6, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument, OrdType=self.order_type_stop_lmt))
    ...
    def run_post_conditions(self):
        ...
        cancel_request_SORPING_STL_Iceberg_MinQty_order = FixMessageOrderCancelRequest(self.SORPING_STL_Iceberg_MinQty_order)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_Iceberg_MinQty_order, case_id_4)
        self.fix_verifier_sell.check_fix_message(cancel_request_SORPING_STL_Iceberg_MinQty_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
        ...
        er_cancel_SORPING_STL_Iceberg_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_Iceberg_MinQty_order, self.gateway_side_sell, self.status_cancel)
        self.fix_verifier_sell.check_fix_message(er_cancel_SORPING_STL_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
        # endregion
