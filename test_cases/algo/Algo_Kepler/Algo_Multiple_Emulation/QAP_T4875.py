...
class QAP_T4875(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.key_params_NOS_child_with_minqty = self.data_set.get_verifier_key_parameters_by_name("verifier_key_parameters_NOS_child_with_minqty")
    ...
    def run_pre_conditions_and_steps(self):
        ...
        case_id_1 = bca.create_event("Create SORPING STL GTC Iceberg MinQty Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_1)

        self.SORPING_STL_GTD_Iceberg_MinQty_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_Multiple_Emulation_Kepler_params()
        self.SORPING_STL_GTD_Iceberg_MinQty_order.add_ClordId((os.path.basename(__file__)[:-3]))
        self.SORPING_STL_GTD_Iceberg_MinQty_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt, TimeInForce=self.tif_gtd)).add_tag(dict(MinQty=self.min_qty, ExpireDate=self.ExpireDate, StopPx=self.stop_price, DisplayInstruction=dict(DisplayQty=self.display_qty)))

        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTD_Iceberg_MinQty_order, case_id_1)

        time.sleep(3)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_GTD_Iceberg_MinQty_order, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new_SORPING_STL_GTD_Iceberg_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_Iceberg_MinQty_order, self.gateway_side_sell, self.status_pending)
        self.fix_verifier_sell.check_fix_message(er_pending_new_SORPING_STL_GTD_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport PendingNew')

        er_new_SORPING_STL_GTD_Iceberg_MinQty_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_STL_GTD_Iceberg_MinQty_order, self.gateway_side_sell, self.status_new)
        self.fix_verifier_sell.check_fix_message(er_new_SORPING_STL_GTD_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport New')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_1_order, key_parameters=self.key_params_NOS_child_with_minqty, message_name='Buy side NewOrderSingle Child DMA 1 order')
        ...
        self.SORPING_STL_GTD_Iceberg_MinQty_order_replace_params.change_parameters(dict(MinQty=self.dec_min_qty))
        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_STL_GTD_Iceberg_MinQty_order_replace_params, case_id_2)

        time.sleep(1)

        self.fix_verifier_sell.check_fix_message(self.SORPING_STL_GTD_Iceberg_MinQty_order_replace_params, direction=self.ToQuod, message_name='Sell side OrderCancelReplaceRequest')

        er_replaced_SORPING_order_params = FixMessageExecutionReportAlgo().set_params_from_order_cancel_replace(self.SORPING_STL_GTD_Iceberg_MinQty_order_replace_params, self.gateway_side_sell, self.status_cancel_replace)
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.er_cancel_dma_1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child DMA 1 order")
        # endregion
        ...
        self.dma_2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit6, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument, OrdType=self.order_type_stop_lmt, TimeInForce=self.tif_gtd))
        self.dma_2_order.add_tag(dict(MinQty=self.dec_min_qty, StopPx=self.stop_price, ExpireDate=self.ExpireDate))
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_2_order, key_parameters=self.key_params_NOS_child_with_minqty, message_name='Buy side NewOrderSingle Child DMA 2 order')
    ...
    def run_post_conditions(self):
        ...
        cancel_request_SORPING_STL_GTD_Iceberg_MinQty_order = FixMessageOrderCancelRequest(self.SORPING_STL_GTD_Iceberg_MinQty_order)

        self.fix_manager_sell.send_message_and_receive_response(cancel_request_SORPING_STL_GTD_Iceberg_MinQty_order, case_id_4)
        self.fix_verifier_sell.check_fix_message(cancel_request_SORPING_STL_GTD_Iceberg_MinQty_order, direction=self.ToQuod, message_name='Sell side Cancel Request')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.er_cancel_dma_2_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child DMA 2 order")

        time.sleep(10)

        self.fix_verifier_buy.set_case_id(bca.create_event("Check that 2 DMA orders on qdl6 was canceled", self.test_id))
        self.fix_verifier_buy.check_fix_message_sequence([self.er_cancel_dma_1_order, self.er_cancel_dma_2_order], key_parameters_list=[self.key_params_ER_child, self.key_params_ER_child], direction=self.ToQuod, pre_filter=self.pre_filter)
        ...
        self.fix_verifier_sell.check_fix_message(er_cancel_SORPING_STL_GTD_Iceberg_MinQty_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
        # endregion
