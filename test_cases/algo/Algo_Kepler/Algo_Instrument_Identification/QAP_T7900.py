...
class QAP_T7900(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=dict(Symbol=self.symbol, SecurityID=self.sid, SecurityIDSource=self.sids, SecurityType=self.sec_type), ExDestination=self.ex_destination_xbru, DisplayInstruction=dict(DisplayQty=self.display_qty), Side=self.side)).remove_parameter('Currency')
