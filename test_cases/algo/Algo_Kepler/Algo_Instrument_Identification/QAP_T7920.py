...
class QAP_T7920(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.text = constants.RejectMessages.no_listing_1.value
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=dict(Symbol=self.symbol, SecurityID=self.sid, SecurityIDSource=self.sids, SecurityExchange=self.sec_exchange, SecurityType=self.sec_type), Side=self.side, ClientAlgoPolicyID=self.algopolicy)).remove_parameter('Currency')
        ...
        er_reject_SORPING_order_params.remove_parameters(['SecondaryAlgoPolicyID', 'ExecRestatementReason']).add_tag(dict(OrdRejReason='*')).change_parameters(dict(Text=self.text, Price='*'))
