...
class QAP_T7895(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.text = constants.RejectMessages.no_listing_10.value
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=dict(Symbol=self.symbol, SecurityID=self.sid, SecurityIDSource=self.sids, SecurityType=self.sec_type), Currency=self.currency, ExDestination=self.ex_destination_xbru, DisplayInstruction=dict(DisplayQty=self.display_qty), Side=self.side))
