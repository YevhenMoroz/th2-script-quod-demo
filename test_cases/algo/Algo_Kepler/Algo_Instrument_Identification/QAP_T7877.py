...
class QAP_T7877(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.text_1 = constants.RejectMessages.no_listing_4.value
        self.text_2 = constants.RejectMessages.no_listing_1.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=dict(Symbol=self.symbol, SecurityID=self.sid, SecurityIDSource=self.sids, SecurityType=self.sec_type), Currency=self.currency, DisplayInstruction=dict(DisplayQty=self.display_qty), Side=self.side)).remove_parameter('ExDestination')
        ...
        er_reject_SORPING_order_params.remove_parameters(['SecondaryAlgoPolicyID', 'ExecRestatementReason']).add_tag(dict(OrdRejReason='*')).change_parameters(dict(Text=(self.text_1, self.text_2), Price='*'))
