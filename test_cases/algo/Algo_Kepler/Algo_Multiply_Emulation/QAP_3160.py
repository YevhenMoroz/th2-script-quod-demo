...
class QAP_3160(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.SORPING_Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price)).add_tag(dict(DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit6, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument))
    ...
    def run_post_conditions(self):
        ...
        cancel_SORPING_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.SORPING_Iceberg_order, self.gateway_side_sell, self.status_cancel)
        self.fix_verifier_sell.check_fix_message(cancel_SORPING_Iceberg_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Cancel')
        # endregion
