...
class QAP_4541(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_fill_dma_2_qdl1_order.change_parameters(dict(CumQty=self.qty_2_child, LeavesQty=0, LastQty=self.qty_2_child, LastPx=self.price)).remove_parameter('ExDestination').add_tag(dict(ExDestination='*'))
        ...
        er_fill_synthMinQty_order.change_parameters(dict(LastPx=self.price, CumQty=self.qty, LeavesQty=self.leaves_qty, LastQty=self.qty_2_child)).remove_parameters(['ReplyReceivedTime', 'TradeReportingIndicator', 'SecAltIDGrp', 'SecondaryClOrdID']).add_tag(dict(NoStrategyParameters='*', SecondaryAlgoPolicyID='*', LastMkt='*', ChildOrderID='*', Instrument='*', ExDestination='*'))
