...
class QAP_4542(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.dma_qdl1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit1, OrderQty=self.trade_1_qty, Price=self.price, TimeInForce=self.tif_fok))
        ...
        er_fill_dma_qdl1_order.change_parameters(dict(CumQty=self.trade_1_qty, LeavesQty=0, LastQty=self.trade_1_qty, LastPx=self.price, Text='Filled')).remove_parameter('ExDestination')
        ...
        self.dma_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit2, OrderQty=self.trade_2_qty, Price=self.price))
        ...
        er_fill_dma_qdl2_order.change_parameters(dict(CumQty=self.trade_2_qty, LeavesQty=0, LastQty=self.trade_2_qty, LastPx=self.price, Text='Filled')).remove_parameter('ExDestination')
        ...
        er_fill_synthMinQty_order.change_parameters(dict(LastPx=self.price, CumQty=self.qty, LeavesQty=self.leaves_qty, LastQty=self.trade_2_qty, Text='Filled')).remove_parameters(['SecAltIDGrp', 'SecondaryClOrdID']).add_tag(dict(LastMkt='*', ChildOrderID='*', Instrument='*', ExDestination='*'))
