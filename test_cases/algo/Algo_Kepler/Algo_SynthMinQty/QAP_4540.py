...
class QAP_4540(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_fill_dma_qdl1_order.change_parameters(dict(CumQty=self.trade_qty, LeavesQty=0, LastQty=self.trade_qty, LastPx=self.price, Text='Filled')).remove_parameter('ExDestination')
        ...
        er_fill_synthMinQty_order.change_parameters(dict(LastPx=self.price, CumQty=self.trade_qty, LeavesQty=self.leaves_qty, LastQty=self.trade_qty, Text='Filled')).remove_parameters(['SecAltIDGrp', 'SecondaryClOrdID']).add_tag(dict(NoStrategyParameters='*', SecondaryAlgoPolicyID='*', LastExecutionPolicy='*',  LastMkt='*', ChildOrderID='*', Instrument='*', ExDestination='*'))
    ...
    def run_post_conditions(self):
        ...
        er_cancel_synthMinQty_order_params.add_tag(dict(SettlDate='*', MinQty=self.min_qty, NoParty='*', SecondaryAlgoPolicyID='*')).change_parameters(dict(AvgPx=self.price, CxlQty=self.leaves_qty, CumQty=self.trade_qty))
