...
class QAP_T4068(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_fok_rule = rule_manager.add_NewOrdSingle_FOK(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, True, self.price_bid_qdl1, self.delay)
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_qdl2 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl2, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl2.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.passive_price, MDEntrySize=self.qty_bid)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl2)
        ...
        self.dma_1_qdl1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit1, OrderQty=self.trade_qty, Price=self.price_bid_qdl1, TimeInForce=self.tif_fok))
        ...
        er_fill_dma_1_qdl1_order.change_parameters(dict(CumQty=self.trade_qty, LeavesQty=0, LastQty=self.trade_qty, LastPx=self.price_bid_qdl1))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_fill_dma_qdl2_order, self.key_params_ER_fill_child, self.ToQuod, "Buy Side ExecReport Fill Aggressive DMA 2 order")
        # endregion
        ...
        # region Check PartiallyFill algo order
