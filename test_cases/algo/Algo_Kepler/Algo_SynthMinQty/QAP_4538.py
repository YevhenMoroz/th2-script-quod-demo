...
class QAP_4538(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_pending_new_dma_qdl1_order_params.change_parameters(dict(ExDestination=self.ex_destination_quodlit1, LeavesQty=0))
        ...
        er_new_dma_qdl1_order_params.change_parameters(dict(ExDestination=self.ex_destination_quodlit1, LeavesQty=0))
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl1_order, self.key_params_ER_fill_child, self.ToQuod, "Buy Side ExecReport Fill first DMA 1 order")
        # endregion
        ...
        er_fill_synthMinQty_order.change_parameters(dict(LastPx=self.price, CumQty=self.qty, LeavesQty=0, LastQty=self.qty, Text='Filled')).remove_parameters(['ReplyReceivedTime', 'TradeReportingIndicator', 'SecAltIDGrp', 'SecondaryClOrdID']).add_tag(dict(MinQty=self.min_qty, IClOrdIdAO='*', SecondaryAlgoPolicyID='*', ShortCode='*', ChildOrderID='*', CFICode='*'))
