...
class QAP_T4782(TestCase):
    ...
    def     __init__(self, report_id, data_set=None, environment=None):
        ...
        self.weight_bats = 4
        self.weight_chix = 6
        self.qty_chix_child, self.qty_bats_child = AlgoFormulasManager.get_child_qty_on_venue_weights(self.qty - self.traded_qty, None, self.weight_chix, self.weight_bats)
        self.qty_chix_child2, self.qty_bats_child2 = AlgoFormulasManager.get_child_qty_on_venue_weights(self.inc_qty - self.traded_qty, None, self.weight_chix, self.weight_bats)
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_partial_filled_chix, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child order')
        # endregion
        ...
        # BATSDARK
        self.dma_bats_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_Dark_Child_Kepler_params()
        self.dma_bats_order.change_parameters(dict(Account=self.client_bats_dark, ExDestination=self.ex_destination_bats_dark, OrderQty=self.qty_bats_child2))
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_bats_order, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle dark child on bats_dark')
    ...
    def run_post_conditions(self):
        ...
        er_cancel_mp_dark_order_params.add_tag(dict(SettlDate='*')).add_tag(dict(NoParty='*')).change_parameters(dict(CxlQty=self.inc_qty - self.traded_qty, AvgPx=self.price, CumQty=self.traded_qty, OrderQty=self.inc_qty, SettlType="*"))
