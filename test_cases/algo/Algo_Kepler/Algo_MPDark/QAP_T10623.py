...
class QAP_T10623(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_new_dma_bats_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_chix_order, self.gateway_side_buy, self.status_new)
        self.fix_verifier_buy.check_fix_message_kepler(er_new_dma_bats_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New 2nd child DMA order on the CHIXDELTA')
        # endregion
        ...
        self.dma_chix_order.change_parameters(dict(OrderQty=self.dec_qty))
        ...
        self.nos_2_chixlis_rfq = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_RFQ_params().change_parameters(dict(Account=self.client, OrderQty=self.dec_qty, ExDestination=self.ex_destination_chixlis, Instrument='*'))
        ...
        self.nos_2_trql_rfq = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_RFQ_params().change_parameters(dict(Account=self.client, OrderQty=self.dec_qty, ExDestination=self.ex_destination_trql, Instrument='*'))
        ...
        self.dma_bats_order.change_parameters(dict(Account=self.account_bats, ExDestination=self.ex_destination_bats, OrderQty=self.dec_qty, Instrument=self.instrument))
