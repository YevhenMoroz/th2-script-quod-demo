...
class QAP_T10534(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_replace_dma_2_chixdelta_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_2_chixdelta_order, self.gateway_side_buy, self.status_cancel_replace)
        er_replace_dma_2_chixdelta_order_params.change_parameters(dict(OrderQty=self.new_leaves_qty))
        self.fix_verifier_buy.check_fix_message_kepler(er_replace_dma_2_chixdelta_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport CancelReplace dark child on the CHIXDELTA')
        # endregion
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Cancel a replaced dark child order", self.test_id))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_2_chixdelta_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Cancel dark child on the CHIXDELTA')
        # endregion
