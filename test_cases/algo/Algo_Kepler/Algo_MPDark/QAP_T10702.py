...
class QAP_T10702(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_2_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account_bats, self.ex_destination_bats, self.dec_price)
        ...
        self.rule_list = [nos_1_rule, ocrr_rule, nos_2_rule, ocr_1_rule, ocr_2_rule, rfq_ocr_rule]
        # endregion
        ...
        nos_chixlis_rfq = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_RFQ_params().change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, ExDestination=self.ex_destination_chixlis, Instrument='*'))
        ...
        nos_trql_rfq = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_RFQ_params().change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, ExDestination=self.ex_destination_trql, Instrument='*'))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_chix_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle 1st child DMA order on the CHIXDELTA')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_pending_new_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew 2nd child DMA order on the CHIXDELTA')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_new_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New 2nd child DMA order on the CHIXDELTA')
        # endregion
        ...
        self.MPDark_order_replace_params.change_parameters(dict(Price=self.dec_price))
        ...
        self.dma_chix_order.change_parameters(dict(Price=self.dec_price))

        # region Check replace first dma child order
        ocrr_dma_chix_order = FixMessageOrderCancelReplaceRequestAlgo(self.dma_chix_order)
        ocrr_dma_chix_order.change_parameters(dict(Price=self.dec_price))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_replace_dma_chix_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport CancelReplace child DMA 2 order on venue CHIX DARKPOOL UK")
        # endregion
        ...
        self.dma_bats_order.change_parameters(dict(Account=self.account_bats, ExDestination=self.ex_destination_bats, OrderQty=self.qty, Price=self.dec_price, Instrument=self.instrument))
        ...
        # region Check those are no new RFQs
        self.fix_verifier_buy.set_case_id(bca.create_event("Check those are no new RFQs", self.test_id))

        self.dma_chix_order.change_parameters(dict(Price=self.price))
        self.fix_verifier_buy.check_fix_message_sequence_kepler([nos_chixlis_rfq, nos_trql_rfq, self.dma_chix_order, self.dma_bats_order], [self.key_params_NOS_child, self.key_params_NOS_child, self.key_params_NOS_child, self.key_params_NOS_child], self.FromQuod, pre_filter=self.pre_filter, check_order=False)
        # endregion
