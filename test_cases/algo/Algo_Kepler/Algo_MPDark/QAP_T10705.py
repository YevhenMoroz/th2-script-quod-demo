...
class QAP_T10705(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price = 1
        self.inc_price = 1000
        self.delay_for_accepted_rfq = 4000
        self.delay_for_cancel_rfq = 6000
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ocrr_rule = rule_manager.add_OrderCancelReplaceRequest(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, False)
        rfq_rule = rule_manager.add_NewOrdSingleRFQExecutionReport(self.fix_env1.buy_side, self.client, self.ex_destination_chixlis, self.qty, self.qty, self.new_reply, self.restated_reply, self.delay_for_accepted_rfq)
        ...
        # region Check reject replace first dma child order
        ocrr_dma_chix_order = FixMessageOrderCancelReplaceRequestAlgo(self.dma_chix_order)
        ocrr_dma_chix_order.change_parameters(dict(Price=self.inc_price))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_reject_replace_dma_chix_order, self.key_params_ER_cancel_reject_child, self.ToQuod, "Buy Side ExecReport RejectReplace child DMA order on venue CHIX DARKPOOL UK")
        # endregion
        
        # region quote canceled on LIS venues
        self.fix_verifier_buy.set_case_id(bca.create_event("Cancel requests on RFQs received", self.test_id))
        ...
        # region Check that are no Cancel request on DMA child
