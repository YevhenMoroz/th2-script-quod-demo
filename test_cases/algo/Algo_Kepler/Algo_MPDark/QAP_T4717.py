...
class QAP_T4717(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_eliminate_rule = rule_manager.add_NewOrderSingle_ExecutionReport_Eliminate(self.fix_env1.buy_side, self.account_bats, self.ex_destination_bats, self.price)
        nos_2_eliminate_rule = rule_manager.add_NewOrderSingle_ExecutionReport_Eliminate(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, self.price)
        nos_3_eliminate_rule = rule_manager.add_NewOrderSingle_ExecutionReport_Eliminate(self.fix_env1.buy_side, self.account_itg_cboe_tqdarkeu, self.ex_destination_itg, self.price)
        self.rule_list = [nos_1_eliminate_rule, nos_2_eliminate_rule, nos_3_eliminate_rule]
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_eliminate_dma_chix_order, self.key_params_ER_eliminate_child, self.ToQuod, "Buy Side ExecReport Eliminate child DMA 1 order")
        # endregion
        ...
        # region check eliminate second dma child order
        er_eliminate_dma_bats_order = FixMessageExecutionReportAlgo().set_params_for_nos_eliminate_rule(self.dma_bats_order)
        self.fix_verifier_buy.check_fix_message_kepler(er_eliminate_dma_bats_order, self.key_params_ER_eliminate_child, self.ToQuod, "Buy Side ExecReport Eliminate child DMA 2 order")
        # endregion
