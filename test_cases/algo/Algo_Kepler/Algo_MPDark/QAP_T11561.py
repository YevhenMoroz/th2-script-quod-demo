...
class QAP_T11561(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.rest_api_manager.modify_strategy_parameter(self.algopolicy, "DarkPoolWeights", AlgoFormulasManager.create_string_for_strategy_weight(dict(CHIXDELTA=6)))
        # endregion
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check that are only one dark child on venues defined in the AllowedVenues were created", self.test_id))

        self.fix_verifier_buy.check_fix_message_sequence_kepler([self.dma_chix_order], [self.key_params_NOS_child], self.FromQuod, pre_filter=self.pre_filter)
        # endregion
