...
class QAP_T4713(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_market_rule = rule_manager.add_NewOrdSingle_Market(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, True, self.qty_chix_child, self.price_ask)
        nos_2_market_rule = rule_manager.add_NewOrdSingle_Market(self.fix_env1.buy_side, self.account_bats, self.ex_destination_bats, True, self.qty_bats_child, self.price_ask)
        ...
        self.rule_list = [nos_1_market_rule, nos_2_market_rule, ocr_1_rule, ocr_2_rule]
        # endregion
        ...
        self.dma_chix_order.change_parameters(dict(Account=self.account_chix, ExDestination=self.ex_destination_chix, OrderQty=self.qty_chix_child, OrdType=self.order_type)).remove_parameter('Price')
        self.fix_verifier_buy.check_fix_message(self.dma_chix_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Child DMA order on venue CHIX DARKPOOL UK')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA order on venue CHIX DARKPOOL UK')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA order on venue CHIX DARKPOOL UK')
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Fill Child DMA order on venue CHIX DARKPOOL UK')
        # endregion
        ...
        self.dma_bats_order.change_parameters(dict(Account=self.account_bats, ExDestination=self.ex_destination_bats, OrderQty=self.qty_bats_child, OrdType=self.order_type)).remove_parameter('Price')
        self.fix_verifier_buy.check_fix_message(self.dma_bats_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Child DMA order on venue BATS DARKPOOL UK')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_bats_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA order on venue BATS DARKPOOL UK')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_bats_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA order on venue BATS DARKPOOL UK')
        ...
        er_fill_dma_bats_order_params.add_tag(dict(Price=self.price_ask))
        self.fix_verifier_buy.check_fix_message(er_fill_dma_bats_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Fill Child DMA order on venue BATS DARKPOOL UK')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message_sequence([self.dma_chix_order, self.dma_bats_order], key_parameters_list=[None, None], direction=self.FromQuod, pre_filter=self.pre_filter)
        # endregion
