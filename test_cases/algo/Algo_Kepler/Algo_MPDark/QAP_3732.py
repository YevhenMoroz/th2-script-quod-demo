...
class QAP_3732(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.MP_Dark_order_replace_params.change_parameters(dict(MinQty=self.dec_minQty)).add_tag(dict(IClOrdIdAO='OD_5fgfDXg-00', ShortCode='17536'))
        ...
        er_replaced_MP_Dark_order_params.add_tag(dict(SettlDate='*')).add_tag(dict(SettlType='*')).add_tag(dict(NoParty='*')).add_tag(dict(MinQty=self.dec_minQty))
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message(er_cancel_dma_2_chix_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child DMA 3 order on venue CHIX DARKPOOL UK")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_cancel_dma_2_bats_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child DMA 4 order on venue BATS DARKPOOL UK")
        # endregion
        ...
        er_cancel_mp_dark_order_params.add_tag(dict(SettlDate='*')).add_tag(dict(SettlType='*')).add_tag(dict(MinQty=self.dec_minQty)).add_tag(dict(NoParty='*'))
