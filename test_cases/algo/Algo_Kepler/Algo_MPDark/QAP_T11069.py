...
class QAP_T11069(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.traded_qty = 3500000
        self.leaves_qty = self.inc_qty - self.traded_qty
        self.price = 20
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_trade_rule = rule_manager.add_NewOrdSingleExecutionReportTradeByOrdQty(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, self.price, self.price, self.qty, self.traded_qty, self.delay_for_trade)
        ocrr_rule = rule_manager.add_OrderCancelReplaceRequest(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, True, self.delay_for_amend)
        ...
        self.rule_list = [nos_1_rule, nos_trade_rule, ocrr_rule, nos_2_rule, ocr_1_rule, ocr_2_rule, rfq_ocr_rule]
        # endregion
        ...
        er_pfl_dma_chix_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_chix_order, self.gateway_side_buy, self.status_partial_fill)
        self.fix_verifier_buy.check_fix_message_kepler(er_pfl_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Partial fill child DMA order on the CHIXDELTA')
        # endregion

        # region Check that the parent order is partially filled
        er_pfl_MPDark_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.MPDark_order, self.gateway_side_sell, self.status_partial_fill)
        self.fix_verifier_sell.check_fix_message(er_pfl_MPDark_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Partial fill')
        # endregion
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check that the modification was accepted on buy-side", self.test_id))

        self.dma_chix_order.change_parameters(dict(OrderQty=self.inc_qty))

        er_replace_dma_chix_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_chix_order, self.gateway_side_buy, self.status_cancel_replace)
        self.fix_verifier_buy.check_fix_message_kepler(er_replace_dma_chix_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Replace child DMA 1 order on venue CHIX DARKPOOL UK")
        # endregion
