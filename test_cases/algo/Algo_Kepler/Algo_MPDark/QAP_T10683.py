...
class QAP_T10683(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check that the child is filled", self.test_id))
        er_filled_dma_chix_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_chix_order, self.gateway_side_buy, self.status_fill)
        self.fix_verifier_buy.check_fix_message_kepler(er_filled_dma_chix_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Fill Child DMA 1 order')
        # endregion

        # region Check that the parent is PFL
        self.fix_verifier_sell.set_case_id(bca.create_event("Check that the parent is filled", self.test_id))
        er_filled_MP_Dark_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.MP_Dark_order, self.gateway_side_sell, self.status_fill)
        self.fix_verifier_sell.check_fix_message(er_filled_MP_Dark_order_params, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport Fill')
        # endregion
