...
class QAP_T4587(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.qty_after_trade = self.qty - self.trade_qty - self.trade_qty_2
    ...
    def run_pre_conditions_and_steps(self):
        ...
        cancel_rule_CHID = rule_manager.add_OrderCancelRequestTradeCancel(self.fix_env1.buy_side, self.client_chix_delta, self.ex_destination_chix_dark, self.price)
        ...
        # region Dark order partial filled
        ...
        # region check cancel child orders

        self.fix_verifier_buy.set_case_id(bca.create_event("Dark venue orders canceled", self.test_id))

        er_part_filled_chixdelta = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_chix_order, side=GatewaySide.Buy, status=Status.PartialFill).change_parameter("OrigClOrdID", "*")
        self.fix_verifier_buy.check_fix_message_kepler(er_part_filled_chixdelta, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PartFill Child order on ChixDelta')
        ...
        er_cancel_bats_dark = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_bats_order, self.gateway_side_buy, self.status_cancel)
