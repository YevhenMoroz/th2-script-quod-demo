...
class QAP_T10526(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        rfq_rule = rule_manager.add_NewOrdSingleRFQExecutionReport(self.fix_env1.buy_side, self.client, self.ex_destination_chixlis, self.qty, self.qty, self.new_reply, self.restated_reply, self.delay)
        rfq_ocr_2_rule = rule_manager.add_OrderCancelRequestRFQExecutionReport(self.fix_env1.buy_side, self.client, self.ex_destination_trqx, True)
        ...
        self.rule_list = [rfq_rule, rfq_ocr_2_rule, nos_1_rule, ocr_2_rule, nos_2_rule, ocr_1_rule]
        # endregion
        ...
        # region Check dark child order
        self.fix_verifier_buy.set_case_id(bca.create_event("After LIS time is over algo generates the child order on the Dark venue", self.test_id))
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_chixdelta_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Dark child on CHIXDELTA')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_chixdelta_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew dark child on chixdelta')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_chixdelta_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New dark child on chixdelta')
        # endregion
        ...
        # region Check child order on the venue CHIXLIS
        self.fix_verifier_buy.set_case_id(bca.create_event("Child order on the CHIXLIS", self.test_id))

        self.nos_chixlis_order = FixMessageNewOrderSingleAlgo().set_DMA_after_RFQ_params()
        self.nos_chixlis_order.change_parameters(dict(ExDestination=self.ex_destination_chixlis))
        self.fix_verifier_buy.check_fix_message(self.nos_chixlis_order, key_parameters=self.key_params_NOS_LIS, message_name='Buy side send child order on the CHIXLIS', direction=self.FromQuod)
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_2_chixlis_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA order on the CHIXLIS')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_2_chixlis_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA order on the CHIXLIS')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        # region Check that the child order on LIS venue was canceled
        er_new_dma_2_chixlis_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.nos_chixlis_order, self.gateway_side_buy, self.status_cancel)
        self.fix_verifier_buy.check_fix_message(er_new_dma_2_chixlis_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Cancel Child DMA order on the CHIXLIS')
        # endregion
