...
class QAP_T10524(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ocr_1_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, False)
        ...
        self.fix_verifier_sell.check_fix_message(er_cancel_reject_mp_dark_order_params, key_parameters=self.key_params_ER_cancel_reject_parent, message_name='Sell side ExecReport CancelReject')
        ...
        self.ocr_2_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account_chix, self.ex_destination_chix, True)

    @try_except(test_id=Path(__file__).name[:-3])
    def run_post_conditions(self):
        # region Cancel Algo Order again (to prevent the child expiring)
        case_id_2 = bca.create_event("Cancel Algo Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)
        cancel_request_MP_Dark_order = FixMessageOrderCancelRequest(self.MP_Dark_order)
        ...
        rule_manager = RuleManager(Simulators.algo)
        rule_manager.remove_rule(self.ocr_2_rule)
