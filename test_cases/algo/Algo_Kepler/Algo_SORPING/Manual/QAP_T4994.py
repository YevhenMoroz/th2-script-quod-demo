...
# Warning! This is the manual test case. It needs to do manual checking audit trail and doesn`t include in regression script
class QAP_T4994(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_3_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, self.dec_price)
        nos_4_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, self.inc_price_after_dec)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, True)
        self.rule_list = [nos_1_rule, nos_2_rule, nos_3_rule, nos_4_rule, ocr_rule]
        # endregion
        ...
        # region Modify parent Lit Dark order
        case_id_2 = bca.create_event("First replace Lit Dark Order: increase price", self.test_id)
        ...
        case_id_2 = bca.create_event("Second replace MP Dark Order: decrease price", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)
        ...
        # region Modify parent MP Dark order
        case_id_2 = bca.create_event("Third replace MP Dark Order: increase price", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)

        self.SORPING_order_replace_params = FixMessageOrderCancelReplaceRequestAlgo(self.SORPING_order)
        self.SORPING_order_replace_params.change_parameters(dict(Price=self.inc_price_after_dec))
        self.fix_manager_sell.send_message_and_receive_response(self.SORPING_order_replace_params, case_id_2)
