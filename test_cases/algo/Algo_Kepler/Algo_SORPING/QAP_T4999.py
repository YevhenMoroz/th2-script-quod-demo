...
class QAP_T4999(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # self.param2 = constants.MiscNumber.ordr_misc_6.value
        # self.param3 = constants.ReadLogParams.cl_ord_id.value
        # self.party_id = constants.PartyID.party_id_4.value
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_par= FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl2, self.fix_env1.feed_handler)
        ...
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask_qdl2, MDEntrySize=self.qty_for_md)
        ...
        self.SORPING_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_SORPING_Iceberg_Kepler_params_with_PartyInfo()
        ...
        # compare_message.change_parameters(dict(Parameter1=self.param1, Value1=self.party_id, Parameter2=self.param2, Value2=self.party_id, Parameter3=self.param3, Value3=self.ClOrdId))
        #
        # self.read_log_verifier.set_case_id(bca.create_event("ReadLog", self.test_id))
        ...
        self.dma_qdl1_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_child_of_SORPING_Iceberg_Kepler_params_with_PartyInfo()
        self.dma_qdl1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_qdl1, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument)).add_tag(dict(AlgoCst01=self.party_id))
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel Child DMA order")
        # endregion
