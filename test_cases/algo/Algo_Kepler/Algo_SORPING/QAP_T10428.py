...
class QAP_T10428(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_reject_rule = rule_manager.add_NewOrderSingle_ExecutionReport_RejectWithReason(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, self.price, self.reason, delay=self.delay)
        ...
        # region Update the MD
        market_data_snap_shot_qdl1 = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.listing_id_qdl1, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=self.px_for_incr, MDEntrySize=self.qty_for_incr, TradingSessionSubID=4)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl1)
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_SORPING_order_params.add_tag(dict(LastMkt='*', Text='*')).remove_parameter('OrigClOrdID')
