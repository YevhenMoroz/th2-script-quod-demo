...
class QAP_T4975(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.qty = 1500
        self.child_1_qty = 1000
        self.child_2_qty = 500
        self.price = 25
        self.dark_price = 44
        self.traded_qty = 0
        ...
        self.listing_id_qdl3 = self.data_set.get_listing_id_by_name("listing_")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_3_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, True, self.child_1_qty, self.price_bid)
        nos_4_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit2, True, self.child_2_qty, self.price_bid)
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, ClientAlgoPolicyID=self.algopolicy, Side=self.sell))
        ...
        # region Check first Lit child DMA order
        ...
        self.dma_qdl1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit1, OrderQty=self.child_1_qty, Price=self.price_bid, TimeInForce=self.tif_ioc, Side=self.sell))
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA 3 order')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 3 order')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl1_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Fill child DMA 3 order")
        # endregion

        # region Check second Lit child DMA order
        self.dma_qdl2_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_Child_of_SORPING_params()
        self.dma_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit2, OrderQty=self.child_2_qty, Price=self.price_bid, TimeInForce=self.tif_ioc, Side=self.sell))
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_qdl2_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA 4 order')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_qdl2_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 4 order')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl2_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Fill child DMA 4 order")
        # endregion
