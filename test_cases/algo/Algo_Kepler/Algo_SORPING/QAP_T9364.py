...
class QAP_T9364(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.text_2 = "Venue Euronext Paris is in phase CLO"
        self.text_3 = "the algorithm will stop now, reference market is closed"
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_eliminate_SORPING_order_params.add_tag(dict(Text="reference market is closed"))
        ...
        self.read_log_verifier.set_case_id(bca.create_event("Check that the dark phase is skipping", self.test_id))
        self.read_log_verifier.check_read_log_message(compare_message_1, self.key_params_readlog)

        self.read_log_verifier.set_case_id(bca.create_event("Check that primary venue goes to the CLO phase", self.test_id))
        self.read_log_verifier.check_read_log_message(compare_message_2, self.key_params_readlog)

        self.read_log_verifier.set_case_id(bca.create_event("Check that algorithm will stop now", self.test_id))
