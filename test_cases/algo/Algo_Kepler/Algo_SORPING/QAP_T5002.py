...
class QAP_T5002(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_3_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit2, True, self.qty_for_md, self.inc_price)
        ...
        self.rule_list = [nos_dark_1_ioc_rule, nos_dark_2_ioc_rule, nos_1_ioc_rule, nos_2_ioc_rule, nos_1_rule, nos_2_rule, nos_3_rule, nos_3_ioc_rule, ocrr_rule, ocr_rule]
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_eliminate_dma_qdpkp1_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Eliminate Dark child DMA order on venue QUODDKP1")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_eliminate_dma_qdpkp2_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Eliminate Dark child DMA order on venue QUODDKP2")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_1_qdl1_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit aggressive Child DMA order on venue QDL1: price 44')

        er_fill_dma_qdl1_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_1_qdl1_order, self.gateway_side_buy, self.status_fill)
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Fill Lit aggressive Child DMA order on venue QDL1: price 44")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_1_qdl2_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit aggressive Child DMA order on venue QDL2: price 40')
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_1_qdl2_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Fill Lit aggressive Child DMA order on venue QDL2: price 40")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_2_qdl1_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit passive Child DMA order on venue QDL1: Qty=13000, price=45')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_2_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Lit passive Child DMA order on venue QDL1: Qty=13000, price=45')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_2_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Lit passive Child DMA order on venue QDL1: Qty=13000, price=45')
        # endregion

        # region Modify parent MP Dark order
        case_id_2 = bca.create_event("First replace MP Dark Order: decrease price", self.test_id)
        ...
        market_data_snap_shot_qdl1 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl1, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl1, MDEntrySize=self.qty_for_md)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=55, MDEntrySize=self.qty_for_md)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl1)

        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_qdl2 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl2, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl2.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl2, MDEntrySize=self.qty_for_md)
        market_data_snap_shot_qdl2.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=52, MDEntrySize=self.qty_for_md)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl2)
        # endregion
        ...
        # region Check replace passive child DMA order on venue QUODLIT1
        ...
        self.fix_verifier_buy.check_fix_message(er_replace_dma_2_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Replace Lit passive Child DMA order on venue QDL1: OldQty=13000, NewQty=12000, price=45")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_replace_dma_2_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Lit passive Child DMA order on venue QDL1: OldQty=13000, NewQty=12000, price=45")
        # endregion

        # region Check 2 child DMA order on venue QUODLIT2
        self.fix_verifier_buy.set_case_id(bca.create_event("Aggressive child DMA order on venue QDL2 after decrease price", self.test_id))
        self.dma_2_qdl2_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_Child_of_SORPING_params()
        self.dma_2_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit2, OrderQty=self.qty_for_md, Price=self.dec_price, TimeInForce=self.tif_ioc))
        self.fix_verifier_buy.check_fix_message(self.dma_2_qdl2_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit aggressive Child DMA order on venue QDL2: price 40')
        ...
        self.fix_verifier_buy.check_fix_message(er_eliminate_dma_2_qdl2_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport fill Lit aggressive Child DMA order on venue QDL2: price 40")
        # endregion

        # region Check 3 child DMA order on venue QUODLIT1
        self.fix_verifier_buy.set_case_id(bca.create_event("Passive child DMA order on venue QDL1 after decrease price", self.test_id))
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_3_qdl1_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit passive Child DMA order on venue QDL1: OldQty=12000, price=40')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_3_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Lit passive Child DMA order on venue QDL1: OldQty=12000, price=40')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_3_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Lit passive Child DMA order on venue QDL1: OldQty=12000, price=40')
        # endregion

        # region Modify parent MP Dark order
        case_id_2 = bca.create_event("Second replace MP Dark Order: increase price", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)
        ...
        market_data_snap_shot_qdl1 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl1, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl1, MDEntrySize=self.qty_for_md)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=100, MDEntrySize=self.qty_for_md)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl1)

        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_qdl2 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl2, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl2.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl2, MDEntrySize=self.qty_for_md)
        market_data_snap_shot_qdl2.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=100, MDEntrySize=self.qty_for_md)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl2)
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_replace_dma_2_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Replace Lit passive Child DMA order on venue QDL1: OldQty=12000, NewQty=11000, price=45")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_replace_dma_3_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel Lit passive Child DMA order on venue QDL1: OldQty=12000, NewQty=11000, price=45")
        # endregion

        # region Check 3 child DMA order on venue QUODLIT2
        self.fix_verifier_buy.set_case_id(bca.create_event("Aggressive child DMA order on venue QDL2 after increase price", self.test_id))
        self.dma_3_qdl2_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_Child_of_SORPING_params()
        self.dma_3_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit2, OrderQty=self.qty_for_md, Price=self.inc_price, TimeInForce=self.tif_ioc))
        self.fix_verifier_buy.check_fix_message(self.dma_3_qdl2_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit aggressive Child DMA order on venue QDL2: price 52')
        ...
        self.fix_verifier_buy.check_fix_message(er_eliminate_dma_3_qdl2_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport fill Lit aggressive Child DMA order on venue QDL2: price 52")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_4_qdl1_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Lit passive Child DMA order on venue QDL1: OldQty=11000, price=52')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_4_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Lit passive Child DMA order on venue QDL1: OldQty=11000, price=52')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma_4_qdl1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Lit passive Child DMA order on venue QDL1: OldQty=11000, price=52')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message(er_cancel_dma_4_qdl1_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Cancel Lit passive Child DMA order on venue QDL1: OldQty=11000, price=52")
        # endregion
