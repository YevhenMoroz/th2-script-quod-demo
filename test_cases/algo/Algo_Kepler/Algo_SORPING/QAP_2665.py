...
class QAP_2665(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_dark_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_quoddkp1, False, self.traded_qty, self.dark_price)
        ...
        self.rule_list = [nos_dark_ioc_rule, nos_1_ioc_rule, nos_2_ioc_rule, nos_rule, nos_3_ioc_rule, ocrr_rule, ocr_rule]
        # endregion
        ...
        market_data_snap_shot_qdl1 = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id_qdl1, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl1, MDEntrySize=self.qty_for_md)
        market_data_snap_shot_qdl1.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.new_price_ask_qdl1, MDEntrySize=self.qty_for_md)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl1)
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl1__order, self.key_params_ER_fill_child, self.ToQuod, "Buy Side ExecReport Fill child DMA 1 order")
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_1_qdl2_order, self.key_params_ER_fill_child, self.ToQuod, "Buy Side ExecReport Fill child DMA 2 order")
        # endregion
        ...
        # er_partial_fill_dma_2_qdl1_order.change_parameters(dict(CumQty=self.qty_for_md, LeavesQty=self.dec_qty_passive_child, LastQty=self.qty_2_ask_qdl2, LastPx=self.price_2_ask_qdl2))
        ...
        # er_partial_fill_SORPING_order.change_parameters(dict(LastPx=self.price_2_ask_qdl2, CumQty=self.qty_2_ask_qdl2, LeavesQty=self.leaves_parent_qty, LastQty=self.qty_2_ask_qdl2))
        ...
        # self.dma_2_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit2, OrderQty=self.qty_for_liquidity_qdl2, Price=self.price_for_liquidity_qdl2, TimeInForce=self.tif_iok))
        ...
        # self.fix_verifier_buy.check_fix_message(er_fill_dma_2_qdl2_order, self.key_params_ER_fill_child, self.ToQuod, "Buy Side ExecReport fill child DMA 4 order")
        # # endregion
        #
