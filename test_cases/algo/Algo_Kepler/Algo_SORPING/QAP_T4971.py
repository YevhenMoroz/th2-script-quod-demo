...
class QAP_T4971(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.dma_qdl4_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit4, OrderQty=self.qty, Price=self.price, Side=self.sell, Instrument=self.instrument))
    ...
    def run_post_conditions(self):
        ...
        er_cancel_dma_qdl4_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_qdl4_order, self.gateway_side_buy, self.status_cancel)
        self.fix_verifier_buy.check_fix_message(er_cancel_dma_qdl4_order, self.key_params_ER_eliminate_or_cancel_child, self.ToQuod, "Buy Side ExecReport Cancel Lit passive Child DMA order on venue qdl4: OldQty=11000, price=52")
        # endregion
