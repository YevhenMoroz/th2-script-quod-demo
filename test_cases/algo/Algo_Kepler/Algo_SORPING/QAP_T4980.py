...
class QAP_T4980(TestCase):
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.er_cancel_dma_2_xpar_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child DMA 2 order")
        # endregion

        time.sleep(10)

        self.fix_verifier_buy.set_case_id(bca.create_event("Check that 2 DMA orders on qdl6 was canceled", self.test_id))
        self.fix_verifier_buy.check_fix_message_sequence_kepler([self.er_cancel_dma_1_xpar_order, self.er_cancel_dma_2_xpar_order], key_parameters_list=[None, None], direction=self.ToQuod, pre_filter=self.pre_filter)
