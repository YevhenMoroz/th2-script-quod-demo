...
class QAP_T4963(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.dark_price = 40
        self.traded_qty = 0
        self.qty_for_md_qdl6 = 400
        self.qty_for_md_qdl7 = 200
        self.price_ask = 40
        self.price_bid_qdl6 = 10
        self.price_bid_qdl7 = 12
        self.px_for_incr = 0
        self.qty_for_incr = 0
    ...
    def run_pre_conditions_and_steps(self):
        ...
        market_data_snap_shot_qdl6.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl6, MDEntrySize=self.qty_for_md_qdl6)
        ...
        market_data_snap_shot_qdl7.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_qdl7, MDEntrySize=self.qty_for_md_qdl7)
        ...
        self.SORPING_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, ClientAlgoPolicyID=self.algopolicy, Instrument=self.instrument, Side=self.side)).add_tag(dict(MinQty=self.min_qty))
        ...
        self.dma_qdpkp1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quoddkp1, OrderQty=self.qty, Price=self.dark_price, Instrument=self.instrument, TimeInForce=self.tif_ioc, Side=self.side)).add_tag(dict(MinQty=self.min_qty))
        ...
        self.dma_qdpkp2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quoddkp2, OrderQty=self.qty, Price=self.dark_price, Instrument=self.instrument, TimeInForce=self.tif_ioc, Side=self.side)).add_tag(dict(MinQty=self.min_qty))
        ...
        self.dma_qdl6_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_Child_of_SORPING_Kepler_params()
        ...
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_qdl6_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Child DMA 1 aggressive order')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_pending_new_dma_qdl6_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Lit aggressive Child DMA 1 order')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_new_dma_qdl6_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child Lit aggressive DMA 1 order')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_fill_dma_qdl6_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Fill Lit aggressive Child DMA 1 order")
        # endregion
