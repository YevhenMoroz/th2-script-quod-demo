...
class QAP_T4797(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_qdl1, True, self.qty, self.price_ask_qdl1)
        ...
        self.dma_qdl1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_qdl1, OrderQty=self.qty, Price=self.price_ask_qdl1, Instrument=self.instrument, TimeInForce=self.tif_ioc))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_fill_dma_qdl1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Fill Lit passive Child DMA order")
        # endregion
