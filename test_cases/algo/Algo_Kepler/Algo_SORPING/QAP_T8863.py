...
class QAP_T8863(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_chixdelta, self.ex_destination_chixdelta, False, self.traded_qty, self.dark_price)
        nos_2_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_batsdark, self.ex_destination_batsdark, False, self.traded_qty, self.dark_price)
        nos_3_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_xpar, True, self.qty, self.price_ask)
        ...
        er_eliminate_dma_qdpkp1_order.add_tag(dict(ExDestination=self.ex_destination_chixdelta))
        ...
        er_eliminate_dma_qdpkp2_order.add_tag(dict(ExDestination=self.ex_destination_batsdark))
        ...
        self.dma_qdl6_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_xpar, OrderQty=self.qty, Price=self.price_ask, Instrument=self.instrument, TimeInForce=self.tif_ioc)).add_tag(dict(MinQty=self.min_qty))
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_qdl6_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Child DMA 3 order')

        er_fill_dma_qdl6_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_qdl6_order, self.gateway_side_buy, self.status_fill)
        self.fix_verifier_buy.check_fix_message_kepler(er_fill_dma_qdl6_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Fill Child DMA 3 order')
        # endregion
