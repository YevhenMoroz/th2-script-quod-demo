...
class QAP_T10420(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_ioc_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit1, True, self.traded_qty, self.price_ask_qdl1, 1500)
        ...
        self.dma_1_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit1, OrderQty=self.qty, Price=self.price_ask_qdl1, TimeInForce=self.tif_ioc))
        ...
        er_partial_fill_dma_1_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_1_order, self.gateway_side_buy, self.status_partial_fill)
        self.fix_verifier_buy.check_fix_message_kepler(er_partial_fill_dma_1_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Partial fill Aggressive child DMA 1 order")
        # endregion
