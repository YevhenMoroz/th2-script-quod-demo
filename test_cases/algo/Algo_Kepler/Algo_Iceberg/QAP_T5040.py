...
class QAP_T5040(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_1_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_qdl12, self.price)
        ...
        self.Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Instrument=self.instrument, ExDestination=self.ex_destination_qdl12, Price=self.price, TimeInForce=self.tif_gtd, DisplayInstruction=dict(DisplayQty=self.display_qty))).add_tag(dict(ExpireDate=self.ExpireDate))
