...
class QAP_T5009(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account_qdl1, self.ex_destination_qdl1, self.price)
        # ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account_qdl1, self.ex_destination_qdl1, True)
        ...
        self.Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Instrument=self.instrument, Price=self.price, ExDestination=self.ex_destination_qdl1, TimeInForce=self.tif_gtd, DisplayInstruction=dict(DisplayQty=self.display_qty))).add_tag(dict(ExpireDate=self.ExpireDate_for_sending))
        ...
        self.dma_order.change_parameters(dict(Account=self.account_qdl1, ExDestination=self.ex_destination_qdl1, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument))
