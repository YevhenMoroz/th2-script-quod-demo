...
class QAP_T5061(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price = 20
        self.display_qty = 50
        self.inc_display_qty = 70
        self.qty_for_low_liquidity = self.inc_display_qty - self.display_qty
        ...
        self.key_params_ER_replace_parent = self.data_set.get_verifier_key_parameters_by_name("verifier_key_parameters_er_replace_display_qty_parent")
    ...
    def run_pre_conditions_and_steps(self):
        ...
        case_id_2 = bca.create_event("1st Replace LitDark Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_2)

        self.Iceberg_order_replace_1_params = FixMessageOrderCancelReplaceRequestAlgo(self.Iceberg_order)
        self.Iceberg_order_replace_1_params.change_parameters(dict(DisplayInstruction=dict(DisplayQty=self.inc_display_qty)))
        self.fix_manager_sell.send_message_and_receive_response(self.Iceberg_order_replace_1_params, case_id_2)

        er_replaced_1_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_order_cancel_replace(self.Iceberg_order_replace_1_params, self.gateway_side_sell, self.status_cancel_replace)
        self.fix_verifier_sell.check_fix_message(er_replaced_1_Iceberg_order_params, key_parameters=self.key_params_ER_replace_parent, message_name='Sell Side ExecReport Replace Request')
        # endregion

        time.sleep(3)

        # region Check 2nd child DMA order for low liquidity
        self.fix_verifier_buy.set_case_id(bca.create_event("New child DMA order after the 1st replace for low liquidity parameter", self.test_id))

        self.dma_2_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_child_of_Iceberg_Kepler_params()
        self.dma_2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_qdl6, OrderQty=self.qty_for_low_liquidity, Price=self.price, Instrument=self.instrument))
        self.fix_verifier_buy.check_fix_message_kepler(self.dma_2_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle Child DMA 1 order')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_pending_new_dma_2_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Child DMA 1 order')
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_new_dma_2_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 1 order')
        # endregion

        time.sleep(2)

        # region 2nd Modify parent LitDark order
        case_id_3 = bca.create_event("2nd Replace LitDark Order", self.test_id)
        self.fix_verifier_sell.set_case_id(case_id_3)

        self.Iceberg_order_replace_2_params = FixMessageOrderCancelReplaceRequestAlgo(self.Iceberg_order)
        self.Iceberg_order_replace_2_params.change_parameters(dict(DisplayInstruction=dict(DisplayQty=self.dec_display_qty)))
        self.fix_manager_sell.send_message_and_receive_response(self.Iceberg_order_replace_2_params, case_id_3)

        er_replaced_2_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_order_cancel_replace(self.Iceberg_order_replace_2_params, self.gateway_side_sell, self.status_cancel_replace)
        self.fix_verifier_sell.check_fix_message(er_replaced_2_Iceberg_order_params, key_parameters=self.key_params_ER_replace_parent, message_name='Sell Side ExecReport Replace Request')
        # endregion

        time.sleep(3)

        # region check cancel 2nd child DMA order
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_2_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child Iceberg order")
        # endregion

        # region Check replace 1st child DMA order
        ...
        self.dma_1_order_replace_params.change_parameters(dict(OrderQty=self.dec_display_qty)).add_tag(dict(ChildOrderID='*', misc5='*'))
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_replace_dma_1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 1 order')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_dma_1_order.change_parameters(dict(OrderQty=self.dec_display_qty))
        ...
        er_cancel_Iceberg_order_params = FixMessageExecutionReportAlgo().set_params_from_order_cancel_replace(self.Iceberg_order_replace_2_params, self.gateway_side_sell, self.status_cancel)
