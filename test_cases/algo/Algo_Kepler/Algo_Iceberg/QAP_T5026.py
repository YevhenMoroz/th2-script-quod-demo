...
class QAP_T5026(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # compare_message_1.change_parameters(dict(Parameter1=self.param1, Value1=self.misc_value, Parameter2=self.param2, Value2=self.ClOrdId))
        #
        # compare_message_2 = ReadLogMessageAlgo().set_compare_message_for_check_mapping()
        # compare_message_2.change_parameters(dict(Parameter1=self.param1, Value1=self.misc_value))
        #
        # self.read_log_verifier_1.set_case_id(bca.create_event("ReadLog: Sell-side", self.test_id))
        ...
        self.dma_xpar_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_xpar, OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument)).add_tag(dict(IClOrdIdTO="test tag 5003"))
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message_kepler(er_cancel_dma_xpar_order, self.key_params_ER_child, self.ToQuod, "Buy Side ExecReport Cancel child Iceberg order")
        # endregion
