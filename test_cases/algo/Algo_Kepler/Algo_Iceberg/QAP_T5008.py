...
class QAP_T5008(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # region venue param
        self.ex_destination_qdl12 = self.data_set.get_mic_by_name("mic_29")
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos__rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_qdl12, self.price)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_qdl12, True)
        self.rule_list = [nos__rule, ocr_rule]
        # endregion
        ...
        market_data_snap_shot_qdl11 = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.listing_id_qdl11, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl11.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=self.px_for_incr, MDEntrySize=self.qty_for_incr)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl11)
        ...
        market_data_snap_shot_qdl12 = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.listing_id_qdl12, self.fix_env1.feed_handler)
        market_data_snap_shot_qdl12.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=self.px_for_incr, MDEntrySize=self.qty_for_incr)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_qdl12)
        ...
        self.Iceberg_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Instrument=self.instrument, ExDestination=self.ex_destination_qdl12, Price=self.price, TimeInForce=self.tif_gtc, DisplayInstruction=dict(DisplayQty=self.display_qty)))
