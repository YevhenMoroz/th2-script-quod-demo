...
class QAP_4540(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit2, self.price)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_quodlit2, True)
        self.rule_list = [nos_fok_rule, nos_rule, ocr_rule]
        # endregion
        ...
        self.fix_verifier_sell.check_fix_message(er_pending_new_synthMinQty_order_params, key_parameters=self.key_params_with_cl_ord_id, message_name='Sell side ExecReport PendingNew')
        ...
        self.fix_verifier_sell.check_fix_message(er_new_synthMinQty_order_params, key_parameters=self.key_params_with_cl_ord_id, message_name='Sell side ExecReport New')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(self.dma_qdl1_order, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle Aggressive Child DMA 1 order')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_qdl1_order_params, key_parameters=self.key_params_with_ex_destination, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Aggressive Child DMA 1 order')

        er_new_dma_qdl1_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_qdl1_order, self.gateway_side_buy, self.status_pending)
        er_new_dma_qdl1_order_params.change_parameters(dict(ExDestination=self.ex_destination_quodlit1))
        self.fix_verifier_buy.check_fix_message(er_new_dma_qdl1_order_params, key_parameters=self.key_params_with_ex_destination, direction=self.ToQuod, message_name='Buy side ExecReport New Aggressive Child DMA 1 order')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(er_fill_dma_qdl1_order, self.key_params_with_ex_destination, self.ToQuod, "Buy Side ExecReport Fill Aggressive DMA 1 order")
        # endregion

        self.fix_verifier_sell.set_case_id(bca.create_event("Partiall fill Algo Order", self.test_id))
        ...
        self.fix_verifier_sell.check_fix_message(er_fill_synthMinQty_order, key_parameters=self.key_params_with_cl_ord_id, message_name='Sell side ExecReport Fill')
        # endregion
        ...
        self.dma_qdl2_order = FixMessageNewOrderSingleAlgo().set_DMA_ChildMinQty_params()
        self.dma_qdl2_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_quodlit1, OrderQty=self.leaves_qty, Price=self.price))
        self.fix_verifier_buy.check_fix_message(self.dma_qdl2_order, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle Passive Child DMA 2 order')
        ...
        er_pending_new_dma_qdl2_order_params.change_parameters(dict(ExDestination=self.ex_destination_quodlit1))
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma_qdl2_order_params, key_parameters=self.key_params_with_ex_destination, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Passive Child DMA 2 order')
        ...
        er_new_dma_qdl2_order_params.change_parameters(dict(ExDestination=self.ex_destination_quodlit1))
        self.fix_verifier_buy.check_fix_message(er_new_dma_qdl2_order_params, key_parameters=self.key_params_with_ex_destination, direction=self.ToQuod, message_name='Buy side ExecReport New Passive Child DMA 2 order')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message(er_cancel_dma_qdl2_order, self.key_params_with_ex_destination, self.ToQuod, "Buy Side ExecReport Cancel Passive child DMA 2 order")
        # endregion
        ...
        er_cancel_synthMinQty_order_params.remove_parameter('NoStrategyParameters').add_tag(dict(SettlDate='*')).add_tag(dict(SettlType='*')).add_tag(dict(NoParty='*')).change_parameters(dict(AvgPx=self.price, CxlQty=self.leaves_qty, CumQty=self.trade_qty))
        self.fix_verifier_sell.check_fix_message(er_cancel_synthMinQty_order_params, key_parameters=self.key_params_with_cl_ord_id, message_name='Sell side ExecReport Cancel')
        # endregion
