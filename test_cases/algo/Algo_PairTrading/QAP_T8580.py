...
class QAP_T8580(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.round_floor = 'floor'
        self.round_ceil = 'ceil'
        ...
        self.child1_qty_leg1 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq, self.qty, self.ratio_leg1, self.round_floor)
        self.child2_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq, self.qty, self.ratio_leg2, self.round_floor)
        self.child3_qty_leg1 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq, self.qty, self.ratio_leg1, self.round_floor)
        self.child4_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq, self.qty, self.ratio_leg2, self.round_floor)
        ...
        self.instrument_agta = self.data_set.get_fix_leg_instrument_by_name("instrument_agta")
        self.instrument_ast = self.data_set.get_fix_leg_instrument_by_name("instrument_ast")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ioc_dma_rule_1 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.child1_qty_leg1, self.price_bid_mod)
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.child2_qty_leg2, self.price_ask)
        ...
        market_data_snap_shot_leg_1.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_init, MDEntrySize=self.qty_bid)
        market_data_snap_shot_leg_1.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask, MDEntrySize=self.qty_ask)
        ...
        market_data_snap_shot_leg_2.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_init, MDEntrySize=self.qty_bid)
        market_data_snap_shot_leg_2.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask, MDEntrySize=self.qty_ask)
        ...
        self.PairTrad_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, OrdType=self.order_type, Instrument=self.instrument, Side=self.side_sell))
        self.PairTrad_order.update_repeating_group('NoLegs', [dict(InstrumentLeg=self.instrument_agta), dict(LegRatioQty=self.ratio_leg1, LegSide=self.side_buy),
                                                              dict(InstrumentLeg=self.instrument_ast), dict(LegRatioQty=self.ratio_leg2, LegSide=self.side_sell)])
        ...
        market_data_snap_shot_leg_2.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_mod, MDEntrySize=self.qty_bid)
        market_data_snap_shot_leg_2.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask, MDEntrySize=self.qty_ask)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_leg_2)
        
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 1 AGTA", self.test_id))
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid_mod, MDEntrySize=self.qty_ltq)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 2 AST_MI", self.test_id))
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask, MDEntrySize=self.qty_ltq)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child1_qty_leg1, Price=self.price_bid_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', Side=self.side_sell, LocateReqd='*'))
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child2_qty_leg2, Price=self.price_ask, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
                                                Account=self.account_trqx, ExDestination=self.ex_destination_trqx, LocateReqd='*'))
        ...
        # region Check child DMA order 3
        self.fix_verifier_buy.set_case_id(bca.create_event("Child DMA order 3 Leg 1", self.test_id))

        self.dma_order_3 = FixMessageNewOrderSingleAlgo().set_DMA_params()
        self.dma_order_3.change_parameters(dict(OrderQty=self.child3_qty_leg1, Price=self.price_bid_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', LocateReqd='*', Side=self.side_sell))
        self.fix_verifier_buy.check_fix_message(self.dma_order_3, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Child DMA 3 Leg 1')
        ...
        # region Check child DMA order 4
        self.fix_verifier_buy.set_case_id(bca.create_event("Child DMA order 4 Leg 2", self.test_id))

        self.dma_order_4 = FixMessageNewOrderSingleAlgo().set_DMA_params()
        self.dma_order_4.change_parameters(dict(OrderQty=self.child4_qty_leg2, Price=self.price_ask, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
                                                Account=self.account_trqx, ExDestination=self.ex_destination_trqx, LocateReqd='*'))

        self.fix_verifier_buy.check_fix_message(self.dma_order_4, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Child DMA 4 Leg 1')
        ...
        self.new_dma_order_4_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_4, self.gateway_side_buy, self.status_new)
        self.fill_dma_order_4_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_4, self.gateway_side_buy, self.status_fill)
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message_sequence([self.pending_dma_order_1_params, self.new_dma_order_1_params, self.fill_dma_order_1_params,
                                                          ...
                                                          self.pending_dma_order_3_params, self.new_dma_order_3_params, self.fill_dma_order_3_params,
                                                          self.pending_dma_order_4_params, self.new_dma_order_4_params, self.fill_dma_order_4_params],
                                                         [self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params,
                                                         self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], self.ToQuod, "Check sequence ER for child orders", None, False)
