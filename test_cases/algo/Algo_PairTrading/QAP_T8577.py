...
class QAP_T8577(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.child1_qty_leg1 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq_1, self.qty, self.ratio_leg1, self.round_floor)
        self.child2_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq_2, self.qty, self.ratio_leg2, self.round_floor)
        self.child3_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq_1, self.qty, self.ratio_leg2, self.round_ceil)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask, MDEntrySize=self.qty_ltq_1)
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid_mod, MDEntrySize=self.qty_ltq_2)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        ioc_dma_rule_1 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.child3_qty_leg2, self.price_bid_mod)

        self.rule_list = [ioc_dma_rule_1]
        # endregion

        # region Send_MarkerData
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 1 DP", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.leg_1, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid_mod, MDEntrySize=self.qty_ltq_1)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        ...
        self.dma_order_3.change_parameters(dict(OrderQty=self.child3_qty_leg2, Price=self.price_bid_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', LocateReqd='*', Side=self.side_sell))
        ...
        self.fix_verifier_buy.check_fix_message_sequence([self.pending_dma_order_1_params, self.new_dma_order_1_params, self.fill_dma_order_1_params,
                                                         ...
                                                         self.key_params, self.key_params, self.key_params], self.ToQuod, "Check sequence ER for child orders", None, False)
