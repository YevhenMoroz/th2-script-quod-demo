...
class QAP_T8108(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.child1_qty_leg1 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq1, self.qty, self.ratio_leg1, self.round_floor)
        self.child2_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq1, self.qty, self.ratio_leg2, self.round_floor)
        self.qty_ltq2 = self.child1_qty_leg1
        self.child3_qty_leg1 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq2, self.qty, self.ratio_leg1, self.round_floor)
        self.child4_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq2, self.qty, self.ratio_leg2, self.round_floor)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ioc_dma_rule_1 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.child1_qty_leg1, self.price_bid)
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.child2_qty_leg2, self.price_ask_mod)
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask_mod, MDEntrySize=self.qty_ltq1)
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid, MDEntrySize=self.qty_ltq1)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_1.change_parameters(dict(OrderQty=self.child1_qty_leg1, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
                                                LocateReqd='*', Side=self.side_sell))
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child2_qty_leg2, Price=self.price_ask_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
        ...
        ioc_dma_rule_1 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.child3_qty_leg1, self.price_bid)
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.child4_qty_leg2, self.price_ask_mod)
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 1 AGTA", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.leg_1, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask_init, MDEntrySize=self.qty_ltq2)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Child DMA order 3 Leg 2", self.test_id))

        self.dma_order_3 = FixMessageNewOrderSingleAlgo().set_DMA_params()
        self.dma_order_3.change_parameters(dict(OrderQty=self.child4_qty_leg2, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', LocateReqd='*', Side=self.side_sell))
        ...
        self.dma_order_4.change_parameters(dict(OrderQty=self.child4_qty_leg2, Price=self.price_ask_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
                                                Account=self.account_trqx, ExDestination=self.ex_destination_trqx, LocateReqd='*'))
        self.fix_verifier_buy.check_fix_message(self.dma_order_4, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Child DMA 4 Leg 2')
