...
class QAP_T8581(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.dma_order_3.change_parameters(dict(OrderQty=self.child_qty, Price=self.price_ask_mod, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*'))
        self.fix_verifier_buy.check_fix_message(self.dma_order_3, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Child DMA Leg 1')
        ...
        self.dma_order_4.change_parameters(dict(OrderQty=self.child_qty, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', Account=self.account_trqx, ExDestination=self.ex_destination_trqx,
                                                Side=self.side_sell, LocateReqd='*'))

        self.fix_verifier_buy.check_fix_message(self.dma_order_4, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Child DMA Leg 2')
        ...
        self.fix_verifier_buy.check_fix_message_sequence([self.pending_dma_order_1_params, self.new_dma_order_1_params, self.fill_dma_order_1_params,
                                                         ...
                                                         self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], self.ToQuod, "Check sequence ER for child orders", None, True)
