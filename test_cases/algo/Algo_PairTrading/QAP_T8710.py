...
class QAP_T8710(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.round = 'floor'
        self.ratio_leg1 = self.ratio_leg2 = 1
        ...
        # region instrument
        self.instrument_pt = self.data_set.get_fix_instrument_by_name("instrument_pt_round")
        self.instrument_leg1 = self.data_set.get_fix_leg_instrument_by_name("instrument_org")
        self.instrument_leg2 = self.data_set.get_fix_leg_instrument_by_name("instrument_pvl")
        # endregion
        ...
        self.leg_1 = self.data_set.get_listing_id_by_name("listing_org")
        self.leg_2 = self.data_set.get_listing_id_by_name("listing_pvl")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.child_qty, self.price_bid)

        self.rule_list = [ioc_dma_rule_2]
        # endregion
        
        # region Send_MarkerData
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data Leg 1 ORG", self.test_id))
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data Leg 2 PVL", self.test_id))
        ...
        self.PairTrad_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, OrdType=self.order_type, Instrument=self.instrument_pt))
        ...
        self.PairTrad_order.update_repeating_group('NoLegs', [dict(InstrumentLeg=self.instrument_leg1), dict(LegRatioQty=self.ratio_leg1, LegSide=self.side_buy),
                                                              dict(InstrumentLeg=self.instrument_leg2), dict(LegRatioQty=self.ratio_leg2, LegSide=self.side_sell)])
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 1 ORG", self.test_id))
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 2 PVL", self.test_id))
        ...
        # region Check child DMA order leg 2
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child_qty, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', Side=self.side_sell, LocateReqd='*'))
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check sequence only listing without roundlot is targeted", self.test_id))
        self.fix_verifier_buy.check_fix_message_sequence([self.pending_dma_order_2_params, self.new_dma_order_2_params, self.fill_dma_order_2_params],
                                                         [self.key_params, self.key_params, self.key_params], self.ToQuod, "Check sequence ER for child orders", None, True)
