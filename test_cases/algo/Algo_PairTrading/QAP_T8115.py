...
class QAP_T8115(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.child2_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq2, self.qty, self.ratio_leg1, self.round_floor)
        self.qty_ltq3 = self.child2_qty_leg2
        self.child3_qty_leg2 = AlgoFormulasManager.get_pov_child_qty_on_ltq(self.pct, self.qty_ltq3, self.qty, self.ratio_leg1, self.round_floor)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.child2_qty_leg2, self.price_bid)
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_bid, MDEntrySize=self.qty_ltq2)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child2_qty_leg2, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
        ...
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.child3_qty_leg2, self.price_bid)

        self.rule_list = [ioc_dma_rule_2]
        # endregion
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 1 AGTA", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.leg_1, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask_init, MDEntrySize=self.qty_ltq1)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)

        self.fix_manager_feed_handler.set_case_id(bca.create_event("Set TradingPhase for Leg 2 AST_MI", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.leg_2, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', MDEntryPx=self.price_ask_init, MDEntrySize=self.qty_ltq3)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        self.dma_order_3.change_parameters(dict(OrderQty=self.child3_qty_leg2, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*',
                                                Account=self.account_trqx, ExDestination=self.ex_destination_trqx, LocateReqd='*', Side=self.side_sell))
