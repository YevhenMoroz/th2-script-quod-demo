...
class QAP_T8819(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ioc_dma_rule = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_par, self.ex_destination_par, True, self.qty_ask, self.price_ask_mod, self.delay)
        ioc_dma_rule_2 = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, True, self.qty_bid, self.price_bid)

        self.rule_list = [ioc_dma_rule, ioc_dma_rule_2]
        # endregion
        ...
        self.dma_order_2.change_parameters(dict(OrderQty=self.child_qty, Price=self.price_bid, Instrument='*', TimeInForce=self.tif_ioc, PositionEffect='*', Account=self.account_trqx,
                                                ExDestination=self.ex_destination_trqx, Side=self.side_sell, LocateReqd='*'))
        ...
        self.fill_dma_order_2_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_2, self.gateway_side_buy, self.status_partial_fill)
        self.fix_verifier_buy.check_fix_message(self.fill_dma_order_2_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PartFill Child DMA 2')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        # region Check DMA order 1 Leg 1
        self.fix_verifier_buy.set_case_id(bca.create_event("Delayed reply for Child DMA order Leg 1", self.test_id))
        ...
        self.fix_verifier_buy.check_fix_message(self.pending_dma_order_1_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 1')

        self.fill_dma_order_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order_1, self.gateway_side_buy, self.status_partial_fill)
        self.fix_verifier_buy.check_fix_message(self.pending_dma_order_1_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PartFill Child DMA 1')
        # endregion
