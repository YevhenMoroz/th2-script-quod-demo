...
class QAP_T4338(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_ioc_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.triggerPrice)
    ...
    def run_post_conditions(self):
        ...
        reject_vwap_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.vwap_order, self.gateway_side_sell, self.status_reject)
        reject_vwap_order_params.add_tag(dict(Text=self.free_notes))
        self.fix_verifier_sell.check_fix_message(reject_vwap_order_params, key_parameters=self.key_params_cl, direction=self.FromQuod, message_name='Buy side ExecReport Fill VWAP child')
        # endregion
