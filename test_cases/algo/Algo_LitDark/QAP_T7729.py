...
class QAP_T7729(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.litdark_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, Side=self.side))
        ...
        dma_1_order.change_parameters(dict(Price=self.dma_ord_px, Instrument=self.instrument, OrderQty=self.qty, Account=self.account, TimeInForce=self.tif_ioc, ExDestination=self.ex_destination_dark, Side=self.side))
