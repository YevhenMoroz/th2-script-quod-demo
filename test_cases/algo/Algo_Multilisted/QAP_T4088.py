...
class QAP_T4088(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.rule_list = [nos_fok_rule, ocr_rule]
        # endregion
        ...
        self.multilisting_order.change_parameters(dict(Price=self.price, Account=self.client, OrderQty=self.qty, OrdType=self.order_type, Instrument=self.instrument, TimeInForce=self.tif_fok))
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_fok))
    ...
    def run_post_conditions(self):
        ...
        eliminate_dma_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order, self.gateway_side_buy, self.status_eliminate)
        eliminate_dma_order.change_parameters(dict(OrdType=self.order_type, TimeInForce=self.tif_fok))
        self.fix_verifier_buy.check_fix_message(eliminate_dma_order, self.key_params, self.ToQuod, "Buy Side ExecReport Cancel child DMA order")
        # endregion
