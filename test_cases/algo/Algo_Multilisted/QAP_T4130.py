...
class QAP_T4130(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument))
        ...
        replaced_multilisting_order_params.change_parameters(dict(ExpireDate=self.ExpireDate)).add_tag(dict(SettlType='*', SettlDate='*')).remove_parameter('NoParty')
        ...
        replaced_dma_order_params.change_parameters(dict(TimeInForce=self.tif_gtd, ExpireDate=self.ExpireDate))
    ...
    def run_post_conditions(self):
        ...
        cancel_multilisting_order_params.change_parameters(dict(ExpireDate=self.ExpireDate, TimeInForce=self.tif_gtd)).add_tag(dict(SettlType='*', SettlDate='*'))
