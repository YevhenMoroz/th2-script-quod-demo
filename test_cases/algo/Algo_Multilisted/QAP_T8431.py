...
class QAP_T8431(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_fok_rule = rule_manager.add_NewOrdSingle_FOK(self.fix_env1.buy_side, self.account_xpar, self.ex_destination_xpar, False, self.price_ask_paris)
        ...
        er_eliminate_dma_2_xpar_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(dma_2_xpar_order, self.gateway_side_buy, self.status_eliminate)
        # endregion

        # region 3rd child DMA order
        dma_3_xpar_order = FixMessageNewOrderSingleAlgo().set_DMA_params()
        dma_3_xpar_order.change_parameters(dict(Account=self.account_xpar, ExDestination=self.ex_destination_xpar, OrderQty=self.qty, Price=self.price_ask_paris, Instrument=self.instrument, TimeInForce=self.tif_fok))
        er_eliminate_dma_3_xpar_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(dma_3_xpar_order, self.gateway_side_buy, self.status_eliminate)
        # endregion

        time.sleep(3)

        # region Check childs
        self.fix_verifier_buy.set_case_id(bca.create_event("Check NOS on the 3 child orders", self.test_id))
        self.fix_verifier_buy.check_fix_message_sequence([dma_1_xpar_order, dma_2_xpar_order, dma_3_xpar_order], key_parameters_list=[None, None, None], direction=self.FromQuod, pre_filter=self.pre_filter_1)

        self.fix_verifier_buy.set_case_id(bca.create_event("Check that 3 child orders are eliminated", self.test_id))
        self.fix_verifier_buy.check_fix_message_sequence([er_eliminate_dma_1_xpar_order, er_eliminate_dma_2_xpar_order, er_eliminate_dma_3_xpar_order], key_parameters_list=[None, None, None], direction=self.ToQuod, pre_filter=self.pre_filter_2)
        # endregion
