...
class QAP_T4107(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.multilisting_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt, Instrument=self.instrument, StopPx=self.stop_price))
        ...
        self.dma_1_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument))
        ...
        # region First modify parent multilisting order
        case_id_1 = bca.create_event("First replace Multilisting Order", self.test_id)
        ...
        replaced_multilisting_order_params_dec.add_tag(dict(SettlType='*', SettlDate='*', StopPx=self.dec_stop_price)).change_parameters(dict(OrdType=self.order_type_stop_lmt, Price=self.dec_price)).remove_parameter('NoParty')
        ...
        # region Check child DMA 2 order
        ...
        # region Second modify parent multilisting order
        case_id_2 = bca.create_event("Second replace Multilisting Order", self.test_id)
        ...
        replaced_multilisting_order_params_inc.add_tag(dict(SettlType='*', SettlDate='*', StopPx=self.inc_stop_price)).change_parameters(dict(OrdType=self.order_type_stop_lmt, Price=self.inc_price)).remove_parameter('NoParty')
        ...
        # region Check child DMA 3 order
    ...
    def run_post_conditions(self):
        ...
        cancel_multilisting_order_params.add_tag(dict(SettlDate='*', SettlType='*', Price=self.inc_price, StopPx=self.inc_stop_price))
