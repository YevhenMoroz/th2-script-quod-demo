...
class QAP_T4123(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.multilisting_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, OrdType=self.order_type_stop_lmt, TimeInForce=self.tif_gtd, ExpireDate=self.ExpireDate, Instrument=self.instrument, StopPx=self.stop_price))
        ...
        self.dma_1_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_gtd, ExpireDate=self.ExpireDate,  Instrument=self.instrument))
    ...
    def run_post_conditions(self):
        ...
        cancel_multilisting_order_params.change_parameters(dict(TimeInForce=self.tif_gtd)).add_tag(dict(ExpireDate=self.ExpireDate))
