...
class QAP_T4058(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule_mod = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.mod_price)
        ...
        self.rule_list = [nos_rule, ocr_rule, ocrr_rule, nos_rule_mod]
        # endregion
        ...
        self.multilisting_order.add_fields_into_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='PostMode', StrategyParameterType=14, StrategyParameterValue=self.post_mode)])
        ...
        dma_1_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument))
        ...
        # region check OCRR first dma child order
        replace_dma_1_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(dma_1_order, self.gateway_side_buy, self.status_cancel_replace)
        replace_dma_1_order_params.change_parameter('OrderQty', self.mod_qty)
        self.fix_verifier_buy.check_fix_message(new_dma_1_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New Child DMA 1 order')
        # endregion
        ...
        cancel_dma_1_order.change_parameter('OrderQty', self.mod_qty)
        ...
        self.dma_2_order.change_parameters(dict(OrderQty=self.mod_qty, Price=self.mod_price, Instrument=self.instrument))
