...
class QAP_1992(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.multilisting_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, OrdType=self.order_type, Instrument=self.instrument, TimeInForce=self.tif_gtd, ExpireDate=self.ExpireDate))
        ...
        fill_multilisted_order.change_parameters(dict(CumQty=self.qty, LeavesQty=0, LastQty=self.qty, Instrument="*")).remove_parameter('SecAltIDGrp').remove_parameter('SecondaryClOrdID').add_tag(dict(ExpireDate=self.ExpireDate))
