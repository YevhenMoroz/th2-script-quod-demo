...
class QAP_T4090(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.delay = 1
        self.expiry_date = (datetime.date.today()+datetime.timedelta(days=10)).strftime("%Y%m%d")
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.multilisting_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, OrdType=self.order_type, Instrument=self.instrument, TimeInForce=self.tif_gtd, ExpireDate=self.expiry_date))
        ...
        self.multilisting_order.change_parameter('TransactTime', '*')
        ...
        pending_multilisting_order_params.add_tag(dict(ExpireDate=self.expiry_date))
        ...
        new_multilisting_order_params.add_tag(dict(ExpireDate=self.expiry_date))
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd))
        self.dma_order.add_tag(dict(ExpireDate=self.expiry_date))
        ...
        pending_dma_order_params.add_tag(dict(ExpireDate=self.expiry_date))
        ...
        new_dma_order_params.add_tag(dict(ExpireDate=self.expiry_date))
    ...
    def run_post_conditions(self):
        ...
        cancel_dma_order.change_parameter('ExDestination', self.ex_destination_1)
        ...
        self.multilisting_order_replace_params.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd))
        ...
        cancel_multilisting_order_params.change_parameters(dict(TimeInForce=6, ExpireDate=self.expiry_date, SettlDate='*', Price=self.price)).remove_parameter('SettlType')
