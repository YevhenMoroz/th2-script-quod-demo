...
class QAP_T4131(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.trade_qty = 500
        self.traded_qty = self.qty - self.trade_qty
        self.delay = 1000
        self.price_ask = 40
        self.price_bid = 30
        ...
        self.status_partiall_fill = Status.PartialFill
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nost_rule = rule_manager.add_NewOrdSingleExecutionReportTrade(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.price, self.trade_qty, self.delay)
        ...
        self.rule_list = [nost_rule, nos_rule, ocr_rule]
        # endregion
        ...
        self.dma_1_order.change_parameters(dict(OrderQty=self.display_qty, Price=self.price, Instrument=self.instrument))
        ...
        # region Check Partially Fill
        case_id_2 = bca.create_event("Partially Fill child Order", self.test_id)
        self.fix_verifier_buy.set_case_id(case_id_2)
        # Partially Fill Order
        partially_fill_dma_1_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_1_order, self.gateway_side_buy, self.status_partiall_fill)
        partially_fill_dma_1_order.change_parameters(dict(CumQty=self.trade_qty, LeavesQty=self.trade_qty, LastQty=self.trade_qty))
        self.fix_verifier_buy.check_fix_message(partially_fill_dma_1_order, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport Fill')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        cancel_multilisting_order_params.change_parameters(dict(AvgPx=self.price, CxlQty=self.traded_qty, CumQty=self.trade_qty))
