...
class QAP_T4896(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.peg_px_type = constants.PegPriceType.LastPeg.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_dma_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.ltp_1)
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data LTP 1", self.test_id))
        ...
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=self.ltp_1, MDEntrySize=self.qty)
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.ltp_1, Instrument='*'))
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data LTP 2", self.test_id))
        market_data_incr_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_incr_par.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=self.ltp_2, MDEntrySize=self.qty)
        self.fix_manager_feed_handler.send_message(market_data_incr_par)
        # endregion
        ...
        er_replaced_dma_order.change_parameters(dict(Price=self.ltp_2))
