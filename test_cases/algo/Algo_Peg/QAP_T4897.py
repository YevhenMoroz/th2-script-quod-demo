...
class QAP_T4897(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price_ask = 11
        self.price_bid_1 = 10
        self.price_bid_2 = 10.7
        self.price_bid_3 = 10.5
        self.price = 10.6
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_dma_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.price_bid_1)
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price_bid_1, Instrument='*'))
        ...
        er_replaced_dma_order.change_parameters(dict(Price=self.price))
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data", self.test_id))
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_3, MDEntrySize=self.qty_bid)
        market_data_snap_shot_par.add_fields_into_repeating_group('NoMDEntries', [dict(MDEntryType=0, MDEntryPx=self.price, MDEntrySize=self.qty, MDEntryPositionNo=2)])
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask, MDEntrySize=self.qty_ask)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par)
        # endregion
        ...
        # region Check that child has new Price
        er_replaced_dma_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order, self.gateway_side_buy, self.status_cancel_replace)
        er_replaced_dma_order.change_parameters(dict(Price=self.price_bid_3))
        self.fix_verifier_buy.check_fix_message(er_replaced_dma_order, key_parameters=self.key_params_cl, direction=self.ToQuod, message_name='Buy side ExecReport Replaced DMA child')
        # endregion
