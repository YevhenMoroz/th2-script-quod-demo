...
class QAP_T4917(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.iceberg_order_replace_params.change_parameter('Price', self.dec_price).add_tag(dict(DisplayInstruction=dict(DisplayQty=self.display_qty)))
        ...
        replaced_iceberg_order_params.change_parameters(dict(NoParty='*', Price=self.dec_price))
        ...
        # region check modifying of dma child order
        replace_dma_1_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_1_order, self.gateway_side_buy, self.status_cancel_replace)
        replace_dma_1_order.change_parameter('Price', self.dec_price)
        self.fix_verifier_buy.check_fix_message(replace_dma_1_order, self.key_params, self.ToQuod, "Buy Side ExecReport Replaced DMA order")
        # endregion
    ...
    def run_post_conditions(self):
        ...
        cancel_iceberg_order_params.change_parameters(dict(Price=self.dec_price, NoParty='*', SettlType='*'))
