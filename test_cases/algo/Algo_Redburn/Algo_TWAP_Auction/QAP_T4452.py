...
class QAP_T4452(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.twap_auction_algo = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_TWAP_auction_params()
        self.twap_auction_algo.add_ClordId((os.path.basename(__file__)[:-3]))
        self.twap_auction_algo.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, ExDestination=self.mic))
        self.twap_auction_algo.add_tag(dict(QuodFlatParameters=dict(ParticipateInClosingAuctions="Y", MaxParticipationClose=self.percentage, Waves=self.waves)))
        self.fix_manager_sell.send_message_and_receive_response(fix_message=self.twap_auction_algo, case_id=case_id_1)
        # endregion

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(fix_message=self.twap_auction_algo, key_parameters=self.key_params_NOS_parent, direction=self.ToQuod, message_name='Sell side NewOrderSingle')

        er_pending_new = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.twap_auction_algo, self.gateway_side_sell, self.status_pending)
        ...
        er_new = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.twap_auction_algo, self.gateway_side_sell, self.status_new)
        ...
        self.fix_verifier_sell.set_case_id(bca.create_event("Check that TWAP order is eliminated", self.test_id))
        er_cancel_auction_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.twap_auction_algo, self.gateway_side_sell, self.status_cancel)
