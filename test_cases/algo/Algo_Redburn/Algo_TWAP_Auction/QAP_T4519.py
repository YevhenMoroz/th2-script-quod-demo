...
class QAP_T4519(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.percentage = 100
        self.save_for_close = 90
        self.auction_child_qty = int(self.qty * self.save_for_close / 100)
        self.twap_qty = int((self.qty - self.auction_child_qty) / self.waves)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        trade = rule_manager.add_NewOrdSingleExecutionReportTradeOnFullQty(self.fix_env1.buy_side, self.account, self.mic, self.price)
        ...
        self.rule_list = [nos_rule, ocr_rule, ocrr_rule, trade, cancel_rule]
        # endregion
        ...
        self.auction_algo.add_tag(dict(QuodFlatParameters=dict(ParticipateInClosingAuctions="Y", MaxParticipationClose=self.percentage, Waves=self.waves, SaveForClosePercentage=self.save_for_close)))
        ...
        case_id_2 = bca.create_event("Check child orders", self.test_id)
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.auction_child_qty, Price=self.price, TimeInForce=self.tif_atc, Instrument=self.instrument))

        self.twap_order_child = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.twap_order_child.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.twap_qty, Price=self.price2, Instrument=self.instrument))
        ...
        scheduler.enterabs(self.verify_time, 2, self.fix_verifier_buy.check_fix_message_sequence,
                           kwargs=dict(
                               fix_messages_list=[self.twap_order_child, self.twap_order_child, self.twap_order_child, self.twap_order_child, self.twap_order_child, self.dma_order],
                               key_parameters_list=[self.key_params_with_ex_destination, self.key_params_with_ex_destination, self.key_params_with_ex_destination, self.key_params_with_ex_destination, self.key_params_with_ex_destination, self.key_params_with_ex_destination], pre_filter=self.pre_filter, message_name="Buy side check that there are 5 TWAP childs and 1 auction"))
    ...
    def run_post_conditions(self):
        ...
        case_id_2 = bca.create_event("Check that algo is eliminated (filled)", self.test_id)
        ...
        er_cancel_auction_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.auction_algo, self.gateway_side_sell, self.status_fill)
        er_cancel_auction_order.add_tag(dict(OrigClOrdID='*')).remove_parameters(['SecondaryClOrdID', 'OrigClOrdID'])
        self.fix_verifier_sell.check_fix_message(er_cancel_auction_order, key_parameters=self.key_params_ER_parent, message_name='Sell side ExecReport filled')