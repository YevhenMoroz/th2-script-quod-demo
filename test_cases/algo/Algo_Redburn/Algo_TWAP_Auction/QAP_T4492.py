...
class QAP_T4492(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price = 40
        self.max_part = 10
        self.max_part_open = 15
        self.max_part_close = 30
        self.scal_opt = False
        ...
        self.pop_auction_child_qty = AFM.get_child_qty_for_auction(self.indicative_volume, self.max_part_open, self.qty)
        self.pcl_auction_child_qty = AFM.get_child_qty_for_auction(self.indicative_volume, self.max_part_close, self.qty)

        # Because of the bi-lateral behaviour the 1st auc child changed qty to 291. And the new child with the qty=9 is created
        self.cancel_auc_qty_1 = 291
        self.cancel_auc_qty_2 = 9
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.twap_order.update_fields_in_component('QuodFlatParameters', dict(StartDate2=start_date, MaxParticipation=self.max_part, MaxParticipationOpen=self.max_part_open, MaxParticipationClose=self.max_part_close))
        ...
        self.dma_auction_child_order_pop.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_xpar, OrderQty=self.pop_auction_child_qty, Price=self.price, TimeInForce=self.tif_ato, Instrument=self.instrument))
        ...
        self.dma_auction_child_order_pcl.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_xpar, OrderQty=self.pcl_auction_child_qty, Price=self.price, TimeInForce=self.tif_atc, Instrument=self.instrument))
