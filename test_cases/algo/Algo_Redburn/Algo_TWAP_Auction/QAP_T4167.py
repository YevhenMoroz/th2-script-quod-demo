...
class QAP_T4167(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        market_data_incremental_ind_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_MDReqID(self.listing_id, self.fix_env1.feed_handler).update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).set_phase(TradingPhases.Open)
        self.fix_manager_feed_handler.send_message(market_data_incremental_ind_par)

        market_data_incremental_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.listing_id, self.fix_env1.feed_handler)
        ...
        self.twap_order.update_fields_in_component('QuodFlatParameters', dict(StartDate2=start_date, MaxParticipation=self.max_part, MaxParticipationClose=self.max_part_close))
        ...
        scheduler.enterabs(send_incremental_refresh_pcl_checkpoint, 2, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=twap_slice_1_order, key_parameters=self.key_params_NOS_child, message_name='Buy side NewOrderSingle DMA Child 1'))
        ...
        scheduler.enterabs(send_incremental_refresh_pcl_checkpoint, 3, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=pending_twap_slice_1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew DMA Child 1'))
        ...
        scheduler.enterabs(send_incremental_refresh_pcl_checkpoint, 4, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=new_twap_slice_1_order_params, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New  DMA Child 1'))
        # endregion
