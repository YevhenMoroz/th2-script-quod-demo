...
def execute(report_id):
    try:
        ...
        vwap_order = FixMessageNewOrderSingleAlgo().set_VWAP_params()
        vwap_order.add_ClordId((os.path.basename(__file__)[:-3]))
        vwap_order.change_parameters(dict(Account= client, OrderQty = qty))
        vwap_order.update_fields_in_component('QuodFlatParameters', dict(WouldPriceOffset=would_price_offset))

        fix_manager.send_message_and_receive_response(vwap_order, case_id_1)

        time.sleep(3)

        #region Check Sell side
        fix_verifier_ss.check_fix_message(vwap_order, direction=ToQuod, message_name='Sell side NewOrderSingle')

        pending_vwap_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(vwap_order, gateway_side_sell, status_pending)
        ...
        reject_vwap_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(vwap_order, gateway_side_sell, status_reject)
        reject_vwap_order_params.change_parameter('Text', text_reject_would_price_reference)
