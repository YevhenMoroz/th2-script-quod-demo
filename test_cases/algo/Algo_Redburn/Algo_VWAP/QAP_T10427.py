...
from test_framework.fix_wrappers.FixMessageMarketDataSnapshotFullRefresh import MDEntryType
...
class QAP_T10427(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        scheduler.enterabs(self.start_date.timestamp() + 65, 5, self.fix_verifier_buy.check_fix_message, kwargs=dict(
            ...
            key_parameters=self.key_params,
            direction=self.FromQuod,
            message_name="Buy side send Cancel request after neutral phase"))
        scheduler.enterabs(self.start_date.timestamp() + 65, 6, self.fix_verifier_buy.check_fix_message, kwargs=dict(
            fix_message=er_cancel_dma,
            key_parameters=self.key_params,
            direction=self.ToQuod,
