...
class QAP_T11114(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        end_time_minus_58_sec = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreClosed, start_time=False) - 58
        ...
        self.fix_manager_sell.set_case_id(case_id=bca.create_event("Send Cancel request on Auction Scaling algo order", self.test_id))
        self.fix_verifier_sell.set_case_id(case_id=bca.create_event("Check Cancel request on Auction Scaling algo order", self.test_id))
        cancel_request_auction_order = FixMessageOrderCancelRequest(self.auction_algo)
        scheduler.enterabs(end_time_minus_1_min, 1, self.fix_manager_sell.send_message_and_receive_response, kwargs=dict(fix_message=cancel_request_auction_order))
        scheduler.enterabs(end_time_minus_1_min, 2,  self.fix_verifier_sell.check_fix_message, kwargs=dict(fix_message=cancel_request_auction_order, direction=self.ToQuod, message_name='Sell side Cancel Request'))

        # region Check 35=D of child orders
        scheduler.enterabs(end_time_minus_58_sec, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order], key_parameters_list=[self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], direction=self.FromQuod, pre_filter=self.pre_fileter_35_D, check_order=self.check_order_sequence))
        # endregion
