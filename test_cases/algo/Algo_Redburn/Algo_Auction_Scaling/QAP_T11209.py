...
class QAP_T11209(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.qty = 10_000
        self.price = 130
        self.indicative_volume = 73_330
        self.indicative_volume_new = 90_000
        ...
        self.scaling_child_order_qty = '%^(9[0-9]|8148|916)$'  # fisrt number 111112, 23-37K and any 3 number
        self.scaling_child_order_qty_new = '%^(845|91)$'
    ...
    def run_pre_conditions_and_steps(self):
        ...
        initial_slice = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreClosed, start_time=False) - 40
        end_time_minus_1_min = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreClosed, start_time=False) - 65
        end_time_minus_30_sec = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreClosed, start_time=False) - 35
        ...
        scaling_dma_child_order_2.change_parameters(dict(Account=self.account, OrderQty=self.scaling_child_order_qty, Price=self.scaling_child_order_price, Instrument='*', TimeInForce=self.tif_atc, ExDestination=self.ex_destination_1))

        pending_scaling_dma_child_order_params_2 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(scaling_dma_child_order_2, self.gateway_side_buy, self.status_pending)

        new_scaling_dma_child_order_params_2 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(scaling_dma_child_order_2, self.gateway_side_buy, self.status_new)

        self.cancel_scaling_child_order_params_2 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(scaling_dma_child_order_2, self.gateway_side_buy, self.status_cancel)
        # endregion

        # region Check 35=D of child orders
        scheduler.enterabs(end_time_minus_1_min, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order, scaling_dma_child_order], key_parameters_list=[self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], direction=self.FromQuod, pre_filter=self.pre_fileter_35_D, check_order=self.check_order_sequence))
        # endregion

        # region Check Pending New and New execution report for child orders
        scheduler.enterabs(end_time_minus_1_min, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params, pending_scaling_dma_child_order_params], key_parameters_list=[self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], direction=self.ToQuod, pre_filter=self.pre_fileter_35_8_Pending_new, check_order=self.check_order_sequence))

        self.fix_verifier_buy.set_case_id(bca.create_event("Check 12 Scaling child orders Buy Side NewOrderSingle, Pending New and New", self.case_id_2))
        scheduler.enterabs(end_time_minus_1_min, 2, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params, new_scaling_dma_child_order_params], key_parameters_list=[self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], direction=self.ToQuod, pre_filter=self.pre_fileter_35_8_New, check_order=self.check_order_sequence))
        # endregion

        # region Send new Indicative volume 90000
        self.fix_manager_feed_handler.set_case_id(case_id=bca.create_event("Increase Indicative volume", self.test_id))
        ...
        scheduler.enterabs(end_time_minus_30_sec, 1, self.fix_manager_feed_handler.send_message, kwargs=dict(fix_message=self.incremental_refresh_1))
        # endregion

        # region Check cancellation of the Scaling child orders (10)
        scheduler.enterabs(end_time_minus_15_sec, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params], key_parameters_list=[self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], direction=self.ToQuod, pre_filter=self.pre_fileter_35_8_Eliminate, check_order=self.check_order_sequence))
        # endregion

        # region Check 35=D of child orders
        scheduler.enterabs(end_time_minus_15_sec, 2, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[scaling_dma_child_order_2, scaling_dma_child_order_2], key_parameters_list=[self.key_params, self.key_params], direction=self.FromQuod, pre_filter=self.pre_fileter_35_D, check_order=self.check_order_sequence))
        # endregion

        # region Check Pending New and New execution report for child orders
        scheduler.enterabs(end_time_minus_15_sec, 3, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[pending_scaling_dma_child_order_params_2, pending_scaling_dma_child_order_params_2], key_parameters_list=[self.key_params, self.key_params], direction=self.ToQuod, pre_filter=self.pre_fileter_35_8_Pending_new, check_order=self.check_order_sequence))

        scheduler.enterabs(end_time_minus_15_sec, 4, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[new_scaling_dma_child_order_params_2, new_scaling_dma_child_order_params_2], key_parameters_list=[self.key_params, self.key_params], direction=self.ToQuod, pre_filter=self.pre_fileter_35_8_New, check_order=self.check_order_sequence))
        # endregion
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message_sequence([self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params, self.cancel_scaling_child_order_params], [ self.key_params, self.key_params, self.key_params, self.key_params], self.ToQuod, pre_filter=self.pre_fileter_35_8_Eliminate, check_order=self.check_order_sequence)
        # endregion
