...
from test_framework.data_sets.constants import DirectionEnum, Status, GatewaySide, TradingPhases, TimeInForce, RBCustomTags, Reference, OrderType, OrderSide
...
class QAP_T4390(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.auction_algo.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, ExDestination=self.ex_destination_1, Side=self.sell_side, OrdType=self.order_type_market))
        ...
        scaling_dma_child_order.change_parameters(dict(Account=self.account, OrderQty=self.scaling_child_order_qty, Price=self.scaling_child_order_price, Instrument='*', TimeInForce=self.tif_atc, Side=self.sell_side))
        ...
        scaling_market_dma_child_order.change_parameters(dict(Account=self.account, OrderQty=self.scaling_child_order_qty, Price=self.scaling_child_order_price, Instrument='*', TimeInForce=self.tif_atc, OrdType=self.order_type_market, Side=self.sell_side))
