...
class QAP_T4459(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.pp2_price)
        ...
        self.auction_algo.update_fields_in_component('QuodFlatParameters', dict(MaxParticipation=self.percentage_volume, PricePoint1Price=self.pp1_price, PricePoint1Participation=self.pp1_percentage, PricePoint2Price=self.pp2_price))
        self.auction_algo.remove_fields_from_component('QuodFlatParameters', ['PricePoint2Participation'])
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check Scaling child order Buy side NewOrderSingle", self.case_id_2))

        # region Aggressive Scaling order
        scaling_dma_child_order = FixMessageNewOrderSingleAlgo().set_DMA_RB_params()
        scaling_dma_child_order.change_parameters(dict(Account=self.account, OrderQty=self.qty, Price=self.pp2_price, Instrument='*', TimeInForce=self.tif_ato))
        self.fix_verifier_buy.check_fix_message(scaling_dma_child_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle')
        ...
        self.fix_verifier_buy.check_fix_message(pending_scaling_dma_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew')
        ...
        self.fix_verifier_buy.check_fix_message(new_scaling_dma_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New')
    ...
    def run_post_conditions(self):
        ...
        self.fix_verifier_buy.check_fix_message(self.cancel_scaling_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport Cancel')
        # endregion
