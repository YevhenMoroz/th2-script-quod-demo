...
def execute(report_id):
    try:
        ...
        twap_order = FixMessageNewOrderSingleAlgo().set_TWAP_Navigator_params()
        twap_order.add_ClordId((os.path.basename(__file__)[:-3]))
        twap_order.change_parameters(dict(Account= client, OrderQty = qty))
        twap_order.update_fields_in_component('QuodFlatParameters', dict(NavigatorLimitPriceOffset=navigator_limit_price_offset))
