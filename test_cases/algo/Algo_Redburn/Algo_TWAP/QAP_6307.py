...
def rules_creation():
    ...
    nos_rule1 = rule_manager.add_NewOrdSingle_Market(connectivity_buy_side, account, ex_destination_1, False, 0, 0)
...
def execute(report_id):
    try:
        ...
        twap_nav_order.update_fields_in_component('QuodFlatParameters', dict(Waves=waves))
        ...
        nav_child_1.change_parameters(dict(OrderQty=qty_nav, OrdType=1))
        nav_child_1.remove_parameter('Price')
        fix_verifier_bs.check_fix_message(nav_child_1, key_parameters=key_params_mkt, message_name='Buy side NewOrderSingle Navigator')
        ...
        fix_verifier_bs.check_fix_message(pending_nav_child_1_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport PendingNew Navigator')
        ...
        fix_verifier_bs.check_fix_message(new_nav_child_1_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport New Navigator')
        # endregion
        ...
        cancel_nav_child_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(nav_child_1, gateway_side_buy, status_eliminate)
        fix_verifier_bs.check_fix_message(cancel_nav_child_1_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport Cancel Navigator')
