...
from test_framework.data_sets.constants import DirectionEnum, Status, GatewaySide, OrderType, TradingPhases
...
class QAP_T4304(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule1 = rule_manager.add_NewOrdSingle_Market(self.fix_env1.buy_side, self.account, self.ex_destination_1, False, 0, 0)
        ...
        twap_nav_order.update_fields_in_component('QuodFlatParameters', dict(Waves=self.waves, EndDate2=end_time))
        ...
        self.fix_verifier_buy.check_fix_message(pending_nav_child_1_params, key_parameters=self.key_params_mkt, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Navigator')
        ...
        self.fix_verifier_buy.check_fix_message(new_nav_child_1_params, key_parameters=self.key_params_mkt, direction=self.ToQuod, message_name='Buy side ExecReport New Navigator')
        # endregion
        ...
        self.fix_verifier_buy.set_case_id(case_id_3)
        cancel_nav_child_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(nav_child_1, self.gateway_side_buy, self.status_eliminate)
        self.fix_verifier_buy.check_fix_message(cancel_nav_child_1_params, key_parameters=self.key_params_mkt, direction=self.ToQuod, message_name='Buy side ExecReport Cancel Navigator')
