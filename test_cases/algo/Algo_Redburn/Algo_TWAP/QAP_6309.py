...
nav_limit_price_reference = DataSet.Reference.Limit.value
price = 29.995
price_nav = 30
...
key_params_mkt=['OrdStatus', 'ExecType', 'OrderQty']
...
def rules_creation():
    ...
    nos_rule = rule_manager.add_NewOrdSingle_Market(connectivity_buy_side, account, ex_destination_1, False, 0, 0)
...
def execute(report_id):
    try:
        ...
        twap_nav_order.update_fields_in_component('QuodFlatParameters', dict(NavigatorLimitPrice=price_nav, Waves=waves, NavigatorLimitPriceReference=nav_limit_price_reference))
        ...
        fix_verifier_ss.check_fix_message(new_twap_nav_order_params, key_parameters=key_params_cl, message_name='Sell side ExecReport New Parent')
        #endregion
        ...
        twap_child.change_parameters(dict(OrderQty=qty_twap_1, OrdType=1))
        twap_child.remove_parameter('Price')
        fix_verifier_bs.check_fix_message(twap_child, key_parameters=key_params_mkt, message_name='Buy side NewOrderSingle TWAP child')
        ...
        fix_verifier_bs.check_fix_message(pending_twap_child_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport PendingNew TWAP child')
        ...
        fix_verifier_bs.check_fix_message(new_twap_child_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport New TWAP child')

        # Check First Navigator child
        nav_child_1 = FixMessageNewOrderSingleAlgo().set_DMA_params()
        nav_child_1.change_parameters(dict(OrderQty=qty_nav, Price=price_nav))
        fix_verifier_bs.check_fix_message(nav_child_1, key_parameters=key_params, message_name='Buy side NewOrderSingle Navigator')
        ...
        fix_verifier_bs.check_fix_message(pending_nav_child_1_params, key_parameters=key_params, direction=ToQuod, message_name='Buy side ExecReport PendingNew Navigator')
        ...
        fix_verifier_bs.check_fix_message(new_nav_child_1_params, key_parameters=key_params, direction=ToQuod, message_name='Buy side ExecReport New Navigator')
        # endregion
        ...
        cancel_twap_child_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(twap_child, gateway_side_buy, status_eliminate)
        fix_verifier_bs.check_fix_message(cancel_twap_child_params, key_parameters=key_params_mkt, direction=ToQuod, message_name='Buy side ExecReport Cancel TWAP child')

        cancel_nav_child_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(nav_child_1, gateway_side_buy, status_cancel)
        fix_verifier_bs.check_fix_message(cancel_nav_child_1_params, key_parameters=key_params, direction=ToQuod, message_name='Buy side ExecReport Cancel Navigator')
