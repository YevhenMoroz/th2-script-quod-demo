...
class QAP_T10675(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        er_pending_new.change_parameters(dict(TimeInForce=self.time_in_force_ATO))
        ...
        er_new.change_parameters(dict(TimeInForce=self.time_in_force_ATO))
        ...
        end_time_preopen = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreOpen, start_time=False)
        self.verifier_time = end_time_preopen + 5

        auction_cancel_time_end = AFM.change_datetime_from_epoch_to_normal(end_time_preopen).astimezone(pytz.utc).isoformat()[:-6]
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, TimeInForce=self.time_in_force_ATO, Instrument=self.instrument))
        ...
        self.incremental_refresh_open = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative()\
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume_before) \
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntryPx', self.price81) \
            .update_MDReqID(self.listing_id, self.fix_env1.feed_handler)\
            .set_phase(TradingPhases.Open)
        scheduler.enterabs(end_time_preopen - 2, 1, self.fix_manager_feed_handler.set_case_id,kwargs=dict(case_id=bca.create_event("Send trading phase Open", self.test_id)))
        scheduler.enterabs(end_time_preopen - 2, 2, self.fix_manager_feed_handler.send_message, kwargs=dict(fix_message=self.incremental_refresh_open))
        ...
        er_cancel_auction_order.change_parameters(dict(TimeInForce=self.time_in_force_ATO, Text=self.reached_uncross, TransactTime="<" + auction_cancel_time_end))
