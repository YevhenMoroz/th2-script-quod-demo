...
class QAP_T4483(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', 0).update_MDReqID(self.listing_id, self.fix_env1.feed_handler).set_phase("1")
        ...
        start_time_pre_open = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreOpen, start_time=True) / 1000
        end_time_pre_open = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreOpen, start_time=False) / 1000
        ...
        scheduler.enterabs(end_time_open-1, 1, self.fix_manager_feed_handler.set_case_id, kwargs=dict(case_id=bca.create_event("Send trading phase - PreClose", self.test_id)))
        ...
        auction_child_time = AFM.change_datetime_from_epoch_to_normal(end_time_open).astimezone(pytz.utc).isoformat()[:-6]

        self.dma_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, TimeInForce=7, Instrument=self.instrument))
    ...
    def run_post_conditions(self):
        ...
        er_cancel_auction_order.change_parameters(dict(TimeInForce=7))
