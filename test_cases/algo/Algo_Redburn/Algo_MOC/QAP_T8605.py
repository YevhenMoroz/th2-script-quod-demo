...
class QAP_T8605(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.incremental_refresh_pre_close = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative()\
            ...
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntryPx', self.price41) \
        ...
        self.auction_algo.update_fields_in_component("QuodFlatParameters", dict(MaxParticipation=self.percentage, AtLast=1))
        ...
        scheduler.enterabs(end_time_preclosed + 5, 1, self.fix_manager_feed_handler.set_case_id,kwargs=dict(case_id=bca.create_event("Send trading phase AtLast", self.test_id)))
        scheduler.enterabs(end_time_preclosed + 5, 2, self.fix_manager_feed_handler.send_message, kwargs=dict(fix_message=self.incremental_refresh_atlast))
        ...
        case_id_2 = bca.create_event("Check AtLast child", self.test_id)
        ...
        self.dma_order2.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.qty, Price=self.price, Instrument=self.instrument))
        self.fix_verifier_buy.check_fix_message(fix_message=self.dma_order2, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_pending_new_dma2, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_new_dma2, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport New child order')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_auction_order.change_parameters(dict(TimeInForce=self.time_in_force_ATC))
