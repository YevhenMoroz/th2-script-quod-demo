...
class QAP_T9096(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.indicative_volume_15sec = 14
        self.indicative_volume_5sec = 2
        ...
        self.child_qty_2 = AFM.get_child_qty_for_auction(self.indicative_volume_30sec, self.percentage, self.qty)
        self.child_qty_3 = AFM.get_child_qty_for_auction(self.indicative_volume_15sec, self.percentage, self.qty)
        self.price = 80
        self.price81 = 81
        # endregion
    ...
    def run_pre_conditions_and_steps(self):
        ...
        order_replace_request_30sec = FixMessageOrderCancelReplaceRequestAlgo(self.dma_order).change_parameters(dict(OrderQty=self.child_qty_2, QtyType=0))
        scheduler.enterabs(self.verifier_time, 5, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=order_replace_request_30sec, key_parameters=self.key_params_ER_child, direction=self.FromQuod, message_name='Buy side 1st OrderCancelReplaceRequest child order'))

        order_replace_request_15sec = FixMessageOrderCancelReplaceRequestAlgo(self.dma_order).change_parameters(dict(OrderQty=self.child_qty_3, QtyType=0))
        scheduler.enterabs(self.verifier_time, 6, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=order_replace_request_15sec, key_parameters=self.key_params_ER_child, direction=self.FromQuod, message_name='Buy side 2nd OrderCancelReplaceRequest child order'))
        ...
        er_new_canceled.change_parameters(dict(OrderQty=2))
        ...
        scheduler.enterabs(end_time_preclosed - 6, 1, self.fix_manager_feed_handler.set_case_id,kwargs=dict(case_id=bca.create_event("Send trading phase PreClosed with volume before 5 sec checkpoint", self.test_id)))
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative()\
        ...
        scheduler.enterabs(end_time_preclosed - 3, 1, self.fix_manager_feed_handler.set_case_id,kwargs=dict(case_id=bca.create_event("Send trading phase PreClosed with volume before 2 sec checkpoint", self.test_id)))
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative()\
