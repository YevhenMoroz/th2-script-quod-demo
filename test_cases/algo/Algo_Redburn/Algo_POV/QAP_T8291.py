...
class QAP_T8291(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        # region PARIS BestBid setup
        self.qty_bid_par_1 = 10000
        self.price_bid_par_1 = 104

        self.qty_bid_par_2 = 10000
        self.price_bid_par_2 = 103.8

        self.qty_bid_par_3 = 9500
        self.price_bid_par_3 = 103.6
        # endregion

        # region TURQUOISE BestBid setup
        self.qty_bid_trqx_1 = 6000
        self.price_bid_trqx_1 = 103.6

        self.qty_bid_trqx_2 = 6500
        self.price_bid_trqx_2 = 103

        self.qty_bid_trqx_3 = 5000
        self.price_bid_trqx_3 = 102.9
        # endregion
        ...
        self.price_ltq_1 = 130
        self.qty_ltq_1 = 900_000

        self.round_floor = Rounding.Floor.value

        # self.passive_xpar_pov_qty = AFM.get_pov_child_qty(self.percentage_volume, self.qty_bid_trqx, self.qty)
        # self.passive_trqx_pov_qty = AFM.get_pov_child_qty(self.percentage_volume, self.qty_bid_trqx, self.qty - self.passive_xpar_pov_qty)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule_xpar_1 = rule_manager.add_NewOrdSingleExecutionReportAll(self.fix_env1.buy_side, self.account_xpar, self.ex_destination_xpar)
        nos_rule_trqx_1 = rule_manager.add_NewOrdSingleExecutionReportAll(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx)
        nos_ioc_rule_par = rule_manager.add_MarketNewOrdSingle_IOC(self.fix_env1.buy_side, self.account_xpar, self.ex_destination_xpar, True, 100000, 130)
        ...
        self.rule_list = [nos_rule_xpar_1, nos_rule_trqx_1, nos_ioc_rule_par, ocrr_rule, ocr_rule]
        # endregion
        ...
        market_data_snap_shot_par_clear.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_ltq_0, MDEntrySize=self.qty_ltq_0)
        market_data_snap_shot_par_clear.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ltq_0, MDEntrySize=self.qty_ltq_0)

        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Clear Market Data SnapShot on TURQUOISE", case_id_0))
        ...
        market_data_snap_shot_trqx_clear.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_ltq_0, MDEntrySize=self.qty_ltq_0)
        market_data_snap_shot_trqx_clear.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ltq_0, MDEntrySize=self.qty_ltq_0)
        # endregion
        ...
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_par_1, MDEntrySize=self.qty_bid_par_1)
        market_data_snap_shot_par.add_fields_into_repeating_group('NoMDEntries', [dict(MDEntryType=0, MDEntryPx=self.price_bid_par_2, MDEntrySize=self.qty_bid_par_2, MDEntryPositionNo=2)])
        market_data_snap_shot_par.add_fields_into_repeating_group('NoMDEntries', [dict(MDEntryType=0, MDEntryPx=self.price_bid_par_3, MDEntrySize=self.qty_bid_par_3, MDEntryPositionNo=3)])
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data SnapShot setup MarketDepth on TURQUOISE", case_id_0))
        ...
        market_data_snap_shot_trqx.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid_trqx_1, MDEntrySize=self.qty_bid_trqx_1)
        market_data_snap_shot_trqx.add_fields_into_repeating_group('NoMDEntries', [dict(MDEntryType=0, MDEntryPx=self.price_bid_trqx_2, MDEntrySize=self.qty_bid_trqx_2, MDEntryPositionNo=2)])
        market_data_snap_shot_trqx.add_fields_into_repeating_group('NoMDEntries', [dict(MDEntryType=0, MDEntryPx=self.price_bid_trqx_3, MDEntrySize=self.qty_bid_trqx_3, MDEntryPositionNo=3)])
        ...
        market_data_incremental_trqx = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.s_trqx, self.fix_env1.feed_handler).set_phase(TradingPhases.Open)
        market_data_incremental_trqx.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=0, MDEntrySize=0)
        # endregion

        time.sleep(2)
        # region Clear MD
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par_clear)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_trqx_clear)
        ...
        # self.passive_child_order_par_1 = FixMessageNewOrderSingleAlgo().set_DMA_RB_params()
        # self.passive_child_order_par_1.change_parameters(dict(Account=self.account_xpar, OrderQty=self.passive_xpar_pov_qty, Price=self.price_bid, Instrument='*', ExDestination=self.ex_destination_xpar))
        #
        ...
        #
        # new_passive_child_order_par_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.passive_child_order_par_1, self.gateway_side_buy, self.status_new)
        # # endregion
        #
        # # region Passive TRQX order
        # self.passive_child_order_trqx_1 = FixMessageNewOrderSingleAlgo().set_DMA_RB_params()
        # self.passive_child_order_trqx_1.change_parameters(dict(Account=self.account_trqx, OrderQty=self.passive_trqx_pov_qty, Price=self.price_bid, Instrument='*', ExDestination=self.ex_destination_trqx))
        #
        ...
        #
        # new_passive_child_order_trqx_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.passive_child_order_trqx_1, self.gateway_side_buy, self.status_new)
        # # endregion
        # # endregion
        #
    ...
    def run_post_conditions(self):
        ...
        #
        # cancel_dma_child_1_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.passive_child_order_par_1, self.gateway_side_buy, self.status_cancel)
        # cancel_dma_child_2_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.passive_child_order_trqx_1, self.gateway_side_buy, self.status_cancel)
        #
