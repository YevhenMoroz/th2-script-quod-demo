...
def execute(report_id):
    try:
        ...
        pov_order.change_parameters(dict(Account= client, OrderQty = qty))
        pov_order.update_fields_in_component('QuodFlatParameters', dict(WouldPriceOffset=would_price_offset))
        ...
        fix_verifier_ss.check_fix_message(pov_order, direction=ToQuod, message_name='Sell side NewOrderSingle')

        pending_pov_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(pov_order, gateway_side_sell, status_pending)
        ...
        reject_pov_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(pov_order, gateway_side_sell, status_reject)
        reject_pov_order_params.change_parameter('Text', text_reject_would_price_reference)
