...
qty = 100000
price = 20
pp1_particiaption = 120
text_reject_invalid_price_point_1_participation = DataSet.FreeNotesReject.InvalidPricePoint1Participation.value
...
def execute(report_id):
    try:
        ...
        pov_scaling_order.update_fields_in_component('QuodFlatParameters', dict(PricePoint1Participation=pp1_particiaption))

        fix_manager.send_message_and_receive_response(pov_scaling_order, case_id_1)

        time.sleep(3)

        #region Check Sell side
        fix_verifier_ss.check_fix_message(pov_scaling_order, direction=ToQuod, message_name='Sell side NewOrderSingle')

        pending_pov_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(pov_scaling_order, gateway_side_sell, status_pending)
        ...
        reject_pov_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(pov_scaling_order, gateway_side_sell, status_reject)
        reject_pov_order_params.change_parameter('Text', text_reject_invalid_price_point_1_participation)
