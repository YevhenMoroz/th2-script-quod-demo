...
class QAP_T4636(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price_child_passive = 30
        self.qty_child_aggressive = 12
        self.price_child_aggressive = 29.995
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.pov_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, ExDestination=self.ex_destination_1, Side=self.side))
        ...
        self.fix_verifier_buy.set_case_id(bca.create_event("Check  Anticipativechild order", self.test_id))

        self.anticipative_child_order = FixMessageNewOrderSingleAlgo().set_DMA_RB_params()
        self.anticipative_child_order.change_parameters(dict(Account=self.account, OrderQty=self.qty_child_passive, Price=self.price_child_passive, Instrument='*', ExDestination=self.ex_destination_1, Side=self.side))
        self.fix_verifier_buy.check_fix_message(self.anticipative_child_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle  AnticipativeChild')
        ...
        self.fix_verifier_buy.check_fix_message(pending_anticipative_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew   AnticipativeChild')

        new_new_anticipative_child_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.anticipative_child_order, self.gateway_side_buy, self.status_new)
        self.fix_verifier_buy.check_fix_message(new_new_anticipative_child_order_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New pov child')
        # endregion
        ...
        self.reactive_child_order.change_parameters(dict(Account=self.account, OrderQty=self.qty_child_aggressive, Price=self.price_child_aggressive, Instrument='*', ExDestination=self.ex_destination_1, Side=self.side))
