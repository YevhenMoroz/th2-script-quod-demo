...
class QAP_T8749(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        cancel_ioc_child_order_1 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(ioc_child_order_1, self.gateway_side_buy, self.status_eliminated)
        cancel_ioc_child_order_1.change_parameters(dict(Price=self.price, OrdType=self.order_type, TimeInForce=self.tif_ioc))
        ...
        cancel_ioc_child_order_2.change_parameters(dict(Price=self.price, OrdType=self.order_type, TimeInForce=self.tif_ioc))
        ...
        ioc_child_order_3.change_parameters(dict(OrderQty=self.qty_child_2, Price=self.price, Instrument='*', TimeInForce=self.tif_ioc))
        ...
        new_ioc_child_order_2_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(ioc_child_order_3, self.gateway_side_buy, self.status_pending)
        self.fix_verifier_buy.check_fix_message(new_ioc_child_order_2_params, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport New  IOC Child 2')

        cancel_ioc_child_order_3 = FixMessageExecutionReportAlgo().set_params_from_new_order_single(ioc_child_order_3, self.gateway_side_buy, self.status_eliminated)
        cancel_ioc_child_order_3.change_parameters(dict(Price=self.price, OrdType=self.order_type, TimeInForce=self.tif_ioc))
        self.fix_verifier_buy.check_fix_message(cancel_ioc_child_order_3, self.key_params, self.ToQuod, "Buy Side ExecReport Partial Fill IOC Child 2")
        # endregion
        ...
        #TODO
        # add check that algo generates only 3 child orders
