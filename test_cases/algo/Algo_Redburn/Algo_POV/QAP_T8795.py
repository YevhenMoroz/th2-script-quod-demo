...
class QAP_T8795(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price_ask_1 = 135
        self.qty_ask = 20_000        
        self.price_ask_2 = 138
        ...
        self.passive_pov_qty_1 = AFM.get_pov_child_qty(self.percentage_volume, self.qty_bid, self.qty)
        self.aggressive_pov_qty_1 = AFM.get_pov_child_qty_on_ltq(self.percentage_volume, self.qty_ltq_1, self.qty)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_ioc_rule_xpar = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_xpar, self.ex_destination_xpar, False, 0, self.price_ask_1)
        nos_ioc_rule_trqx = rule_manager.add_NewOrdSingle_IOC(self.fix_env1.buy_side, self.account_trqx, self.ex_destination_trqx, False, 0, self.price_ask_1)
        ocr_rule = rule_manager.add_OCR(self.fix_env1.buy_side)
        self.rule_list = [nos_rule_xpar_1, nos_rule_trqx_1, nos_ioc_rule_xpar, nos_ioc_rule_trqx, ocr_rule]
        # endregion
        ...
        passive_child_order_par_1.change_parameters(dict(Account=self.account_xpar, OrderQty=self.passive_pov_qty_1, Price=self.price_bid, Instrument='*', ExDestination=self.ex_destination_xpar))
        ...
        passive_child_order_trqx_1.change_parameters(dict(Account=self.account_trqx, OrderQty=self.passive_pov_qty_1, Price=self.price_bid, Instrument='*', ExDestination=self.ex_destination_trqx))
        ...
        ioc_child_order_par_1.change_parameters(dict(Account=self.account_xpar, OrderQty=self.aggressive_pov_qty_1, Price=self.price_ask_1, TimeInForce=self.tif_ioc, Instrument='*', ExDestination=self.ex_destination_xpar))
        ...
        ioc_child_order_trqx_1.change_parameters(dict(Account=self.account_trqx, OrderQty=self.aggressive_pov_qty_1, Price=self.price_ask_1, TimeInForce=self.tif_ioc, Instrument='*', ExDestination=self.ex_destination_trqx))
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data SnapShot setup MarketDepth on PARIS", case_id_2))
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=self.qty_bid)
        ...
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data Incremental setup LastTrade on PARIS", case_id_2))
        market_data_incremental_par = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq().update_MDReqID(self.s_par, self.fix_env1.feed_handler).set_phase(TradingPhases.Open)
        market_data_incremental_par.update_repeating_group_by_index('NoMDEntriesIR', 0, MDEntryPx=0, MDEntrySize=0)
        self.fix_manager_feed_handler.send_message(market_data_incremental_par)
        # endregion
        ...
        # region Send MarketData on TRQX
        self.fix_manager_feed_handler.set_case_id(bca.create_event("Send Market Data Incremental setup LastTrade on TURQUOISE", case_id_2))
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_trqx, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=self.qty_bid)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask_2, MDEntrySize=self.qty_ask)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par)
        ...
        # region Check that POV algo order doesn't generate new aggressive SOR orders
        case_id_2 = bca.create_event("Check that POV algo order doesn't generate new aggressive SOR orders", self.test_id)
        self.fix_verifier_buy.set_case_id(bca.create_event("Check 14 child orders Buy side NewOrderSingle Passive and Aggressive", case_id_2))
        self.fix_verifier_buy.check_fix_message_sequence([passive_child_order_par_1, passive_child_order_trqx_1, ioc_child_order_par_1, ioc_child_order_par_1, ioc_child_order_par_1, ioc_child_order_par_1, ioc_child_order_par_1, ioc_child_order_par_1, ioc_child_order_trqx_1, ioc_child_order_trqx_1, ioc_child_order_trqx_1, ioc_child_order_trqx_1, ioc_child_order_trqx_1, ioc_child_order_trqx_1], [self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params, self.key_params], self.FromQuod, pre_filter=self.data_set.get_pre_filter('pre_filer_equal_D'), check_order=self.check_order_sequence)
        ...
        # region Send new MarketData to trigger the new aggressive SOR child order creation
        self.fix_verifier_buy_2 = FixVerifier(self.fix_env1.buy_side, self.test_id)

        case_id_4 = bca.create_event("Send new MarketData to trigger the new aggressive SOR child order creation", self.test_id)

        self.fix_manager_feed_handler.set_case_id(case_id_4)
        market_data_snap_shot_par = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.s_par, self.fix_env1.feed_handler)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 0, MDEntryPx=self.price_bid, MDEntrySize=self.qty_bid)
        market_data_snap_shot_par.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask_1, MDEntrySize=self.qty_ask)
        self.fix_manager_feed_handler.send_message(market_data_snap_shot_par)
        ...
        market_data_snap_shot_par_trqx.update_repeating_group_by_index('NoMDEntries', 1, MDEntryPx=self.price_ask_1, MDEntrySize=self.qty_ask)
        ...
        ioc_child_order_par_2.change_parameters(dict(Account=self.account_xpar, OrderQty=self.aggressive_pov_qty_2, Price=self.price_ask_1, TimeInForce=self.tif_ioc, Instrument='*', ExDestination=self.ex_destination_xpar))
        ...
        ioc_child_order_trqx_2.change_parameters(dict(Account=self.account_trqx, OrderQty=self.aggressive_pov_qty_2, Price=self.price_ask_1, TimeInForce=self.tif_ioc, Instrument='*', ExDestination=self.ex_destination_trqx))
        ...
        case_id_4 = bca.create_event("SOR 2. Check sequence of 12 Aggressive POV child orders", self.test_id)
        self.fix_verifier_buy_2.set_case_id(bca.create_event("Check 12 child orders Buy side NewOrderSingle Aggressive", case_id_4))
        ...
        self.fix_verifier_buy_2.set_case_id(bca.create_event("Check 12 child orders Buy Side Pending New Aggressive", case_id_4))
        ...
        self.fix_verifier_buy_2.set_case_id(bca.create_event("Check 12 child orders Buy Side New Aggressive", case_id_4))
        ...
        self.fix_verifier_buy_2.set_case_id(bca.create_event("Check 12 child orders Buy Side Eliminate Aggressive", case_id_4))
