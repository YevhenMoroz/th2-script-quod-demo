...
class QAP_T4524(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.auction_slice_initial_multiplier = 200

        self.auction_child_qty = AFM.get_child_qty_for_auction_first_child(self.indicative_volume, self.percentage_volume, self.qty, self.auction_slice_initial_multiplier)
        self.auction_child_qty_2 = AFM.get_child_qty_for_auction(self.indicative_volume, self.percentage_volume, self.qty)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.pov_algo.update_fields_in_component('QuodFlatParameters', dict(MaxParticipation=self.percentage_volume, AuctionInitialSliceMultiplier=self.auction_slice_initial_multiplier))
        ...
        scheduler.enterabs(end_time_minus_1_min, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.auction_dma_child_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle Auction child order'))
        ...
        scheduler.enterabs(end_time_minus_1_min, 2, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=pending_auction_dma_child_order, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew Auction child order'))
        ...
        scheduler.enterabs(end_time_minus_1_min, 3, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=new_auction_dma_child_order, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport Auction child order'))

        self.fix_verifier_buy.set_case_id(bca.create_event("Check Auction algo child order Buy Side NewOrderSingle, Pending New, New and Cancel", self.case_id_2))
        ...
        cancel_auction_dma_child_order.change_parameters(dict(OrderQty=self.auction_child_qty_2))
