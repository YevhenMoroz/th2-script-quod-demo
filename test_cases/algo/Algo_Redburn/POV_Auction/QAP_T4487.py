...
class QAP_T4487(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.percentage_volume = 30
        self.max_participation_open = 10
        self.max_participation_close = 15
        ...
        self.moo_auction_child_qty = AFM.get_child_qty_for_auction(self.indicative_volume, self.max_participation_open, self.qty)
        self.moc_auction_child_qty = AFM.get_child_qty_for_auction(self.indicative_volume, self.max_participation_close, self.qty)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.pov_algo.update_fields_in_component('QuodFlatParameters', dict(MaxPercentageVolume=self.percentage_volume, MaxParticipationOpen=self.max_participation_open, MaxParticipationClose=self.max_participation_close))
        ...
        self.auction_dma_child_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_1, OrderQty=self.moo_auction_child_qty, Price=self.price, TimeInForce=self.tif_ato, Instrument=self.instrument))
        ...
        self.auction_dma_child_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.auction_dma_child_order.change_parameters(dict(Account=self.account, ExDestination=self.ex_destination_1, OrderQty=self.moc_auction_child_qty, Price=self.price, TimeInForce=self.tif_atc, Instrument=self.instrument))
