...
class QAP_T4312(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).update_MDReqID(self.listing_id, self.fix_env1.feed_handler).set_phase("2")
        ...
        er_rejected.change_parameters(dict(TimeInForce=2, NoParty='*', SecAltIDGrp='*',Account=self.client, NoStrategyParameters='*', LastQty=0, SettlDate='*', Currency='EUR', HandlInst=2, LastPx=0, OrderCapacity='A', QtyType=0, ExecRestatementReason=4, Instrument="*", Text='missing LimitPriceReference')).remove_parameter('ExDestination')
