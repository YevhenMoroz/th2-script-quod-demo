...
class QAP_T4466(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.price = 30
        self.offset = 100
        self.would_price = AFM.calc_bps_offset_minus(self.price, self.offset)
        self.prm_would_reference = Reference.Primary.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_manager_feed_handler.set_case_id(case_id=bca.create_event("Send default prices on market", self.test_id))
        self.full_refresh = FixMessageMarketDataSnapshotFullRefreshAlgo().set_market_data().update_MDReqID(self.listing_id, self.fix_env1.feed_handler)
        self.fix_manager_feed_handler.send_message(fix_message=self.full_refresh)

        self.fix_manager_feed_handler.set_case_id(case_id=bca.create_event("Send trading phase", self.test_id))
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_MDReqID(self.listing_id, self.fix_env1.feed_handler).update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).set_phase(TradingPhases.PreOpen)
        ...
        self.auction_algo.update_fields_in_component("QuodFlatParameters", dict(MaxParticipation=self.percentage, WouldInAuction=1, WouldPriceReference=self.prm_would_reference, WouldPriceOffset=self.offset))
        ...
        self.would_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.would_qty, Price=self.would_price, Instrument=self.instrument, TimeInForce=self.tif_ato))
