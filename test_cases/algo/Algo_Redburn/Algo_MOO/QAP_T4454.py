...
class QAP_T4454(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        reject_rule = rule_manager.add_NewOrderSingle_ExecutionReport_RejectWithReason(self.fix_env1.buy_side, self.account, self.mic, self.price, 2, "Order book is closed (ERROR_CODE:27419)")
        self.rule_list = [reject_rule]
        # endregion
        ...
        self.fix_manager_feed_handler.set_case_id(case_id=bca.create_event("Send Close trading phase", self.test_id))
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_MDReqID(self.listing_id, self.fix_env1.feed_handler).update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).set_phase(TradingPhases.Closed)
        ...
        er_pending_new.change_parameters(dict(TimeInForce=self.tif_ato))
        ...
        er_new.change_parameters(dict(TimeInForce=self.tif_ato))
        ...
        start_time = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.PreOpen, start_time=True) / 1000
        release_time = AFM.change_datetime_from_epoch_to_normal(start_time).astimezone(pytz.utc).isoformat()[:-6]

        case_id_3 = bca.create_event("Send PreOpen phase", self.test_id)
        ...
        scheduler.enterabs(start_time +1 , 1, self.fix_manager_feed_handler.send_message, kwargs=dict(fix_message=self.incremental_refresh_pre_open))

        # region Check child orders

        self.dma_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, TimeInForce=self.tif_ato))
        self.dma_order.change_parameter("TransactTime", ">" + release_time)

        # scheduler.enterabs(start_time + 3, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.dma_order, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle child order'))
        scheduler.enterabs(start_time + 3, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[self.dma_order, self.dma_order], key_parameters_list=[self.key_params_with_ex_destination, self.key_params_with_ex_destination], message_name='Buy side check that there is 2 NewOrderSingle'))

        er_rejected = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order, self.gateway_side_buy, self.status_rejected).change_parameter("Text", "Order book is closed (ERROR_CODE:27419)")
        # scheduler.enterabs(start_time + 3, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=er_rejected, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport Rejected child order'))
        scheduler.enterabs(start_time + 3, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[er_rejected, er_rejected] , key_parameters_list=[self.key_params_ER_child, self.key_params_ER_child], direction=self.ToQuod, message_name='Buy side heck that there is 2 ExecReport Rejected'))
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_auction_order.change_parameters(dict(TimeInForce=self.tif_ato, Text='core order Rejected - Order book is closed (ERROR_CODE:27419). OrdRejReason code: Exchange Closed'))
