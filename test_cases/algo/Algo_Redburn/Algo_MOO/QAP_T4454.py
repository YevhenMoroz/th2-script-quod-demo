...
class QAP_T4454(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.incremental_refresh_pre_open = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).update_MDReqID(self.listing_id, self.fix_env1.feed_handler).set_phase("2")
        ...
        er_cancel_auction_order.add_tag(dict(SettlDate='*')).add_tag(dict(NoParty='*', SecAltIDGrp='*', Text='core order Rejected - Order book is closed (ERROR_CODE:27419). OrdRejReason code: Exchange Closed', LastMkt=self.mic)).change_parameters(dict(TimeInForce=2)).remove_parameters(["CxlQty", 'TargetStrategy', 'OrigClOrdID'])
