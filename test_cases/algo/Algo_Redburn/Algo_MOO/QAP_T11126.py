...
class QAP_T11126(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.close_price, TimeInForce=self.tif_ato, Instrument=self.instrument))
        self.fix_verifier_buy.check_fix_message(fix_message=self.dma_order, key_parameters=self.key_params_with_ex_destination, message_name='Buy side NewOrderSingle child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_pending_new_dma, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_new_dma, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport child order')
