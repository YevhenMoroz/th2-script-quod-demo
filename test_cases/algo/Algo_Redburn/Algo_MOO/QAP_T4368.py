...
class QAP_T4368(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.auction_algo.update_fields_in_component("QuodFlatParameters", dict(MaxParticipation=self.percentage, WouldInAuction=1, TriggerPriceRed=self.would_price, MaxWouldPercentage=self.max_would_percentage))
        ...
        case_id_3 = bca.create_event("Check that wouldn't order doesn't exist", self.test_id)
        self.fix_verifier_buy.set_case_id(case_id_3)

        scheduler.enterabs(self.checkpoint, 1, self.fix_verifier_buy.check_fix_message_sequence, kwargs=dict(fix_messages_list=[self.dma_order], key_parameters_list=[self.key_params_with_ex_destination], direction=self.FromQuod, pre_filter=self.data_set.get_pre_filter('pre_filer_equal_D')))
        scheduler.run()
        # endregion
