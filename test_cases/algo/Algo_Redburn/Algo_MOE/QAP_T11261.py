...
class QAP_T11261(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_MDReqID(self.listing_id, self.fix_env1.feed_handler).update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).set_phase(TradingPhases.Expiry)
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.valid_for_auction))
        self.fix_verifier_buy.check_fix_message(self.dma_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle child')
        ...
        self.fix_verifier_buy.check_fix_message(er_pending_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew child ')
        ...
        self.fix_verifier_buy.check_fix_message(er_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport dark child')
        # endregion
        ...
        self.would_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.would_qty, Price=self.would_price, Instrument=self.instrument, TimeInForce=self.valid_for_auction))
        self.would_order.change_parameter("TransactTime", ">"+ would_time_from)
        scheduler.enterabs(self.checkpoint, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.would_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle would child'))

        self.would_order2 = deepcopy(self.would_order)
        self.would_order2.change_parameter("TransactTime", "<" + would_time_to)
        scheduler.enterabs(self.checkpoint, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.would_order2, key_parameters=self.key_params, message_name='Buy side NewOrderSingle would child less checkpoint + 0.1ms'))
        ...
        scheduler.enterabs(self.checkpoint, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=er_pending_new_would, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew would child'))
        ...
        scheduler.enterabs(self.checkpoint, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=er_new_would, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport would child'))
    ...
    def run_post_conditions(self):
        ...
        er_cancel_auction_order.change_parameters(dict(TimeInForce=self.tif_gtx))
