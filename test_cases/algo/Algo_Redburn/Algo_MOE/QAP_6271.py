...
def execute(report_id):
    try:
        ...
        moe_order = FixMessageNewOrderSingleAlgo().set_MOE_params()
        moe_order.add_ClordId((os.path.basename(__file__)[:-3]))
        moe_order.change_parameters(dict(Account= client, OrderQty = qty))
        moe_order.update_fields_in_component('QuodFlatParameters', dict(WouldPriceOffset=would_price_offset))

        fix_manager.send_message_and_receive_response(moe_order, case_id_1)
        ...
        reject_moe_order_params.change_parameters(dict(Text=text_reject_would_price_reference, TimeInForce=tif_atc))
