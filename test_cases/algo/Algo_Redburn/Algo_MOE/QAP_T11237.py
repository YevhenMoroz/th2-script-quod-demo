...
class QAP_T11237(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        now = datetime.now()
        end_date_expiry = now + timedelta(minutes=1)
        # endregion

        # region Update Trading Phase
        self.rest_api_manager.set_case_id(case_id=bca.create_event("Modify trading phase profile", self.test_id))
        trading_phases = AFM.get_timestamps_for_next_phase(TradingPhases.Expiry)
        trading_phases = AFM.update_endtime_for_trading_phase_by_phase_name(trading_phases, TradingPhases.Expiry, end_date_expiry)
        ...
        self.auction_algo.update_fields_in_component("QuodFlatParameters", dict(MaxParticipation=self.percentage_volume))
        self.fix_manager_sell.send_message_and_receive_response(self.auction_algo, case_id_1)

        # region Check Sell side
        self.fix_verifier_sell.check_fix_message(self.auction_algo, key_parameters=self.key_params, direction=self.ToQuod, message_name='Sell side NewOrderSingle')
        ...
        self.fix_verifier_sell.check_fix_message(er_pending_new, key_parameters=self.key_params_cl, message_name='Sell side ExecReport PendingNew')
        ...
        self.fix_verifier_sell.check_fix_message(er_new, key_parameters=self.key_params_cl, message_name='Sell side ExecReport New')
        # endregion
        ...
        start_time = AFM.get_timestamp_from_list(phases=trading_phases, phase=TradingPhases.Expiry, start_time=True) - 2
        release_time = AFM.change_datetime_from_epoch_to_normal(start_time).astimezone(pytz.utc).isoformat()[:-6]
        release_time_end = AFM.change_datetime_from_epoch_to_normal(start_time + 1).astimezone(pytz.utc).isoformat()[:-6]

        self.fix_manager_feed_handler.set_case_id(case_id=bca.create_event("Send trading phase - Open", self.test_id))

        self.open_phase = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative().update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', self.indicative_volume).update_MDReqID(self.listing_id, self.fix_env1.feed_handler).set_phase(TradingPhases.Open)
        scheduler.enterabs(end_time, 1, self.fix_manager_feed_handler.send_message, kwargs=dict(fix_message=self.open_phase))


        self.dma_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, TimeInForce='A', Instrument=self.instrument))
        ...
        self.dma_order2.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.price, TimeInForce='A', Instrument=self.instrument))
        self.dma_order2.change_parameter("TransactTime", "<" + release_time_end)


        scheduler.enterabs(end_time + 3, 1, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.dma_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle child order'))
        scheduler.enterabs(end_time + 3, 2, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=self.dma_order2, key_parameters=self.key_params, message_name='Buy side NewOrderSingle child order'))
        ...
        scheduler.enterabs(end_time + 3, 3, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=er_pending_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew child order'))
        ...
        scheduler.enterabs(end_time + 3, 4, self.fix_verifier_buy.check_fix_message, kwargs=dict(fix_message=er_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport child order'))
        # endregion
