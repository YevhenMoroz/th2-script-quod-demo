...
class QAP_T11268(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.close_price = 29
        self.percentage = 100
        self.lmt_reference = Reference.Close.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.mic, self.close_price)
        ...
        self.incremental_refresh_with_close_px = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_ltq()\
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntryPx', self.close_price)\
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntryType', '5')\
            .update_MDReqID(self.listing_id, self.fix_env1.feed_handler)\
            .set_phase(TradingPhases.Expiry)
        self.fix_manager_feed_handler.send_message(fix_message=self.incremental_refresh_with_close_px)
        # endregion
        ...
        self.incremental_refresh = FixMessageMarketDataIncrementalRefreshAlgo().set_market_data_incr_refresh_indicative()\
            .update_value_in_repeating_group('NoMDEntriesIR', 'MDEntrySize', 1000).update_MDReqID(self.listing_id, self.fix_env1.feed_handler)\
            .set_phase(TradingPhases.Expiry)
        ...
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.child_qty, Price=self.close_price, TimeInForce=self.valid_for_auction, Instrument=self.instrument))
        self.fix_verifier_buy.check_fix_message(fix_message=self.dma_order, key_parameters=self.key_params, message_name='Buy side NewOrderSingle child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_pending_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport PendingNew child order')
        ...
        self.fix_verifier_buy.check_fix_message(fix_message=er_new_dma, key_parameters=self.key_params, direction=self.ToQuod, message_name='Buy side ExecReport child order')
