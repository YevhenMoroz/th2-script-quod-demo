...
class QAP_T4209(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.free_notes = FreeNotesReject.AuctionOrderEliminatedCoreOrderRejected.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule = rule_manager.add_NewOrderSingle_ExecutionReport_Reject(self.fix_env1.buy_side, self.account, self.mic, self.price)
        ...
        self.fix_verifier_buy.set_case_id(case_id=case_id_2)

        self.dma_order = FixMessageNewOrderSingleAlgo(data_set=self.data_set).set_DMA_RB_params()
        self.dma_order.change_parameters(dict(Account=self.account, ExDestination=self.mic, OrderQty=self.qty, Price=self.price, TimeInForce=self.tif_atc, Instrument=self.instrument))
        ...
        er_reject_dma = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.dma_order, self.gateway_side_buy, self.status_reject).remove_parameters(["OrdRejReason", "Text"])
        self.fix_verifier_buy.check_fix_message(fix_message=er_reject_dma, key_parameters=self.key_params_ER_child, direction=self.ToQuod, message_name='Buy side ExecReport reject child order')
        # endregion
    ...
    def run_post_conditions(self):
        ...
        er_cancel_auction_order.add_tag(dict(Text=self.free_notes))
