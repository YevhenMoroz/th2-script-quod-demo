...
class QAP_T8986(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.trigger_instrument = self.instrument
        ...
        self.trigger_px = 24
        self.trig_px_type = constants.TriggerPriceType.BestMid.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        dma_trade_rule = rule_manager.add_NewOrdSingleExecutionReportTrade(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.price, self.qty, self.delay)

        self.rule_list = [nos_dma_rule, dma_trade_rule]
        # endregion
        ...
        self.Triggering_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd, ExpireDate=self.expire_date, Side=self.side))
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument='*', TimeInForce=self.tif_gtd, ExpireDate=self.expire_date, Side=self.side))
        ...
        self.fill_Triggering_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.Triggering_order, self.gateway_side_sell, self.status_fill)
        self.fix_verifier_sell.check_fix_message(self.fill_Triggering_order_params, key_parameters=self.key_params, direction=self.FromQuod, message_name='Sell side ExecReport Fill')
        # endregion
