...
class QAP_T9081(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.expire_date = (datetime.today() + timedelta(days=2)).strftime("%Y%m%d")
        self.price_ask = 22
        self.price_ask_2 = 21
        self.price_bid = 20
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.Triggering_order.change_parameters(dict(Account=self.client, OrderQty=self.qty, Price=self.price, Instrument=self.instrument, TimeInForce=self.tif_gtd, ExpireDate=self.expire_date))
        ...
        self.dma_order.change_parameters(dict(OrderQty=self.qty, Price=self.price, Instrument='*', TimeInForce=self.tif_gtd, ExpireDate=self.expire_date))
    ...
    def run_post_conditions(self):
        ...
        cancel_requets_Triggering_order = FixMessageOrderCancelRequest(self.Triggering_order)
        self.fix_manager_sell.send_message_and_receive_response(cancel_requets_Triggering_order, case_id_3)
        # endregion

        # region check cancel SS
        self.fix_verifier_sell.check_fix_message(cancel_requets_Triggering_order, direction=self.ToQuod, message_name='Sell side Cancel Request')

        cancel_requets_Triggering_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.Triggering_order, self.gateway_side_sell, self.status_cancel)
        self.fix_verifier_sell.check_fix_message(cancel_requets_Triggering_order, message_name='Sell side ExecReport Cancelled')
        # endregion
        ...
        self.fix_verifier_buy.check_fix_message(cancel_dma_order, self.key_params, self.ToQuod, "Buy Side ExecReport canceld Child DMA")
        # endregion
