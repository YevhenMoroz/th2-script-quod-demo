...
class QAP_T4883(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        nos_rule_1 = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew(self.fix_env1.buy_side, self.account, self.ex_destination_1, self.inc_price)
        ocrr_rule = rule_manager.add_OrderCancelReplaceRequest_ExecutionReport(self.fix_env1.buy_side, False)
        ocr_rule = rule_manager.add_OrderCancelRequest(self.fix_env1.buy_side, self.account, self.ex_destination_1, True)
        self.rule_list = [nos_rule, nos_rule_1, ocr_rule, ocrr_rule]
        # endregion
        ...
        self.twap_order.add_fields_into_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='StartDate', StrategyParameterType=19, StrategyParameterValue=f'{start_time}'), dict(StrategyParameterName='EndDate', StrategyParameterType=19, StrategyParameterValue=f'{end_time}'), dict(StrategyParameterName='Waves', StrategyParameterType='1', StrategyParameterValue=self.waves)])
        ...
        self.twap_order_replace_params = FixMessageOrderCancelReplaceRequestAlgo(self.twap_order)
        self.twap_order_replace_params.change_parameter('Price', self.inc_price)
        self.fix_manager_sell.send_message_and_receive_response(self.twap_order_replace_params, case_id_2)

        time.sleep(1)

        self.fix_verifier_sell.check_fix_message(self.twap_order_replace_params, direction=self.ToQuod, message_name='Sell side OrderCancelReplaceRequest')

        replaced_twap_order_params = FixMessageExecutionReportAlgo().set_params_from_order_cancel_replace(self.twap_order_replace_params, self.gateway_side_sell, self.status_cancel_replace)
        ...
        cancel_slice1_order.change_parameters(dict(Price=self.inc_price, TimeInForce=self.tif_ioc, OrdType=self.order_type)).remove_parameter('ExDestination')
        ...
        self.slice2_order.change_parameters(dict(OrderQty=self.slice2_qty, Price=self.inc_price, Instrument='*', TimeInForce=self.tif_day))
    ...
    def run_post_conditions(self):
        ...
        # region check cancellation child DMA slice 2
        cancel_slice2_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.slice2_order, self.gateway_side_buy, self.status_cancel)
        cancel_slice2_order.change_parameter('ExDestination', self.ex_destination_1)
        self.fix_verifier_buy.check_fix_message(cancel_slice2_order, self.key_params, self.ToQuod, "Buy Side ExecReport Cancel Child DMA Slice 2")
        # endregion
        ...
        cancel_twap_order.change_parameters(dict(NoParty='*', Price=self.inc_price, SettlType='B'))
