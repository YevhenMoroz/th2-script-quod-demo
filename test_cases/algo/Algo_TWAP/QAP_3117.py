...
class QAP_3117(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.twap_order.add_fields_into_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='StartDate', StrategyParameterType=19, StrategyParameterValue=start_time), dict(StrategyParameterName='EndDate', StrategyParameterType=19, StrategyParameterValue=end_time), dict(StrategyParameterName='Aggressivity',
                    StrategyParameterType='1', StrategyParameterValue=self.aggressivity), dict(StrategyParameterName='Waves', StrategyParameterType='1', StrategyParameterValue=self.waves)])
        ...
        pending_twap_order_params.remove_parameter('NoParty')
        ...
        eliminate_slice1_order = FixMessageExecutionReportAlgo().set_params_from_new_order_single(slice1_order, self.gateway_side_buy, self.status_cancel)
        eliminate_slice1_order.change_parameters(dict(Price=self.price, TimeInForce=self.tif_ioc, OrdType=self.order_type)).remove_parameter('ExDestination')
        ...
        eliminate_slice2_order.change_parameters(dict(Price=self.price, TimeInForce=self.tif_ioc, OrdType=self.order_type)).remove_parameter('ExDestination')
        ...
        self.fix_verifier_sell.check_fix_message(eliminate_twap_order, key_parameters=self.key_params_cl,  message_name='Sell side ExecReport Eliminate')
        # endregion
