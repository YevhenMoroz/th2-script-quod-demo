...
class QAP_T4268(TestCase):
    ...
    def __init__(self, report_id, data_set=None, environment=None):
        ...
        self.time_format = constants.StrategyParameterType.String.value
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.twap_order_replace_params.update_repeating_group('NoStrategyParameters', [dict(StrategyParameterName='EndDate', StrategyParameterType=self.time_format, StrategyParameterValue=end_time_mod)])
        self.fix_manager_sell.send_message_and_receive_response(self.twap_order_replace_params, case_id_2)

        time.sleep(3)

        self.twap_order_replace_params_reject = FixMessageOrderCancelRejectReportAlgo().set_params_from_new_order_single(nos_twap_parent, self.gateway_side_sell, self.status_new)
        self.twap_order_replace_params_reject.change_parameters(dict(CxlRejResponseTo='*', ExecRestatementReason='*'))
        self.fix_verifier_sell.check_fix_message(self.twap_order_replace_params_reject, key_parameters=self.key_params_cl, message_name='Sell side ExecReport Reject')
        # endregion
