...
class QAP_T4605(TestCase):
    ...
    def run_post_conditions(self):
        # region cancel Order
        self.case_id_cancel = bca.create_event("Check Eliminated Algo Order", self.test_id)
        ...
        cancel_twap_order_params = FixMessageExecutionReportAlgo().set_params_from_new_order_single(self.twap_order, self.gateway_side_sell, self.status_eliminate)
        cancel_twap_order_params.add_tag(dict(Text=self.free_notes)).remove_parameter("LastMkt")
        self.fix_verifier_sell.check_fix_message(cancel_twap_order_params, key_parameters=self.key_params_cl, message_name='Sell side ExecReport Eliminate')
        # endregion
