...
class QAP_T3580(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region step 3, Check that user with hierarchical level - Zone can not Modify 'Location' without 'zoneID' field
        ...
        data_validation(test_id=self.test_id,
                        event_name="Check that user with hierarchical level - Zone can not Modify 'Location' without"
                                   "'zoneID' field.",
                        expected_result=self.error_message_empty_zone,
                        actual_result=modify_location_response)
        # endregion

        # region step 4, Check that user with hierarchical level - Zone can not Create 'Location' without 'zoneID' field
        ...
        create_location_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.location_message))
        data_validation(test_id=self.test_id,
                        event_name="Check that user with hierarchical level - Zone can not Create 'Location' without"
                                   "'zoneID' field.",
                        expected_result=self.error_message_empty_zone,
                        actual_result=create_location_response)
        # endregion

        # region step 6, Check that user with hierarchical level - Zone can not Modify 'Desk' without 'locationID' field
        ...
        data_validation(test_id=self.test_id,
                        event_name="Check that user with hierarchical level - Zone can not Modify 'Desk' without"
                                   "'locationID' field.",
                        expected_result=self.error_message_empty_location,
                        actual_result=modify_desk_response)
        # endregion

        # region step 7, Check that user with hierarchical level - Zone can not Create 'Desk' without 'locationID' field
        ...
        create_desk_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.desk_message))
        data_validation(test_id=self.test_id,
                        event_name="Check that user with hierarchical level - Zone can not Create 'Desk' without"
                                   "'locationID' field.",
                        expected_result=self.error_message_empty_location,
