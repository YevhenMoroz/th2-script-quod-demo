...
from test_framework.rest_api_wrappers.web_admin_api.Risk_Limits_API.trading_limit_api.RestApiTradingLimit import \
    RestApiTradingLimit
...
class QAP_T8618(TestCase):
    def __init__(self, report_id, data_set: BaseDataSet, environment):
        ...
        self.trading_limit_message = RestApiTradingLimit(data_set=data_set)
        ...
        self.trading_limit_id = f'{self.qap_id}_api_rule_step2'
        self.error_message_step1 = "Request fails: code=QUOD-32533:Request not allowed:  HierarchicalLevel validations:" \
                                   " User adm_rest is not allowed to execute Adminmonitoring_TradingLimitCreationRequest"
        self.error_message_step2 = "Request fails: code=QUOD-32533:Request not allowed:  HierarchicalLevel validations:" \
                                   " User adm_rest is not allowed to execute Adminmonitoring_TradingLimitModificationRequest"
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region step 1, Create 'TradingLimit' rule with 'Institution' which assigned to another hierarchical assignments
        parameters_step1 = {
            "tradingLimitDesc": self.trading_limit_id,
            "maxOrdAmtCurrency": "EUR",
            "institutionID": self.institution_id_rin['institutionID']
        }
        self.trading_limit_message.create_trading_limit_rule(custom_params=parameters_step1)
        create_trading_limit_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.trading_limit_message))
        data_validation(test_id=self.test_id,
                        event_name=f"Create 'TradingLimit' rule with 'Institution'"
                                   f" which assigned to another hierarchical assignments.",
                        expected_result=create_trading_limit_response,
                        actual_result=self.error_message_step1)
        # endregion

        # region step 2, Modify 'TradingLimit' rule with 'Institution' which assigned to another hierarchical assignments
        parameters_step2 = {
            "tradingLimitDesc": self.trading_limit_id,
            "maxOrdAmtCurrency": "EUR",
        }
        self.trading_limit_message.create_trading_limit_rule(custom_params=parameters_step2)
        self.wa_api_manager_test.send_post_request(self.trading_limit_message)

        self.trading_limit_message.find_all_trading_limit_rules()
        trading_limit_rule = self.wa_api_manager_test.parse_response_details(
            response=self.wa_api_manager_test.send_get_request(self.trading_limit_message),
            filter_dict={"tradingLimitDesc": self.trading_limit_id})
        data_validation(test_id=self.test_id,
                        event_name=f"Check that trading limit '{self.trading_limit_id}' rule was created",
                        expected_result=self.trading_limit_id,
                        actual_result=trading_limit_rule[0]['tradingLimitDesc'])
        trading_limit_rule[0].update(self.institution_id_rin)
        trading_limit_rule[0].pop('alive')
        self.trading_limit_message.modify_trading_limit_rule(params=trading_limit_rule[0])
        modify_trading_limit_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.trading_limit_message))
        data_validation(test_id=self.test_id,
                        event_name=f"Modify 'TradingLimit' rule with 'Institution'"
                                   f" which assigned to another hierarchical assignments.",
                        expected_result=modify_trading_limit_response,
