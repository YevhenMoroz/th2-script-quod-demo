...
from test_framework.rest_api_wrappers.web_admin_api.Risk_Limits_API.RestApiRiskLimitDimensions import RestApiRiskLimitDimensions
...
class QAP_T3203(TestCase):
    def __init__(self, report_id, data_set: BaseDataSet, environment):
        ...
        self.risk_limit_dimension_id = f'{self.qap_id}_api_rule_step2'
        self.error_message_step1 = "Request fails: code=QUOD-32533:Request not allowed:  HierarchicalLevel validations:" \
                                   " User adm_rest is not allowed to execute Adminmonitoring_RiskLimitDimensionCreationRequest"
        self.error_message_step2 = "Request fails: code=QUOD-32533:Request not allowed:  HierarchicalLevel validations:" \
                                   " User adm_rest is not allowed to execute Adminmonitoring_RiskLimitDimensionModificationRequest"
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region step 1, Create 'RiskLimitDimension' rule with 'Institution' which assigned to another hierarchical assignments
        parameters_step1 = {
            "riskLimitDimensionName": f'{self.qap_id}_api_rule_step1',
            "riskLimitDimensionDesc": f'{self.qap_id}_api_rule_step1',
            "riskLimitDimensionExp": "test",
            "institutionID": self.institution_id_rin['institutionID'],
            "riskLimitDimDesk": [
                {
                    "deskID": self.desk_id_test
                }
            ],
        }
        self.risk_limit_dimension_message.create_risk_limit_dimension(custom_params=parameters_step1)
        create_risk_limit_dimension_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.risk_limit_dimension_message))
        data_validation(test_id=self.test_id,
                        event_name=f"Create 'RiskLimitDimension' rule with 'Institution'"
                                   f" which assigned to another hierarchical assignments.",
                        expected_result=create_risk_limit_dimension_response,
                        actual_result=self.error_message_step1)
        # endregion

        # region step 2, Modify 'RiskLimitDimension' rule with 'Institution' which assigned to another hierarchical assignments
        parameters_step2 = {
            "riskLimitDimensionName": self.risk_limit_dimension_id,
            "riskLimitDimensionDesc": self.risk_limit_dimension_id,
            "riskLimitDimensionExp": "test",
            "riskLimitDimDesk": [
                {
                    "deskID": self.desk_id_test
        ...
        self.risk_limit_dimension_message.create_risk_limit_dimension(custom_params=parameters_step2)
        self.wa_api_manager_test.send_post_request(self.risk_limit_dimension_message)

        self.risk_limit_dimension_message.find_all_risk_limit_dimension()
        risk_limit_dimension_rule = self.wa_api_manager_test.parse_response_details(
            response=self.wa_api_manager_test.send_get_request(self.risk_limit_dimension_message),
            filter_dict={"riskLimitDimensionName": self.risk_limit_dimension_id})
        data_validation(test_id=self.test_id,
                        event_name=f"Check that risk limit '{self.risk_limit_dimension_id}' rule was created",
                        expected_result=self.risk_limit_dimension_id,
                        actual_result=risk_limit_dimension_rule[0]['riskLimitDimensionName'])

        risk_limit_dimension_rule[0].update(self.institution_id_rin)
        risk_limit_dimension_rule[0].pop('alive')
        self.risk_limit_dimension_message.modify_risk_limit_dimension(params=risk_limit_dimension_rule[0])
        modify_risk_limit_dimension_response = self.wa_api_manager_test.parse_response_error_message_details(
            response=self.wa_api_manager_test.send_multiple_request(self.risk_limit_dimension_message))
        data_validation(test_id=self.test_id,
                        event_name=f"Modify 'RiskLimitDimension' rule with 'Institution'"
                                   f" which assigned to another hierarchical assignments.",
                        expected_result=modify_risk_limit_dimension_response,
