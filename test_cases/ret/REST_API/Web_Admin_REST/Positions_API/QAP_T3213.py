...
class QAP_T3213(TestCase):
    def __init__(self, report_id, data_set: BaseDataSet, environment):
        ...
        # mdfW1JW540Y24GIzFiwmeQ
        self.tested_instrument_id = self.data_set.get_instrument_id_by_name('instrument_id_1')
        # "api_account_rin_desk"
        self.tested_security_account = self.data_set.get_account_by_name('account_4')
        self.tested_collateral_qty = 10
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.security_account_position_message.find_positions(self.tested_security_account)
        security_position_after_collateral = self.wa_api_manager.parse_response_details(
            response=self.wa_api_manager.send_get_request_with_parameters(self.security_account_position_message),
            filter_dict={'instrID': self.tested_instrument_id})
        security_position_fields_after_collateral = security_position_after_collateral[0].keys()
        if 'collateralQty' in security_position_fields_after_collateral:
            data_validation(self.test_id,
                            ...
                            actual_result=float(security_position_after_collateral[0]['collateralQty']))
        else:
