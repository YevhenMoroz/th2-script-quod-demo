...
def execute(report_id):
    ...
    # region Set two hierarchy levels - step 2
    multiple_hierarchy_params = {
    ...
    api_message_tested_user.modify_user(multiple_hierarchy_params)
    api_manager_tested_user.send_post_request(api_message_tested_user)
    # endregion

    # region Send Get request after negative Post request and verify result - step 2
    api_message_tested_user.find_all_user()
    response = api_manager_tested_user.send_get_request(api_message_tested_user)
    print(response)
    user_params = api_manager_tested_user.get_response_details(
        ...
        entity_field_id="userID")
    verifier(case_id=case_id,
             event_name="Check User HierarchicalLevel after send negative POST request",
             expected_value="5",
             actual_value=user_params["deskUserRole"].list_value.values[0].message_value.fields["deskID"].simple_value)

    # endregion
    #
    # # region Set hierarchy level - Site, step 3
    #
    # site_params = {
    #     "userConfirmFollowUp": "false",
    #     "userID": "adm_rest",
    ...
    # # endregion
    #
    # # region Send Get request after negative Post request and verify result - step 3
    ...
    #                                                entity_field_id="userID")
    #
    # # In this area we got error exception, because have a bug according with "User Admin Data Segregation"
    # # User with hierarchical level = Desk should not set hierarchy level = Site
    # try:
    #     verifier(case_id=case_id,
    #              event_name="Check User HierarchicalLevel after send negative POST request",
    #              expected_value="5",
