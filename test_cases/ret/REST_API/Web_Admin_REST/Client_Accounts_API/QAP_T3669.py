...
from test_framework.rest_api_wrappers.web_admin_api.Client_Accounts_API.RestApiClientGroupMessages import \
    RestApiClientGroupMessages
from test_framework.core.try_exept_decorator import try_except


class QAP_T3669(TestCase):
    def __init__(self, report_id, data_set: BaseDataSet, environment):
        ...
        self.api_client_group_message = RestApiClientGroupMessages(data_set=data_set)
        self.wa_api_manager = WebAdminRestApiManager(session_alias=self.session_alias_wa, case_id=self.test_id)
        self.error_message = "invalid request code=QUOD-17:Message format is incorrect, ClientGroupName isn't found"

    @try_except(test_id=os.path.basename(__file__)[:-3])
    def run_pre_conditions_and_steps(self):
        # region, step 2, Create new ClientGroup without required fields and check result
