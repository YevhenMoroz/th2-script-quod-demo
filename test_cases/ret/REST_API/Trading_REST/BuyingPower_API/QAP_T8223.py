    ...
    def run_pre_conditions_and_steps(self):
        ...
        if 'ExecType' in nos_response.keys() and nos_response['ExecType'] == 'Open':
            ...
            if 'ExecType' in cancel_response_buy_side.keys():
                ...
                # region check BuyingPower updating upon order creation, Sell side
                self.nos_message.set_default_request()
                self.nos_message.change_parameter(parameter_name='Side', new_parameter_value='Sell')
                self.nos_message.change_parameter(parameter_name='Price', new_parameter_value=self.price + 10)
                self.nos_message.change_parameter(parameter_name='OrdQty', new_parameter_value=self.qty + 10)
                self.nos_message.change_parameter_in_component(component_name='PreTradeAllocations',
                                                               fields={'AllocQty': self.qty + 10})
                self.nos_message.change_key_fields_web_socket_response({})
                ...
                if 'ExecType' in nos_response.keys() and nos_response['ExecType'] == 'Open':
                    self.cash_account_message.find_cash_account_counters(cash_account_id=self.cash_account_id)
                    booked_amount_after_sell_order_creation = self.wa_api_manager.parse_response_details(
                        response=self.wa_api_manager.send_get_request_with_parameters(self.cash_account_message))
                    self.security_position_message.find_positions(security_account_name=self.security_account)

                    position_after_sell_order_creation = self.wa_api_manager.parse_response_details(
                        response=self.wa_api_manager.send_get_request_with_parameters(self.security_position_message),
                        filter_dict={'instrID': self.instrument_id})

                    calculation_results_after_sell_order_creation = self.formulas_manager.calc_booked_amount_sell_side(
                        test_id=self.test_id,
                        response_wa_cash_account=booked_amount_after_buy_order_creation[0],
                        response_wa_security_account=position_after_buy_order_creation,
                        fee=self.fee,
                    ...
                    print(calculation_results_after_sell_order_creation)
                    booked_amount_validation(test_id=self.test_id,
                                             ...
                                             booked_amount_simulated=calculation_results_after_sell_order_creation,
                                             reserved_qty_current=position_after_sell_order_creation[0])

                    # region Cancel new order and check BuyingPower updating, Side=Sell
                    self.cancel_message.set_cancellation_parameters(nos_response)
                    self.cancel_message.change_key_fields_web_socket_response({'OrderStatus': 'Cancelled'})
                    cancel_response_sell_side = self.trd_api_manager.parse_response_details(
                        response=self.trd_api_manager.send_http_request_and_receive_websocket_response(self.cancel_message))

                    if 'ExecType' in cancel_response_sell_side.keys():
                        self.cash_account_message.find_cash_account_counters(cash_account_id=self.cash_account_id)

                        calculation_results_after_sell_order_cancellation = self.formulas_manager.calc_booked_amount_sell_side(
                            test_id=self.test_id,
                            response_wa_cash_account=booked_amount_after_sell_order_creation[0],
                            response_wa_security_account=position_after_sell_order_creation,
                            fee=self.fee,
                            commission=self.commission,
                            execution_type=cancel_response_buy_side['ExecType']
                        )
                        booked_amount_validation(test_id=self.test_id,
                                                 event_name='Booked Amount calculation upon order Cancellation. Side=Sell',
                                                 booked_amount_current=booked_amount_before_order_creation[0],
                                                 booked_amount_simulated=calculation_results_after_sell_order_cancellation,
                                                 reserved_qty_current=position_before_order_creation[0])
                    else:
                        bca.create_event("Order Cancel response wasn't received. (Side=Sell)",
                                         status="FAILED", parent_id=self.test_id)
                    # endregion
                else:
                    bca.create_event("New Order Single response wasn't received. (Side=Sell)",
                                     status="FAILED", parent_id=self.test_id)
                # endregion
            else:
