...
class QAP_T8209(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        if 'ExecType' in nos_response.keys() and nos_response['ExecType'] == 'Open':
            ...
            # region BuyingPower updating upon Price increasing
            self.price += 5
            self.modification_message.set_modification_parameters(nos_response, {'Price': self.price})
            ...
            if 'ExecType' in modified_response.keys() and modified_response['ExecType'] == 'Replaced':
                ...
                booked_amount_validation(test_id=self.test_id,
                                         event_name=f'Booked Amount upon order Modification. Increase Price to {self.price}.',
                ...
                # region BuyingPower updating upon Price decreasing
                self.price -= 5
                self.modification_message.set_modification_parameters(nos_response, {'Price': self.price})
                self.modification_message.change_key_fields_web_socket_response({'ExecType': 'Replaced'})
                modified_response = self.trd_api_manager.parse_response_details(
                ...
                if 'ExecType' in modified_response.keys() and modified_response['ExecType'] == 'Replaced':
                    ...
                    calculation_results_after_order_modification = self.formulas_manager.calc_booked_amount_buy_side(
                        ...
                        amended_net_order_value=self.qty * self.price + self.commission + self.fee)
                    booked_amount_validation(test_id=self.test_id,
                                             event_name=f'Booked Amount upon order Modification. Decrease Price to {self.price}.',
                    ...
                    bca.create_event(f"Modification response wasn't received, (decrease Price to {self.price})",
                                     status="FAILED",
                                     parent_id=self.test_id)
                # endregion
            else:
                bca.create_event(f"Modification response wasn't received, (increase Price to {self.price})",
                                 status="FAILED",
                                 parent_id=self.test_id)
            # endregion
        else:
