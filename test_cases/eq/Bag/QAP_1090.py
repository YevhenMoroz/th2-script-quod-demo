...
class QAP_1090(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        qty_of_bag = QAP_1090._adjustment_of_value(str(int(qty) * 3))
        leaves_qty_of_bag_after_cancel_order = QAP_1090._adjustment_of_value(str(int(qty) * 2))
        orders_id = []
        name_of_bag = 'QAP_1090'
        ...
        # region cancel  order from bag
        java_api_message = CancelOrderRequest()
        java_api_message.set_default(orders_id[0])
        self.java_api_manager.send_message(java_api_message)
        self.client_inbox.accept_and_cancel_children(filter=inbox_filter)
        self.order_book.set_filter([OrderBookColumns.order_id.value, orders_id[0]])
        sts_of_order = self.order_book.extract_field(OrderBookColumns.sts.value)
        self.order_book.compare_values({OrderBookColumns.sts.value: ExecSts.cancelled.value},
                                       {OrderBookColumns.sts.value: sts_of_order},
                                       'Comparing values of order after cancelling it')
        # endregion
        ...
        self.__extracting_and_comparing_value_for_bag_order([OrderBagColumn.order_bag_qty.value,
                                                             ...
                                                             OrderBagColumn.leaves_qty.value
                                                                ...
                                                                name_of_bag,
                                                                qty_of_bag,
                                                                leaves_qty_of_bag_after_cancel_order
                                                            ],
                                                            False, ' modification')
        # endregion
