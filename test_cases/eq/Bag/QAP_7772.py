...
class QAP_7772(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        for i in range(2):
            if i == 1:
                self.fix_message.change_parameter("TimeInForce", '6')
                self.fix_message.add_tag({'ExpireDate': expire_data})
        ...
        try:
            new_order_single = rule_manager.add_NewOrdSingleExecutionReportPendingAndNew_FIXStandard(
                ...
                float(price))
            self.bag_order_book.set_order_bag_wave_details(price=price, qty=str(int(qty) * 2))
        ...
        for index in range(len(orders_id)):
            values = self.order_book.extract_2lvl_fields(SecondLevelTabs.child_tab.value,
                                                         [OrderBookColumns.expire_date.value,
                                                          OrderBookColumns.tif.value],
                                                         [1],
                                                         {OrderBookColumns.order_id.value: orders_id[index]})[0]
            if index == 0:
                self.order_book.compare_values(
                    {OrderBookColumns.tif.value: TimeInForce.DAY.value, OrderBookColumns.expire_date.value: ''}, values,
                    f'Comparing values of {orders_id[index]} order')
            else:
                values[OrderBookColumns.expire_date.value] = values[OrderBookColumns.expire_date.value].replace('/', '')
                self.order_book.compare_values({OrderBookColumns.tif.value: TimeInForce.GTD.value,
                                                OrderBookColumns.expire_date.value: expire_data}, values,
                                               f'Comparing values of {orders_id[index]} order')
        # endregion
