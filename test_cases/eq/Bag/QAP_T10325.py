...
class QAP_T10325(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region Step 2
        self.bag_creation_request.set_default(BagChildCreationPolicy.Biggest_to_Smallest.value, bag_name, orders_id)
        ...
        self.java_api_manager.compare_values(
            ...
            'Check second CO after sliced order trade (step 6)')

        # second CO
        self.__check_order_sts(ord_id2, ExecutionReportConst.TransExecStatus_PFL.value, self.qty_create_order_1 + '.0',
                               6)

        # sliced order
        self.__check_order_sts(sliced_ord_id_1, ExecutionReportConst.TransExecStatus_FIL.value,
                               self.qty_create_order_1 + '.0', 6)
        # endregion
        ...
        # region check orders sts after trade
        # first CO
        self.__check_order_sts(ord_id, ExecutionReportConst.TransExecStatus_PFL.value, '50.0', 8)
        ...
        # first CO
        self.__check_order_sts(ord_id, ExecutionReportConst.TransExecStatus_FIL.value, self.qty1 + '.0', 10)

        # sliced order
        self.__check_order_sts(sliced_ord_id_3, ExecutionReportConst.TransExecStatus_FIL.value,
                               self.qty_create_order_3 + '.0', 10)
        # endregion
