...
class QAP_T7644(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        exp_res = {JavaApiFields.PostTradeStatus.value: "RDY", JavaApiFields.TransExecStatus.value: "PFL"}
        self.java_api_manager.compare_values(exp_res, ord_rep, "Check order")
        bag_not = self.java_api_manager.get_last_message(ORSMessageType.OrderBagNotification.value, ord_id).get_parameter(
            JavaApiFields.OrderBagNotificationBlock.value)
        exp_res = {JavaApiFields.LeavesQty.value: str(float(self.qty)), JavaApiFields.OrderBagExecStatus.value: "PFL"}
        self.java_api_manager.compare_values(exp_res, bag_not, "Check bag")
        # endregion
