...
class QAP_T7537(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        qty = '200'
        price = '10'
        account = self.data_set.get_venue_client_names_by_name('client_pos_3_venue_1')
        client = self.data_set.get_client_by_name('client_pos_3')
        self.fix_message.set_default_dma_limit()
        self.fix_message.change_parameter('OrderQtyData', {'OrderQty': qty})
        self.fix_message.change_parameter('Account', self.data_set.get_client_by_name('client_pos_3'))
        ...
        # region book order and verify values after it at order book(step 1 and step 2)
        error = self.middle_office.check_error_in_book()
        error.pop('hasErrorMessage')

        self.middle_office.compare_values({'errorMessage': f'Error - [QUOD-11699] Invalid AccountGroupID: {client}'},
                                          error, 'Comparing Error Message after book')
        # endregion
