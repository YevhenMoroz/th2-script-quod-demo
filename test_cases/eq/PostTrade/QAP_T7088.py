...
from test_framework.java_api_wrappers.java_api_constants import JavaApiFields, AllocationReportConst, \
    ConfirmationReportConst, SubmitRequestConst
...
class QAP_T7088(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ord_notif = self.java_api_manager.get_last_message(ORSMessageType.OrdNotification.value).get_parameters()[
        ...
        self.all_instr.update_fields_in_component("AllocationInstructionBlock",{"AccountGroupID":client})
        self.java_api_manager.send_message_and_receive_response(self.all_instr)
        allocation_report = \
                self.java_api_manager.get_last_message(ORSMessageType.AllocationReport.value).get_parameters()[
                JavaApiFields.AllocationReportBlock.value]
        alloc_id = allocation_report[JavaApiFields.ClientAllocID.value]
        expected_result = {JavaApiFields.MatchStatus.value: ConfirmationReportConst.MatchStatus_UNM.value}
        actually_result = {JavaApiFields.MatchStatus.value: allocation_report[JavaApiFields.MatchStatus.value]}
        ...
        res = Path('./logs.txt').read_text()
        time = (tm(datetime.utcnow().isoformat())).strftime('%Y-%b-%d')
        self.java_api_manager.compare_values({
            'LastExecTransactTime': f'LastExecTransactTime={time}'}, {
             "LastExecTransactTime": res}, "Check Allocation report",
