...
from test_framework.java_api_wrappers.ors_messages.BlockValidateRequest import BlockValidateRequest
...
class QAP_T7469(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set=None, environment=None):
        ...
        self.val_req = BlockValidateRequest(self.data_set)
    ...
    def run_pre_conditions_and_steps(self):
        ...
        account1 = "DUMMY_AKK"
        account2 = self.data_set.get_account_by_name("client_pt_8_acc_1")
        ...
        # endregion
        # region Step 2
        cl_alloc_id = self.fix_alloc.get_parameter("AllocationInstructionBlock")["ClientAllocID"]
        alloc_id = alloc_report["AllocInstructionID"]
        alloc_inst_acc_id = alloc_report["AllocAccountList"]["AllocAccountBlock"][0]["AllocInstructionAccountID"]
        self.alloc_ack.set_default(cl_alloc_id, "3")
        self.fix_verifier.check_fix_message(self.alloc_ack, message_name="Step 2")
        # endregion
        # region Step 3-4
        self.val_req.set_default(alloc_id, alloc_inst_acc_id, client, account2)
        self.java_api_manager.send_message_and_receive_response(self.val_req)
        alloc_report = self.java_api_manager.get_last_message(ORSMessageType.AllocationReport.value).get_parameter(
            JavaApiFields.AllocationReportBlock.value)
        self.java_api_manager.compare_values({"AllocAccountID": account2}, alloc_report["AllocAccountList"][
            "AllocAccountBlock"][0], "Step 3-4")
        # endregion
