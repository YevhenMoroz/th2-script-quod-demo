...
class QAP_T11439(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        param[0]["gatingRuleCondExp"] = "TimeInForce!=Day"
        ...
        # region Step 1 - Check that the Gating rule is not applied to the order
        ...
        self.java_api_manager.compare_values(
            ...
            order_notification,
            "Step 1 - Check GatingRuleCondName and ExecPcy = Care",
        )
        # endregion

        # region Step 2 - Create 2nd Care order
        self.order_submit.update_fields_in_component(
            "NewOrderSingleBlock", {"TimeInForce": "AtTheClose", "ClOrdID": basic_custom_actions.client_orderid(9)}
        )
        responses = self.java_api_manager.send_message_and_receive_response(self.order_submit)
        print_message("CREATE", responses)
        # endregion

        # region Step 2 - Check that the Gating rule is applied to the order
        order_notification = self.java_api_manager.get_last_message(
            ORSMessageType.OrdNotification.value,
        ).get_parameters()[JavaApiFields.OrderNotificationBlock.value]
        ...
        self.java_api_manager.compare_values(
            {
                ...
                JavaApiFields.TimeInForce.value: "ATC"
            },
            order_notification,
            "Step 2 - Check GatingRuleCondName and ExecPcy = DMA",
        )
        # endregion
