...
class QAP_T9291(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        ors = tree.getroot().find("ors/FrontToBack/GatingRules/removeOrdrParametersFromDMAOrder")
        ors.text = 'true'
        tree.write("temp.xml")
        ...
        try:
            ...
            algo_params_presents = False
            if JavaApiFields.ExternalAlgoParameterListBlock.value not in order_reply[JavaApiFields.ExternalAlgoParametersBlock.value]:
                algo_params_presents = True
            self.ja_manager.compare_values({'AlgoParamsAreNotPresented': 'True'},
                                           {'AlgoParamsAreNotPresented': str(algo_params_presents)},
                                           f'Check that {JavaApiFields.ExternalAlgoParameterListBlock.value} not present')
