...
class QAP_T4296(TestCase):
    ...
    def send_submit_message_and_check_result(self, order_submit):
        ...
        status = self.ja_manager.get_last_message(ORSMessageType.OrdReply.value).get_parameters()[
            JavaApiFields.OrdReplyBlock.value]
        act_res_rule.update(status)
        self.ja_manager.compare_values({"GatingRuleCondName": "Default Result",
                                        JavaApiFields.ExecType.value: OrderReplyConst.ExecStatus_OPN.value},
                                       act_res_rule,
