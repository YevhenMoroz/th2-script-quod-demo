...
class QAP_T8944(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.trade_entry_message.set_default_trade(order_id, self.price, qty_to_execution)
        ...
        self.java_api_manager.compare_values(
            {JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_PFL.value}, exec_report,
            'Check order after first execution')
        self.java_api_manager.send_message_and_receive_response(self.trade_entry_message)
        exec_report = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value).get_parameter(
            JavaApiFields.ExecutionReportBlock.value)
        self.java_api_manager.compare_values(
            {JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_FIL.value}, exec_report,
            'Check order after second execution')
        # endregion
        ...
        # STEP 5
        # region cancel first manual execution
        ...
        self.java_api_manager.compare_values(
            {JavaApiFields.ExecPrice.value: '0.0',
             ...
             JavaApiFields.ExecType.value: ExecutionReportConst.ExecType_CAN.value}, exec_report,
            'Check order after execution cancellation')
        # endregion

        # STEP 6
        # region create new execution
        self.trade_entry_message = TradeEntryOMS(self.data_set)
        self.trade_entry_message.set_default_trade(order_id, self.price_to_third_fill, qty_to_execution)
        self.java_api_manager.send_message_and_receive_response(self.trade_entry_message)
        exec_report = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value).get_parameter(
            JavaApiFields.ExecutionReportBlock.value)
        self.java_api_manager.compare_values(
            {JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_FIL.value}, exec_report,
            'Check order after third execution')
        # endregion
