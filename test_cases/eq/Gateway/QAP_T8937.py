...
class QAP_T8937(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.trade_entry_message.set_default_trade(order_id, self.price_to_full_fill)
        ...
        self.trade_entry_message.set_default_replace_execution(order_id, exec_id, self.price_to_full_fill, qty_to_execution)
        self.java_api_manager.send_message_and_receive_response(self.trade_entry_message)
        exec_report = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value).get_parameter(
            JavaApiFields.ExecutionReportBlock.value)
        self.java_api_manager.compare_values(
            {JavaApiFields.ExecPrice.value: self.price_to_full_fill + '.0',
        ...
        exec_report = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value).get_parameter(
            JavaApiFields.ExecutionReportBlock.value)
        self.java_api_manager.compare_values(
            {JavaApiFields.ExecPrice.value: self.price + '.0',
             JavaApiFields.ExecQty.value: qty_to_execution + '.0',
             JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_FIL.value}, exec_report,
            'Check order after full filling')
        # endregion
