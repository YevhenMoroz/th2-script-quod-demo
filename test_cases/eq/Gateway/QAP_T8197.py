...
class QAP_T8197(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        new_qty = "200"
        self.modify_request.set_modify_order_limit(cl_ord_id1, new_qty)
        ...
        self.accept_request.set_default(ord_id1, cd_order_notif_id, desk_id1, 'M', True)
        ...
        self.java_api_manager.compare_values(
            ...
            order_reply,
            "Step 3 - Checking that order isn't updated (Step 3)",
        )
        # endregion
        ...
        # region Step 7 - Accept cancel replace request
        self.accept_request.set_default(ord_id1, cd_order_notif_id, desk_id1, 'M')
        self.java_api_manager.send_message_and_receive_response(self.accept_request)
        order_reply = self.java_api_manager.get_last_message(ORSMessageType.OrdReply.value, ord_id1).get_parameters()[
            JavaApiFields.OrdReplyBlock.value
        ]
        self.java_api_manager.compare_values(
            {JavaApiFields.TransStatus.value: OrderReplyConst.TransStatus_OPN.value,
             JavaApiFields.OrdQty.value: new_qty + '.0'},
            order_reply,
            "Step 3 - Checking that order is modified (Step 7)",
        )
        # endregion
