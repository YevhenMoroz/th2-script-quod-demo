...
class QAP_T11235(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        reject_notif = self.java_api_manager.get_last_message(ORSMessageType.OrdRejectedNotif.value).get_parameters()[JavaApiFields.OrdRejectedNotifBlock.value]
        self.java_api_manager.compare_values({JavaApiFields.Reason.value: f"ErrorCD: [QUOD-11642] - ErrorLevel: [Error] - ErrorMsg: [Unknown account identifier: {client_account_id}]"},
                                             reject_notif, 'Verify that order rejected')
