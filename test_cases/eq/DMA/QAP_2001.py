...
class QAP_2001(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        try:
            nos_rule = self.rule_manager.add_MarketNewOrdSingle_IOC_FIXStandard(self.bs_connectivity,
                                                                                self.venue_client_names, self.venue,
                                                                                True, 350, 20)
            self.fix_message.set_default_dma_market()
            self.fix_message.change_parameters({'Side': '2', 'TimeInForce': '3'})
        ...
        self.order_book.compare_values({OrderBookColumns.exec_sts.value: ExecSts.partially_filled.value},
