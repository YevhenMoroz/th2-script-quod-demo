...
class QAP_T10241(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.listing_quote.set_default(
            self.listing_id, quoting_px_ref_type="MID", spread_px_type="PRC", bid_spread=spread, offer_spead=spread
        )
        ...
        # region Step 2
        bid_px = str((float(best_ask) + float(best_bid)) / 2 - float(spread))
        offer_px = str((float(best_ask) + float(best_bid)) / 2 + float(spread))
        self.quote_request.set_default(
            self.listing_id, offer_px, bid_px, offer_size, bid_size, side="B", th_px=last_trd_px
        )
        ...
        self.java_api_manager.compare_values(exp_res, ord_rep, "Step 3 - Check created order")
        # endregion
