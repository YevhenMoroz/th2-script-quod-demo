...
class QAP_T10184(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.listing_quote.set_default(self.listing_id, spread_px_type="PRC", bid_spread=new_spread, offer_spead=new_spread)
        self.java_api_manager.send_message(self.listing_quote)
        bid_px: str = str(float(last_trd_px) - float(new_spread))
        offer_px: str = str(float(last_trd_px) + float(new_spread))
        self.quote_request.set_default(self.listing_id, offer_px, bid_px,notional_amt=not_amt)
        ...
        self.quote_status_report.set_default(quote_id, "ACK")
        self.java_api_manager.send_message(self.quote_status_report)
        time.sleep(1)
        res = self.db_manager.execute_query(
            f"SELECT quotestatus, bidpx, offerpx FROM quote WHERE quoteid = '{quote_id}'")
        self.java_api_manager.compare_values({"quotestatus": "ACK", "bidpx": bid_px, "offerpx": offer_px},{
            "quotestatus": res[0][0], "bidpx": str(float(res[0][1])), "offerpx": str(float(res[0][2]))}, "Check quote")
        # endregion
