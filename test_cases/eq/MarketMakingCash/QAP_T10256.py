...
class QAP_T10256(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.quote_request.set_default(self.listing_id, offer_px, bid_px, offer_size, bid_size, th_px=open_px)
        ...
        quote_count = self.db_manager.execute_query("select count(*) from quote;")[0][0]
        # endregion

        # region Step 3
        self.java_api_manager.send_message(self.quote_request)
        quote_count_new = self.db_manager.execute_query("select count(*) from quote;")[0][0]
        self.java_api_manager.compare_values({"count": quote_count}, {"count": quote_count_new}, "Check Quote count")
        # endregion
