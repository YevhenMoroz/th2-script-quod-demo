...
class QAP_T10224(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_manager_fh.send_message(self.market_data_refresh)
        self.market_data_refresh.set_market_data_incr_refresh(md_req_id, "1", "4", open_price, size)
        self.fix_manager_fh.send_message(self.market_data_refresh)
        # endregion
        # region Step 1
        self.listing_quote.set_default(self.listing_id, quoting_px_ref_type="OPN", spread_px_type="PRC",
        ...
        # endregion
        # region Step 2
        bid_px = str(float(open_price) - float(spread))
        offer_px = str(float(open_price) + float(spread))
        self.quote_request.set_default(self.listing_id, offer_px, bid_px, notional_amt=nat_amt, side="B",
                                       th_px=str(float(open_price)))
        ...
        exp_res = {"OrdQty": "486.0", "Price": bid_px, "Side": "B"}
