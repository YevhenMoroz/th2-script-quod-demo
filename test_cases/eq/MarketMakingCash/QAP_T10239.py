...
class QAP_T10239(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_manager_fh.send_message(self.market_data_refresh)
        self.market_data_refresh.set_market_data_incr_refresh(md_req_id, "1", "5", closing_px, size)
        self.fix_manager_fh.send_message(self.market_data_refresh)
        # endregion
        # region Step 1
        ...
        # endregion
        # region Step 2
        bid_px = str(float(closing_px)-float(spread))
        offer_px = str(float(closing_px)+float(spread))
        self.quote_request.set_default(self.listing_id, offer_px, bid_px, offer_size, bid_size, th_px=closing_px)
