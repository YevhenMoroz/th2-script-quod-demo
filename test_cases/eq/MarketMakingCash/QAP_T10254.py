...
class QAP_T10254(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.quote_request.set_default(self.listing_id, offer_px, bid_px, size, size, str(float(close_px)))
        ...
        ord_rep = self.java_api_manager.get_last_message(ORSMessageType.OrdNotification.value, ord_id).get_parameter(
            JavaApiFields.OrderNotificationBlock.value)
        ord_rep2 = self.java_api_manager.get_last_message(ORSMessageType.OrdNotification.value, ord_id2).get_parameter(
            JavaApiFields.OrderNotificationBlock.value)
        exp_res = {"OrdStatus": "SUB"}
        self.java_api_manager.compare_values(exp_res, ord_rep, "Check order")
        self.java_api_manager.compare_values(exp_res, ord_rep2, "Check order2")
        # endregion
