...
class QAP_T10175(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.listing_quote.set_default(
            self.listing_id, quoting_px_ref_type="PRI", spread_px_type="PRC", bid_spread=spread, offer_spead=spread
        )
        ...
        # region Step 2
        bid_px: str = str(float(best_bid) - float(spread))
        offer_px: str = str(float(best_ask) + float(spread))
        self.quote_request.set_default(
            self.listing_id, offer_px, bid_px, notional_amt=not_amt, th_bid_px=best_bid, th_offer_px=best_ask
        )
        ...
        exp_res.pop("TheoreticalOfferPx")
        exp_res.pop("TheoreticalBidPx")
