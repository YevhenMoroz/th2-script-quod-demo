...
class QAP_T10221(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        size = "200"
        spread = "1.0"
        mid = str((int(best_ask) + int(best_bid))/2)
        # region Precondition
        ...
        # endregion
        # region Step 1
        self.listing_quote.set_default(self.listing_id, quoting_px_ref_type="MID", spread_px_type="PRC",
        ...
        # endregion
        # region Step 2
        bid_px = str(float(mid) - float(spread))
        offer_px = str(float(mid) + float(spread))
        ...
        exp_res.update({"BidSize": "527.0"})
        exp_res.pop("NotionalAmt")
        self.java_api_manager.compare_values(exp_res, quote_rep, "Check created quote")
        ord_rep = self.java_api_manager.get_last_message(ORSMessageType.OrdNotification.value).get_parameters()[
            JavaApiFields.OrderNotificationBlock.value]
        exp_res = {"OrdQty": "527.0", "Price": bid_px, "Side": "B"}
