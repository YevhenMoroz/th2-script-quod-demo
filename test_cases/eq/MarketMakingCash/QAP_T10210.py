...
class QAP_T10210(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        bid_size = "527.0"
        offer_size = "435.0"
        mid = str((float(best_ask) + float(best_bid)) / 2)
        ...
        # region Step 2
        bid_px: str = str(float(mid) - float(spread))
        offer_px: str = str(float(mid) + float(spread))
        self.quote_request.set_default(self.listing_id, offer_px, bid_px, notional_amt=not_amt, th_px=mid)
