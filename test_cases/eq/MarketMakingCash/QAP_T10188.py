...
class QAP_T10188(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        bid_size = "109"
        offer_size = "90"
        self.quote_request.set_default(self.listing_id, offer_px, bid_px,notional_amt=not_amt_new)
        ...
        res = self.db_manager.execute_query(
            f"SELECT quotestatus, bidsize, offersize FROM quote WHERE quoteid = '{quote_id}'")
        self.java_api_manager.compare_values({"quotestatus": "ACK", "bidsize": bid_size, "offersize": offer_size},
                                             {"quotestatus": res[0][0], "bidsize": str(int(res[0][1])),
                                              "offersize":str(int(res[0][2]))}, "Check new quote")
        # endregion
