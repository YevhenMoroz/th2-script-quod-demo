...
class QAP_T10166(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        try:
            ...
            trd_rule = self.rule_manager.add_NewOrdSingleExecutionReportTrade_FIXStandard(
                self.fix_env.buy_side, self.venue_client, self.exec_destination, float(bid_px), int(bid_size[:-2]), 1)
            ...
            time.sleep(3)
            self.rule_manager.remove_rules([trd_rule, cr_rule, nos_rule])
        quote_rep = self.java_api_manager.get_last_message(QSMessageType.QuoteStatusReport.value,"CXL").get_parameters()[
            "QuoteStatusReportBlock"]
        self.java_api_manager.compare_values({"QuoteStatus": "CXL"}, quote_rep, "Check quotes status")
        ...
        exp_res = {"Price": bid_px, "Side": "B", "TransStatus": "TER"}
        ...
        exp_res = {"Price": offer_px, "Side": "S", "TransStatus": "CXL"}
