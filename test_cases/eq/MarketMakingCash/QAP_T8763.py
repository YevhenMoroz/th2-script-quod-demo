...
class QAP_T8763(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        new_ord_id2 = basic_custom_actions.client_orderid(9)
        self.exec_rep.set_default_trade(new_ord_id2)
        self.exec_rep.update_fields_in_component("ExecutionReportBlock", {"InstrumentBlock": self.instrument
            , "QuoteMsgID": quote_id, "LastPx": offer_px, "AvgPrice": offer_px,"Side": "Sell"})
        self.java_api_manager.send_message(self.exec_rep)
        self.quote_status_report.set_default(quote_id, "TER")
        self.java_api_manager.send_message(self.quote_status_report)
        time.sleep(3)
        res = self.db_manager.execute_query(f"SELECT count(execid)  FROM execution WHERE quoteid = '{quote_id}'")
        self.java_api_manager.compare_values({"Exec": 2}, {"Exec": res[0][0]}, "Check execution present")
        res = self.db_manager.execute_query(f"SELECT quotestatus FROM quote WHERE quoteid = '{quote_id}'")
        self.java_api_manager.compare_values({"quotestatus": "TER"}, {"quotestatus": res[0][0]}, "Check quote status")
        self.stop_quoting_req.set_default(self.listing_id)
        self.java_api_manager.send_message(self.stop_quoting_req)
        # endregion
        # region Step 5: Start quoting
        self.java_api_manager.send_message_and_receive_response(self.quote_request)
        quote_rep = self.java_api_manager.get_last_message(QSMessageType.QuoteStatusReport.value).get_parameters()[
            "QuoteStatusReportBlock"]
        self.java_api_manager.compare_values({"QuoteStatus": "ACK", "BidPx": bid_px, "OfferPx": offer_px}, quote_rep,
