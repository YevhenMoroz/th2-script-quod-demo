...
class QAP_T7229(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.commission_sender.set_modify_client_commission_message(comm_profile='12', client=self.client).change_message_params({"accountGroupID":self.client, "venueID": self.data_set.get_venue_by_name("venue_2")}).send_post_request()
        order_id = None
        try:
        ...
        exec_id = self.order_book.extract_sub_lvl_fields([OrderBookColumns.exec_id.value], [SecondLevelTabs.executions.value], {OrderBookColumns.order_id.value:order_id})
        # endregion
        # region check commission currency
        res = self.trade_book.extract_sub_lvl_fields([TradeBookColumns.comm_cur.value], TradeBookColumns.cl_comm_tab.value, 1, {TradeBookColumns.exec_id.value: exec_id[OrderBookColumns.exec_id.value]})
        self.trade_book.compare_values({TradeBookColumns.comm_cur.value: self.comm_cur}, {TradeBookColumns.comm_cur.value: res['1']}, "Check commissions currency")
        # endregion
