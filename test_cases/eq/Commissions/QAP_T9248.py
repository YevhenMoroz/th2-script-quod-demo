...
class QAP_T9248(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.java_api_manager.compare_values(
            ...
            exec_report, 'check ExecSts of Manual execution')
        # endregion
        ...
        self.java_api_manager.compare_values(
            {JavaApiFields.MiscFeeType.value: ExecutionReportConst.MiscFeeType_EXC.value,
             ...
             JavaApiFields.MiscFeeRate.value: '0.01'}, misc_fees_block, 'Check fee after complete')
        # endregion

        # region dma send order
        nos_rule = None
        trade_rule = None
        try:
            ...
            trade_rule = self.rule_manager.add_NewOrdSingleExecutionReportTrade_FIXStandard(self.bs_connectivity,
                                                                                            ...
                                                                                            int(self.price),
                                                                                            int(self.qty), 2)
            self.fix_message.set_default_dma_limit("instrument_3")
            self.fix_message.change_parameters(
                {'ClOrdID': basic_custom_actions.client_orderid(9), "Account": self.client,
                 "ExDestination": self.mic,
                 "Currency": self.cur})
            response = self.fix_manager.send_message_and_receive_response_fix_standard(self.fix_message)
        finally:
        ...
        # check fees after execution
        miscs = {'NoMiscFees': [{'MiscFeeAmt': '0.01', 'MiscFeeCurr': self.misc_cur, 'MiscFeeType': '4'}]}
        ignored_fields = ['GatingRuleCondName', 'GatingRuleName', 'Parties', 'QuodTradeQualifier', 'BookID', 'Currency',
                          'TradeReportingIndicator', 'NoParty', 'SecondaryOrderID', 'tag5120', 'ExecBroker',
                          'CommissionData']
        self.execution_report.set_default_calculated(self.fix_message)
        self.execution_report.change_parameters({'NoMiscFees': miscs})
        self.fix_verifier.check_fix_message_fix_standard(self.execution_report, ignored_fields=ignored_fields)
        # endregion
