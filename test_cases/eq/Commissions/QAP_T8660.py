...
class QAP_T8660(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.rest_commission_sender.set_modify_client_commission_message(comm_profile=self.perc_amt,
            ...
            {'commissionAmountType': self.comm_type_research, 'venueID': self.venue})
        self.rest_commission_sender.send_post_request()
        self.rest_commission_sender.set_modify_client_commission_message(comm_profile=self.perc_amt,
                                                                         client=self.client).change_message_params(
            {'commissionAmountType': self.comm_type_local, 'clCommissionID': self.commission2.value,
    ...
    def __check_comm_amount_type(self, report, action):
        com_type_list_exp = sorted([self.comm_type_research, self.comm_type_local])
        # extract actual data
        com_type_list_act = []
        for i in range(2):
