...
class QAP_T7431(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region Wave
        self.wave_creation_request.set_default(list_id, ord_id_list=[ord_id2], percent_qty=self.per_qty1)
        ...
        self.java_api_manager.compare_values(
            ...
            'Check created first wave')
        # endregion

        # region Verify child orders
        ord_notify_element = list_wave_notify_block['OrdNotificationElements']['OrdNotificationBlock']
        self.java_api_manager.compare_values(
            {JavaApiFields.OrdQty.value: '70.0', JavaApiFields.RootParentOrdID.value: ord_id2},
            ord_notify_element[0],
            'Check first Child Order after waving')
        # endregion

        # region Wave
        self.wave_creation_request.set_default(list_id, ord_id_list=[ord_id2], percent_qty=self.per_qty2)
        responses = self.java_api_manager.send_message_and_receive_response(self.wave_creation_request)
        self.return_result(responses, ORSMessageType.OrderListWaveNotification.value)
        list_wave_notify_block = self.result.get_parameter('OrderListWaveNotificationBlock')
        # endregion
        ...
        self.java_api_manager.compare_values(
            ...
            list_wave_notify_block,
            'Check created second wave')
        # endregion

        # region Verify child orders
        ord_notify_element = list_wave_notify_block['OrdNotificationElements']['OrdNotificationBlock']
        self.java_api_manager.compare_values(
            {JavaApiFields.OrdQty.value: '30.0', JavaApiFields.RootParentOrdID.value: ord_id2},
            ord_notify_element[0],
            'Check first Child Order after waving')
        # endregion
