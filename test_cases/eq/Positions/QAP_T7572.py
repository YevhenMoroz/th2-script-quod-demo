...
class QAP_T7572(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.pos_trans.set_default_amend_transfer(self.acc1, self.acc2, trans_id, self.qty_to_transfer_third,
                                                  self.price)
        self.pos_trans.update_fields_in_component("PositionTransferInstructionBlock",
                                                  {"InstrID": self.instrument_id})
        self.ja_manager.send_message_and_receive_response(self.pos_trans)
        ...
        self._check_common_daily_pl_and_today_daily_pl(posit_acc1_before_transfer, common_daily_pl_before_transfer,
                                                       self.qty_to_transfer_third, 'step 10')
        # endregion
    ...
    def _check_common_daily_pl_and_today_daily_pl(self, posit_before_transfer, common_daily_pl_before_transfer,
        ...
        self.ja_manager.compare_values({JavaApiFields.DailyRealizedNetPL.value: today_net_pl_after_transfer},
                                       position_response[JavaApiFields.PositionList.value][
                                           JavaApiFields.PositionBlock.value][0],
                                       f'Check that {JavaApiFields.DailyRealizedNetPL.value} has properly value ({step})')
        self.ja_manager.compare_values({JavaApiFields.TodayRealizedPL.value: expected_daily_pl},
                                       ...
                                       f'Check that {JavaApiFields.TodayRealizedPL.value} has properly value ({step})')
