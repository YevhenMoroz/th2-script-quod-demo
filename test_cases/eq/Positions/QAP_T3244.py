...
class QAP_T3244(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        posit_qty = result[JavaApiFields.PositQty.value]
        self.ja_manager.compare_values({JavaApiFields.PositQty.value: self.qty},
                                       result,f'Make sure, that {JavaApiFields.PositQty.value}  == {self.qty} (step 2)')
        self.order_submit.update_fields_in_component(JavaApiFields.NewOrderSingleBlock.value,
        ...
        [JavaApiFields.LeavesSellQty.value, self.account, f"{JavaApiFields.PositionType.value}:'L'"]).get_parameters()[
