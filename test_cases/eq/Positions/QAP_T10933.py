...
class QAP_T10933(TestCase):
    ...
    def __init__(self, report_id, session_id, data_set, environment):
        ...
        self.acc2 = self.data_set.get_account_by_name('client_pt_1_acc_2')
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.java_api_manager.compare_values(pre_trade_dict1, ord_reply, 'Check first Order after modification')
        # endregion

        # region steps 5 Amend second order:
        pre_trade_dict2 = {'PreTradeAllocationBlock': {
            'PreTradeAllocationList': {'PreTradeAllocAccountBlock': [
                {'AllocAccountID': self.acc2,
                 'AllocQty': str(float(self.qty2))}]}}}
        self.order_modify.set_change_params(order_id2)
        self.order_modify.update_fields_in_component('OrderModificationRequestBlock', pre_trade_dict2)
        self.java_api_manager.send_message_and_receive_response(self.order_modify)

        ord_reply = self.java_api_manager.get_last_message(ORSMessageType.OrdReply.value).get_parameter(
            JavaApiFields.OrdReplyBlock.value)
        self.java_api_manager.compare_values(pre_trade_dict2, ord_reply, 'Check second Order after modification')
        # endregion
        ...
        # region step 8 Send wave request
        nos_rule = None
        try:
        ...
        # region step 10: get position for Security Account CareWB
