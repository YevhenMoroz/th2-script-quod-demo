...
class QAP_T3245(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.ja_manager.compare_values({JavaApiFields.PositQty.value: self.qty},
                                       result,
                                       f'Make sure, that {JavaApiFields.PositQty.value}  == {self.qty} (step 2)')
        self.order_submit.update_fields_in_component(JavaApiFields.NewOrderSingleBlock.value,
                                                     {JavaApiFields.ClOrdID.value: bca.client_orderid(9),
                                                      JavaApiFields.OrdQty.value: self.qty_for_second_order,
        ...
        self.order_submit.get_parameter(JavaApiFields.NewOrderSingleBlock.value) \
            ...
            [JavaApiFields.PreTradeAllocAccountBlock.value][0][JavaApiFields.AllocQty.value] = self.qty_for_second_order
        self._create_order()
        ord_notification = \
            self.ja_manager.get_last_message(ORSMessageType.OrdNotification.value).get_parameters()[
        ...
        self.ja_manager.compare_values({JavaApiFields.OrdStatus.value: OrderReplyConst.OrdStatus_REJ.value},
                                       ord_notification,
                                       f'Verify that order rejected (step 5)')
        # endregion

        # region step 6: Check that LeavesSellQty still the same
        leaves_sell_qty = \
            self.db_manager.execute_query(f"SELECT leavessellqty FROM posit WHERE accountid = '{self.account}'")[0][0]
