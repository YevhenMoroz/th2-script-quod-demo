...
def execute(report_id, session_id):
    ...
    #         test_cases.old_wrappers.eq_fix_wrappers.get_buy_connectivity(),
    #         client + '_PARIS', "XPAR", float(price))
    #     nos_rule2 = rule_manager.add_NewOrdSingleExecutionReportTrade(
    #         test_cases.old_wrappers.eq_fix_wrappers.get_buy_connectivity(),
    #         client + '_PARIS', 'XPAR',
    #         float(price), int(qty), 1)
    #     test_cases.old_wrappers.eq_fix_wrappers.create_order_via_fix(case_id, 1, 1, client, 2, qty, 0, price)
    # except Exception:
