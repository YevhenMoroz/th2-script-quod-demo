...
class QAP_T7325(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # region step 6-7: Send Order Modification Request
        ...
        # region step 8: Accept + Child  Modification Request
        ...
        cd_ord_notif_id = str(int(
            self.db_manager.execute_query(
                ...
                0][0]))
        self.accept_request.set_default(order_id, cd_ord_notif_id, desk, ack_type='M')
        self.accept_request.update_fields_in_component(JavaApiFields.CDOrdAckBatchRequestBlock.value,
                                                       {
                                                           JavaApiFields.ModifyChildren.value: 'Y'
                                                       })
        self.java_api_manager.send_message_and_receive_response(self.accept_request)
        order_reply = self.java_api_manager.get_last_message(ORSMessageType.OrdReply.value).get_parameters()[
            JavaApiFields.OrdReplyBlock.value]
        self.java_api_manager.compare_values({JavaApiFields.ExecType.value: OrderReplyConst.ExecType_PMO.value},
                                             ...
                                             f'equals {OrderReplyConst.ExecType_PMO.value}, (step 8)')
        ...
        cd_order_notif_message = \
            self.java_api_manager.get_last_message(CSMessageType.CDOrdNotif.value).get_parameters()[
                JavaApiFields.CDOrdNotifBlock.value]
        cd_ord_notif_id = cd_order_notif_message[JavaApiFields.CDOrdNotifID.value]
        self.accept_request.set_default(child_co_order_id, cd_ord_notif_id, desk, 'M', set_reject=True)
        self.java_api_manager_user_2.send_message_and_receive_response(self.accept_request)
        ...
        self.java_api_manager.compare_values({JavaApiFields.ExecType.value: OrderReplyConst.ExecType_PMO.value},
                                             ...
                                             f'Verify that order Parent Order has Sts = {OrderReplyConst.ExecType_PMO.value} (step 10)')
