...
class QAP_T7144(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.fix_manager.send_message(self.cancel_request)
        # endreigon

        # region reject cancel request:step 2-3
        cd_ord_notif_id = str(int(
            self.db_manager.execute_query(
                f"SELECT cdordnotifid FROM cdordnotif WHERE transid = '{parent_order_id}' AND cdrequesttype='CAN'")[
                0][0]))
        self.accept_request.set_default(parent_order_id, cd_ord_notif_id, desk, 'C', set_reject=True)
        self.java_api_manager.send_message_and_receive_response(self.accept_request)
        order_reply = self.java_api_manager.get_last_message(ORSMessageType.OrdReply.value).get_parameters()[
            JavaApiFields.OrdReplyBlock.value]
        self.java_api_manager.compare_values(
