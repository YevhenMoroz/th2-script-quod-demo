...
def execute(report_id, session_id):
    ...
    # endregionA
    # region Create CO
    fix_message1 = test_cases.wrapper.eq_fix_wrappers.create_order_via_fix(case_id, 3, 2, client, 1, int(qty1), 0)
    eq_wrappers.accept_order(lookup, qty1, "0")
    fix_message2 = test_cases.wrapper.eq_fix_wrappers.create_order_via_fix(case_id, 3, 2, client, 1, int(qty2), 0)
    eq_wrappers.accept_order(lookup, qty2, "0")
    fix_message3 = test_cases.wrapper.eq_fix_wrappers.create_order_via_fix(case_id, 3, 1, client, 1, int(qty3), 0)
