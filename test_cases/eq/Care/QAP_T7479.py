...
class QAP_T7479(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        exec_rep = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value, ord_id_care) \
                .get_parameters()[JavaApiFields.ExecutionReportBlock.value]
        exec_rep2 = self.java_api_manager.get_last_message(ORSMessageType.ExecutionReport.value, ord_id_care2) \
            .get_parameters()[JavaApiFields.ExecutionReportBlock.value]
        self.java_api_manager.compare_values({
            JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_FIL.value}, exec_rep,
            'Check execution')
        self.java_api_manager.compare_values({
            JavaApiFields.TransExecStatus.value: ExecutionReportConst.TransExecStatus_FIL.value}, exec_rep2,
            'Check execution2')
        # endregion
