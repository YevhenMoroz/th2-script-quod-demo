...
class QAP_T7663(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.order_book.set_filter([OrderBookColumns.order_id.value, order_id]).manual_execution(qty=str(int(self.qty) - 10))
        # endregion
        ...
        # endregion
        # region accept order via FIX
        self.client_inbox.accept_modify_plus_child()
        # endregion
        # region check changed values
        self.order_book.set_filter([OrderBookColumns.order_id.value, order_id]).check_order_fields_list(
            {OrderBookColumns.exec_sts.value: ExecSts.partially_filled.value, OrderBookColumns.qty.value: self.qty2})
        # endregion
