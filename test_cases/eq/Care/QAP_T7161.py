...
class QAP_T7161(TestCase):
    def __init__(self, report_id, session_id, data_set, environment):
        ...
        self.expected_sec_account = self.data_set.get_account_by_name('dummy_sec_account')
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.java_api_manager.compare_values({JavaApiFields.AllocAccountID.value: self.expected_sec_account},
                                             order_notification[JavaApiFields.PreTradeAllocationBlock.value][
                                                 JavaApiFields.PreTradeAllocationList.value][
                                                 JavaApiFields.PreTradeAllocAccountBlock.value][0],
                                             'Checking that order has properly SecurityAccount (step 1)')
        self.java_api_manager.compare_values({JavaApiFields.OrdStatus.value: OrderReplyConst.OrdStatus_HLD.value},
                                             order_notification, f'Verify that order has {OrderReplyConst.OrdStatus_HLD.value} status (step 1)')
        # endregion
