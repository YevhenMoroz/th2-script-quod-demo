...
class QAP_T10845(TestCase):
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.client_int = self.data_set.get_client_by_name("client_int_2")
        self.account_int = self.data_set.get_account_by_name("account_int_2")
        self.currency = self.data_set.get_currency_by_name("currency_gbp")
        self.gbp_cad = self.data_set.get_symbol_by_name("symbol_10")
        ...
        self.position_verifier = PositionVerifier(self.test_id)
        self.base_qty = "-1000000"
        self.destination_qty = "1000000"
        self.now = None
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.position_transfer.change_symbol(self.gbp_cad)
        ...
        self.position_report.change_parameter("TransactTime",
                                              f">{self.now.isoformat()[:-6]}")
        self.fix_verifier.check_fix_message(self.position_report,
                                            message_name="Check that position report contains TransactTime, LastPositEventType and LastPositUpdateEventID for Source")
        ...
        position_response: list = self.fix_manager.send_message_and_receive_response(self.request_for_position,
                                                                                     self.test_id)
        self.position_verifier.check_base_position(position_response, self.destination_qty,
                                                   text="Check base for destination")
        # endregion
        # region Step 5
        self.pos_report_ack.set_params_from_reqeust(self.request_for_position)
        self.pos_report_ack.change_parameter("TotalNumPosReports", "1")
        ...
        self.fix_verifier.check_fix_message(self.position_report,
                                            message_name="Check that position report contains TransactTime, LastPositEventType and LastPositUpdateEventID for Destination")
