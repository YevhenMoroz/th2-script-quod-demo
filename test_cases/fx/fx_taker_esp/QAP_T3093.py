...
class QAP_T3093(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.order_ticket.set_order_details(tif=self.tif, order_type=self.ord_type, client=self.client, pending=True)
        ...
        self.order_ticket.set_order_details(qty=self.amend_qty)
        self.order_ticket.create_order()
        self.order_ticket.release_order_request([ob.symbol.value, self.symbol, ob.qty.value, self.amend_qty])
        self.order_book.set_filter(
            [ob.symbol.value, self.symbol, ob.qty.value, self.amend_qty]).check_order_fields_list(
