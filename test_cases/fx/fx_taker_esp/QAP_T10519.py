...
class QAP_T10519(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.eur_cad_spot = {"Symbol": self.eur_cad,
                             "SecurityType": self.security_type}
        self.gs_sw = self.data_set.get_market_id_by_name("market_8")

    @try_except(test_id=Path(__file__).name[:-3])
    def run_pre_conditions_and_steps(self):
        # region Step 1
        self.gating_rule.set_main_rule()
        self.gating_rule.enable_gating_rule()
        self.rest_manager.send_post_request(self.gating_rule)
        self.gating_rule.set_secondary_rule()
        self.gating_rule.enable_gating_rule()
        self.rest_manager.send_post_request(self.gating_rule)
        self.sleep(1)
        # region Step 1
        self.new_order.set_default_mo().change_parameters(
            {"Currency": self.eur, "Instrument": self.eur_cad_spot, "ExDestination": self.gs_sw})
