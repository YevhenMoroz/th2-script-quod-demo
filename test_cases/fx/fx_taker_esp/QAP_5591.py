...
# Status
status = DataSet.Status.PartialFill
...
def execute(report_id, session_id):
    ...
    try:
        ...
        new_order_sor = FixMessageNewOrderSingleAlgoFX().set_default_SOR().change_parameters(
            {'TimeInForce': '3', 'OrderQty': '10000000'})
        ...
        time.sleep(5)
        execution_report_partially_filled_1 = FixMessageExecutionReportAlgoFX(). \
            set_params_from_new_order_single(new_order_sor, gateway_side_sell, status)
        fix_verifier.check_fix_message(fix_message=execution_report_partially_filled_1,
        ...
        FXOrderBook(case_id, session_id).set_filter(
            ...
            {ob_col.exec_sts.value: exe_sts.partially_filled.value})
        FXOrderBook(case_id, session_id).set_filter(
            [ob_col.order_id.value, "AO", ob_col.qty.value, "10000000", ob_col.orig.value, "FIX", ob_col.lookup.value,
             "EUR/USD-SPO.SPO", ob_col.client_id.value, "TH2_Taker",
