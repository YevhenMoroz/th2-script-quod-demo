...
class QAP_T3092(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.md_request_ndf = FixMessageMarketDataRequestFX(data_set=self.data_set)
        self.md_snapshot = FixMessageMarketDataSnapshotFullRefreshSellFX()
        self.md_snapshot_ndf = FixMessageMarketDataSnapshotFullRefreshSellFX()
        ...
        self.settle_type = self.data_set.get_settle_type_by_name("spo_ndf")

        self.security_type_ndf = self.data_set.get_security_type_by_name("fx_ndf")
        self.settle_date_ndf = self.data_set.get_settle_date_by_name("wk1_ndf")
        self.settle_type_ndf = self.data_set.get_settle_type_by_name("wk1_ndf")
        ...
        self.md_id_fwd = f"{self.symbol}:NDF:WK1:{self.venue}_{self.rand_id}"
        self.md_req_id_fwd = f"{self.symbol}:NDF:WK1:{self.venue}"
        ...
        self.no_related_symbols_fwd = [{
            "Instrument": self.instrument,
            "SettlDate": self.settle_date_ndf,
            "MarketID": self.market_id
        }]
    ...
    def run_pre_conditions_and_steps(self):
        ...
        # endregion
        # region step 3
        self.md_request_ndf.set_md_req_parameters_taker().change_parameter("MDReqID", self.md_req_id_fwd)
        self.md_request_ndf.update_repeating_group("NoRelatedSymbols", self.no_related_symbols_fwd)
        self.fix_manager_gtw.send_message_and_receive_response(self.md_request_ndf, self.test_id)
        # endregion
        # region step 4
        self.md_snapshot_ndf.set_params_for_md_response_taker_wk1(self.md_request_ndf, ["*"])
        time.sleep(1)
        self.md_snapshot.change_parameters({"SettlDate": self.settle_date_ndf,
                                            "MDReqID": self.md_id_fwd})
        self.md_snapshot_ndf.remove_parameters(["OrigMDArrivalTime", "OrigMDTime"])
        self.fix_verifier.check_fix_message(fix_message=self.md_snapshot_ndf)
        self.md_request_ndf.set_md_uns_parameters_maker()
        self.fix_manager_gtw.send_message_and_receive_response(self.md_request_ndf, self.test_id)
        # endregion
