...
def check_order_book(base_request, act_ob, case_id, instr_type, owner, qty):
    ...
    verifier.compare_values('ExecPcy', 'Care', response[ob_exec_pcy.name])
...
def check_order_book_after_split(base_request, act_ob, case_id, ord_id, initial_qty, split_qty):
    unmatched_qty = str(int(initial_qty)-int(split_qty))
    ob = OrdersDetails()
    execution_id = bca.client_orderid(4)
    ob.set_default_params(base_request)
    ob.set_extraction_id(execution_id)
    ob.set_filter(["Order ID", ord_id])
    ob_qty = ExtractionDetail('orderBook.UnmatchedQty', 'UnmatchedQty')
    ob_ord_id = ExtractionDetail('orderBook.OrderId', 'Order ID')
    ob.add_single_order_info(
        OrderInfo.create(
    ...
    verifier.compare_values('UnmatchedQty after split', response[ob_qty.name].replace(',', ''), unmatched_qty)
    verifier.verify()
...
def split_order(base_request, service, qty):
    order_ticket = FXOrderDetails()
...
def execute(report_id, session_id):
    ...
    split_qty = str(random.randint(1000000, int(initial_qty)))
    try:
        # Step 1
        ...
        split_order(case_base_request, order_ticket_service, split_qty)

        # Step 3
        check_order_book_after_split(case_base_request, ob_act, case_id, placed_order_id, initial_qty, split_qty)
    except Exception:
