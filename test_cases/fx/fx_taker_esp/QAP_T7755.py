...
class QAP_T7755(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.aspect_citi = self.data_set.get_client_by_name("client_1")
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.sleep(4)
        # region Step 1
        self.new_order_sor.set_default_mo().change_parameters({"ExDestination": "HSBC-SW", "Account": self.aspect_citi})
        ...
        #                                                     "trailer", "header"])
        self.result = self.ssh_client. \
            find_regex_pattern("/Logs/quod314/QUOD.FIXBUYTH2ESPO.log",
                               rf"^.*gateway NewOrderSingle.*PartyID=.TESTCP_ClinetID.*{order_id}.*$")
        if self.result:
            self.result = "ok"
        else:
            self.result = "failed"
