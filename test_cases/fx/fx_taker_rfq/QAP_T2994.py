...
class QAP_T2994(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        near_tenor = self.data_set.get_tenor_by_name('tenor_1w')
        ...
        self.order_book.set_filter(
            [ob.symbol.value, usd_php_symbol, ob.qty.value, self.qty]).check_order_fields_list(
            {ob.sts.value: st.terminated.value,
             ob.exec_sts.value: ExecSts.filled.value,
             ob.qty.value: self.qty,
             ob.currency.value: php_currency}, 'Checking currency value in order book')
