...
class QAP_T2621(TestCase):
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.result = "Silver1 haven't mapped in AllocClientAccountID, it's most likely a bug"
        for line in logs:
            self.key = re.findall(rf"^.*{order_id}.*AllocClientAccountID=.QUOD7.*$", line)
            if self.key:
                self.result = "data is a duplicate"
                break
        self.verifier.set_parent_id(self.test_id)
        self.verifier.set_event_name("Check \"AllocClientAccountID\" tag")
        ...
        self.qs = self.tree.getroot().find("connectivity/quickfix/mapAllocAccount/tagList/tag")
        self.qs.text = '1'
        ...
        self.ssh_client.get_file("/Logs/quod314/QUOD.QS_ESP_FIX_TH2.log",
                                 self.temp_path)
        logs = open(self.temp_path, "r")
        self.result = "QUOD7 haven't mapped in AllocClientAccountID, it's most likely a bug"
        for line in logs:
            self.key = re.findall(rf"^.*{order_id}.*AllocClientAccountID=.Silver1.*$", line)
            if self.key:
                self.result = "ok"
                break
        self.verifier.set_parent_id(self.test_id)
