...
class QAP_T10443(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.ah_order_2 = FixMessageNewOrderSingleTakerDC()
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.trade_request_2.update_fields_in_component("TradeEntryRequestBlock",
                                                        {"ClientAccountGroupID": self.client_ext})
        self.trade_request_2.change_instrument(self.eur_gbp, self.instr_type_spo)
        response_2: list = self.java_api_manager.send_message_and_receive_response(self.trade_request_2)
        ah_order_id_2 = self.trade_request_2.get_ord_id_from_held(response_2)
        print(ah_order_id_2)
        ...
        self.ah_order_2.set_default_sor_from_trade(self.trade_request_2)
        self.ah_order_2.change_parameters({"Account": self.client_int, "ClOrdID": ah_order_id_2})
        prefilter = {
            "header": {
                "MsgType": ("D", "EQUAL"),
                "TargetCompID": "QUOD8",
        ...
        key_params = ["Misc0", "ClOrdID"]
        self.fix_drop_copy_verifier.check_fix_message_sequence([self.ah_order, self.ah_order_2],
                                                               key_parameters_list=[key_params, key_params],
                                                               pre_filter=prefilter,
                                                               message_name="Check that we create AH 2 order")
        # endregion
        # region Step 4
        self.ack_request.set_default_ack(ah_order_id)
        self.java_api_manager.send_message(self.ack_request)
        self.ack_request.set_default_ack(ah_order_id_2)
        self.java_api_manager.send_message(self.ack_request)
        self.sleep(5)
        ...
        self.request_for_position_backup.change_parameters({"Instrument": self.instrument, "Currency": self.currency,
                                                            "Account": self.client_back_up})
        self.fix_manager.send_message_and_receive_response(self.request_for_position_backup,
                                                                                 self.test_id)
        self.pos_report_none.set_params_from_reqeust(self.request_for_position_backup)
        self.fix_pos_verifier.check_fix_message(self.pos_report_none,
                                                message_name=f"Check position for {self.client_back_up} after test")
        self.request_for_position_backup.set_unsubscribe()
        self.fix_manager.send_message(self.request_for_position_backup)
        self.sleep(1)
        # endregion
