...
class QAP_T8588(TestCase):
    ...
    def __init__(self, report_id, session_id=None, data_set: BaseDataSet = None, environment: FullEnvironment = None):
        ...
        self.instrument_nok_sek = {"Symbol": self.nok_sek,
                                   "SecurityType": self.sec_type_spot}
        if self.choice == 1:
            self.instrument_spot = {"Symbol": self.eur_nok,
                                    "SecurityType": self.sec_type_spot}
            self.md_req_id = f"{self.eur_nok}:SPO:REG:{self.hsbc}"
        elif self.choice == 2:
            self.instrument_spot = {"Symbol": self.usd_sek,
                                    "SecurityType": self.sec_type_spot}
            self.md_req_id = f"{self.usd_sek}:SPO:REG:{self.hsbc}"
    ...
    def run_pre_conditions_and_steps(self):
        ...
        self.quote_request.update_repeating_group_by_index("NoRelatedSymbols", 0, Account=self.platinum,
                                                           Currency=self.sek, Instrument=self.instrument_nok_sek,
