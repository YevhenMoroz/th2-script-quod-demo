...
class QAP_6706(CommonTestCase):
    ...
    def __init__(self, web_driver_container: WebDriverContainer, second_lvl_id, data_set=None, environment=None):
        ...
        self.id = 'KOPQ2OFNQ'
        self.name = 'KJH1DWHKQ'
        self.ext_id_client = 'HDU4WHQ'
        self.description = 'OJWJD283'
        self.disclose_exec = 'Manual'
        self.confirmation_service = "FIX"
        self.desk = self.data_set.get_desk("desk_1")
        self.desk_2 = self.data_set.get_desk("desk_2")
        self.bic = 'LOJDQN9'
        self.bo_field_1 = 'KHIFU3HQ'
        ...
        self.venue_client_name = 'BJDQKWBD'
        self.route = self.data_set.get_route("route_1")
        self.route_client_name = 'OIHQBJQ'
    ...
    def precondition(self):
        ...
        if not main_page.is_searched_client_found(self.client_name):
            ...
            managements_tab.set_fix_order_recipient_desk(self.desk_2)
    ...
    def test_context(self):
        ...
        try:
            ...
            actual_result = [values_tab.get_name(), values_tab.get_description(), values_tab.get_disclose_exec(),
                             values_tab.get_confirmation_service(), assignments_tab.get_desk(),
                             external_source_tab.get_bic_venue_act_grp_name(), external_source_tab.get_bo_field_1(),
                             ...
                             pos_maintenance_tab.is_pnl_maintenance_selected(), instr_types_tab.click_on_edit(),
                             instr_types_tab.get_instr_type(), venues_tab.click_on_edit(), venues_tab.get_venue(),
                             venues_tab.get_venue_client_name(), routes_tab.click_on_edit(), routes_tab.get_route(),
                             routes_tab.get_route_client_name(), trade_confirm_tab.get_trade_confirm_preference(),
                             trade_confirm_tab.click_on_edit(), trade_confirm_tab.get_email_address(),
                             trade_confirm_tab.get_recipient_types()]
            while None in actual_result:
                actual_result.pop(actual_result.index(None))

            excepted_result = [self.client_name, self.description, self.disclose_exec,
                               self.confirmation_service, self.desk, self.bic, self.bo_field_1, self.desk_2,
                               self.default_execution_strategy, self.cash_maintenance, True, self.instr_type,
                               self.venue, self.venue_client_name, self.route, self.route_client_name,
                               self.trade_confirm_preference, self.email, self.recipient_types]
