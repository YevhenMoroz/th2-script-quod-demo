...
def execute(session_id, report_id):
    ...
    # region Create order via FE according to pre-condition
    create_order_multilisting(base_request, order_ticket_service, qty, client, order_type, price, tif[0], side[0],
                              lookup, allow_missing_prim=True, available_venue=True)
    # endregion

    order_id = get_order_id(base_request)

    # region Check values in Order book
    ...
    create_order_multilisting(base_request, order_ticket_service, qty, client, order_type, price, tif[1], side[1],
                              lookup, allow_missing_prim=True, available_venue=True)
    # endregion

    order_id_2 = get_order_id(base_request)

    # region Check values in Order book according to 5th step
    algo_parameters = check_order_algo_parameters_book(base_request, order_id_2, order_book_service)
    ...
    order_status = extract_parent_order_details(base_request=base_request, column_name="ExecSts",
                                                extraction_id="Extract Order ExecStatus", order_id=order_id_2)

    verifier(case_id=case_id, event_name="Verify Order ExecStatus", expected_value="Filled", actual_value=order_status)
    # endregion
