...
def verifier(case_id, event_name, filed_name, expected_rezult, actual_rezult):
    verifier_step1 = Verifier(case_id)
    verifier_step1.set_event_name(event_name)
    verifier_step1.compare_values(filed_name, expected_rezult, actual_rezult)
...
def execute(report_id):
    ...
    rezult_qty_zero = extract_error_message_order_ticket(base_request, order_ticket_service)

    # expected result for step 1
    verifier(case_id, "Check error message step 1", "Login Error with qty - 0", expected_rezult,
             rezult_qty_zero["ErrorMessage"])
    # end region
    ...
    rezult_qty_symbols = extract_error_message_order_ticket(base_request, order_ticket_service)

    verifier(case_id, "Check error message step 2", "Login Error with qty - a%/", expected_rezult,
             rezult_qty_symbols["ErrorMessage"])
    # end region
