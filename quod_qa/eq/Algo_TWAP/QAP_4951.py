...
would_price_qty = qty - trade_qty  #350 would price child order
qty_2 = qty - trade_qty       #333 posible qty
qty_2 = 350                             #qty for marketdata
qty_3 = 250                             #second slice
...
def rule_creation():
    ...
    nos_ioc_md_rule = rule_manager.add_NewOrdSingle_IOC_MarketData(connectivity_buy_side, account, ex_destination_1, price_2, trade_qty, True, connectivity_fh, s_par, price_2, qty_2, [NoMDEntries(MDEntryType="0", MDEntryPx="0", MDEntrySize="0", MDEntryPositionNo="1"), NoMDEntries(MDEntryType="1", MDEntryPx="20", MDEntrySize="500", MDEntryPositionNo="1")], [NoMDEntries(MDUpdateAction='0', MDEntryType='2', MDEntryPx='40', MDEntrySize='1000', MDEntryDate= datetime.utcnow().date().strftime("%Y%m%d"), MDEntryTime=datetime.utcnow().time().strftime("%H:%M:%S"))])
...
def execute(report_id):
    try:
        ...
        new_order_single_params = {
            'Account': client,
            'ClOrdID': 'QAP_4951_' + bca.client_orderid(9),
            'HandlInst': 2,
            'Side': side,
            'OrderQty': qty,
            'TimeInForce': tif_day,
