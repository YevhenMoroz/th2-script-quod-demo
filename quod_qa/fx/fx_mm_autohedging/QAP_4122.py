...
def execute(report_id, session_id):
    ...
    try:
        ...
        order_id_main = check_order_book(case_base_request, ob_service, orig_fix, client_tier)
        order_id = check_order_book(case_base_request, ob_service, orig_ah, client)
        ...
        verifier.submitCheckRule(
            bca.create_check_rule("Check Drop Copy Execution Report MO",
                                  bca.filter_to_grpc('ExecutionReport', order_params_mo, ["OrderID", "ExecType"]),
                                  checkpoint_id,
                                  connectivity_drop_copy,
                                  case_id)
        )
        verifier.submitCheckRule(
            bca.create_check_rule("Check Drop Copy Execution Report AH MO",
                                  bca.filter_to_grpc('ExecutionReport', order_params_ah, ["OrderID", "ExecType"]),
                                  checkpoint_id,
                                  connectivity_drop_copy,
                                  case_id)
        )
        ...
        send_rfq_and_filled_order_buy(case_id, "1000000")
        order_id_ao = check_order_book(case_base_request, ob_service, orig_ah, client_int)
        order_params_ao = {
            'ExecID': '*',
            'OrderQty': "2000000",
            'LastQty': "2000000",
        ...
        verifier.submitCheckRule(
            bca.create_check_rule("Check Drop Copy Execution Report AH AO",
                                  bca.filter_to_grpc('ExecutionReport', order_params_ao, ["OrderID", "ExecType"]),
                                  checkpoint_id1,
                                  connectivity_drop_copy,
                                  case_id))
        send_rfq_and_filled_order_sell(case_id, "2000000")
