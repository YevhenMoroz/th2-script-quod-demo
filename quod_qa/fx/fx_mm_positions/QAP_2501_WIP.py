...
def get_dealing_positions_details(del_act, base_request, symbol, account):
    ...
    daily_mtm_pnl = ExtractionPositionsFieldsDetails("dealingpositions.dailyMTMPnl", "Daily MTM PnL")
    mtm_pnl_usd = ExtractionPositionsFieldsDetails("dealingpositions.mtmPnlUsd", " MTM PnL (USD)")
    daily_mtm_pnl_usd = ExtractionPositionsFieldsDetails("dealingpositions.dailyMTMPnlUsd", "Daily MTM PnL (USD)")
        ...
        PositionsInfo.create(
            action=ExtractionPositionsAction.create_extraction_action(
                extraction_details=[position, daily_mtm_quote_position,
                                    mkt_px, quote_position, mtm_pnl, mtm_pnl_usd, daily_mtm_pnl_usd,
                                    daily_mtm_pnl])))
...
def check_daily_pnl(case_id, position, mtk_px, quote_pos, extracted_pnl):
    quote_pos = float(quote_pos.replace(",", ""))
    position = float(position.replace(",", ""))
    mtk_px = float(mtk_px)
    expected_pnl = (position * mtk_px) + quote_pos
    ...
    verifier.compare_values("Daily MTM Pnl", str(expected_pnl), extracted_pnl.replace(",", ""))
    verifier.verify()


def check_pnl_usd(case_id, position, mtk_px, quote_pos, extracted_pnl):
    position = float(position.replace(",", ""))
    mtk_px = float(mtk_px)
    quote_pos = float(quote_pos.replace(",", ""))
...
def check_daily_pnl_usd(case_id, position, mtk_px, quote_pos, extracted_pnl):
    position = float(position.replace(",", ""))
    mtk_px = float(mtk_px)
    quote_pos = float(quote_pos.replace(",", ""))
    expected_pnl = (position * mtk_px + quote_pos) / mtk_px
    ...
    verifier.compare_values("Daily MTM Pnl USD", str(round(expected_pnl, 2)), extracted_pnl.replace(",", ""))
    verifier.verify()
...
def execute(report_id, session_id):
    ...
    try:
        ...
        check_daily_pnl(case_id, position_info["dealingpositions.position"], position_info["dealingpositions.mktPx"],
                        position_info["dealingpositions.dailyMTMquotePosition"],
                        position_info["dealingpositions.dailyMTMPnl"])
        ...
        check_daily_pnl_usd(case_id, position_info["dealingpositions.position"],
                            position_info["dealingpositions.mktPx"],
                            position_info["dealingpositions.dailyMTMquotePosition"],
                            position_info["dealingpositions.dailyMTMPnlUsd"])
        # Step 3
        ...
        check_pnl(case_id, position_info["dealingpositions.position"], position_info["dealingpositions.mktPx"],
                  position_info["dealingpositions.quotePosition"], position_info["dealingpositions.mtmPnl"])
        check_daily_pnl(case_id, position_info_after_8m["dealingpositions.position"],
        ...
        check_daily_pnl_usd(case_id, position_info_after_8m["dealingpositions.position"],
                            position_info_after_8m["dealingpositions.mktPx"],
                            position_info_after_8m["dealingpositions.dailyMTMquotePosition"],
                            position_info_after_8m["dealingpositions.dailyMTMPnlUsd"])
        # Step 5
        ...
        check_pnl(case_id, position_info["dealingpositions.position"], position_info["dealingpositions.mktPx"],
                  position_info["dealingpositions.quotePosition"], position_info["dealingpositions.mtmPnl"])
        check_daily_pnl(case_id, position_info_after_3m["dealingpositions.position"],
        ...
        check_daily_pnl_usd(case_id, position_info_after_3m["dealingpositions.position"],
                            position_info_after_3m["dealingpositions.mktPx"],
                            position_info_after_3m["dealingpositions.dailyMTMquotePosition"],
                            position_info_after_3m["dealingpositions.dailyMTMPnlUsd"])
    except Exception:
