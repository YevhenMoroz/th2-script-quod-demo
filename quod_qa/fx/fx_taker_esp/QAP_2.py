...
def check_order_after_amend(base_request, act_ob, case_id, order_id, price, qty, sts):
    ob = OrdersDetails()
    execution_id = bca.client_orderid(4)
    ob.set_default_params(base_request)
    ob.set_extraction_id(execution_id)
...
def execute(report_id, session_id):
    ...
    try:
        ...
        # Step 2
        amend_order(ob_fx_service, case_base_request, "price")
        check_child_book(case_base_request, ob_service, case_id, order_info[1])
        new_child = check_order_after_amend(case_base_request, ob_service, case_id, order_info[0], "1.18", qty, "Open")
        # Step 3
        amend_order(ob_fx_service, case_base_request, "qty")
        check_child_book(case_base_request, ob_service, case_id, new_child)
        check_order_after_amend(case_base_request, ob_service, case_id, order_info[0], "1.18", "8000000",
                                "Open")
        # Step 4
