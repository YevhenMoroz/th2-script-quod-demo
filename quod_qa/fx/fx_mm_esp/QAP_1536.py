...
def check_instrument(base_request, service, case_id, instrument, client_tier):
    extract_request = ExtractRatesTileValues(details=base_request)
    extraction_id = bca.client_orderid(4)
    extract_request.set_extraction_id(extraction_id)
    extract_request.extract_instrument("ratesTile.instrument")
    extract_request.extract_client_tier("ratesTile.clientTier")
    response = call(service.extractRateTileValues, extract_request.build())

    extracted_instrument = response["ratesTile.instrument"]
    extracted_client_tier = response["ratesTile.clientTier"]

    verifier = Verifier(case_id)
    verifier.set_event_name("Check instrument and client tier")
    verifier.compare_values("Instrument", instrument, extracted_instrument)
    verifier.compare_values("Client tier", client_tier, extracted_client_tier)
    verifier.verify()
