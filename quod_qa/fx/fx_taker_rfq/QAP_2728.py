...
def check_order_book(ex_id, base_request, instr_type, act_ob, case_id, side, ccy1_side, currency):
    ob = OrdersDetails()
    ...
    verifier.compare_values("Side", side, response[ob_side.name])
    verifier.compare_values("CCY1 Side", ccy1_side, response[ob_ccy1_side.name])
