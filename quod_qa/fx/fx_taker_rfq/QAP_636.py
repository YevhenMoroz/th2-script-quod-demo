...
from win_gui_modules.aggregated_rates_wrappers import ModifyRFQTileRequest, \
    ExtractRFQTileValues, TableActionsRequest, TableAction, CellExtractionDetails, ContextAction
...
def check_value_in_column(exec_id, base_request, service, case_id, venue):
    ...
    table_actions_request.add_actions([extract1, extract2])
    ...
    dist = abs(extracted_best_bid - extracted_pts)

    verifier = Verifier(case_id)
    verifier.set_event_name("Check calculation Dist")
    verifier.compare_values("Dist", str(round(dist, 1)), str(extracted_dist))
    verifier.verify()
...
def execute(report_id, session_id):
    ...
    try:
        ...
        modify_rfq_tile_swap(base_rfq_details, ar_service, case_qty, case_from_currency,
                             case_to_currency, case_near_tenor, case_far_tenor, case_client, venue)
        # Step 2
        send_rfq(base_rfq_details, ar_service)
        cancel_rfq(base_rfq_details, ar_service)
        check_value_in_column("ChWK1_0", base_rfq_details, ar_service, case_id, venue)
