...
def check_quote_request_b(base_request, service, case_id, status, auto_q, qty):
    ...
    verifier.compare_values("Near leg qty", qty, response[qr_near_leg_qty.name])
    verifier.compare_values("Far leg qty", qty, response[qr_far_leg_qty.name])
    verifier.verify()
...
def execute(report_id, session_id):
    ...
    try:
        ...
        # Step 3
        quote_id = check_quote_request_b(case_base_request, ar_service, case_id, "New", "Yes", qty)
        # Step 4
