...
from quod_qa.web_admin.web_admin_core.pages.market_making.client_tier.client_tier_instrument_tiered_quantities_sub_wizard import \
...
class QAP_1693(CommonTestCase):
    ...
    def test_context(self):
        ...
        try:
            ...
            time.sleep(2)
            try:
                client_tier_instrument_tiered_quantities_sub_wizard.set_quantity_filter("1000000")
                time.sleep(2)
                client_tier_instrument_tiered_quantities_sub_wizard.click_on_edit()
                self.verify("Entity not deleted", True, False)
            except Exception as e:
                self.verify("Is entity deleted", "TimeoutException", e.__class__.__name__)
