...
class ExecutionStrategiesLitPassiveSubWizard(CommonPage):
    ...
    # Venue priority
    def click_on_plus_at_venue_priority_sub_wizard(self):
        self.find_by_xpath(ExecutionStrategiesConstants.PLUS_BUTTON_AT_VENUE_PRIORITY_SUB_WIZARD).click()

    def click_on_checkmark_at_venue_priority_sub_wizard(self):
        self.find_by_xpath(ExecutionStrategiesConstants.CHECKMARK_BUTTON_AT_VENUE_PRIORITY_SUB_WIZARD).click()

    def click_on_cancel_at_venue_priority_sub_wizard(self):
        self.find_by_xpath(ExecutionStrategiesConstants.CANCEL_BUTTON_AT_VENUE_PRIORITY_SUB_WIZARD).click()

    def click_on_delete_at_venue_priority_sub_wizard(self):
        self.find_by_xpath(ExecutionStrategiesConstants.DELETE_BUTTON_AT_VENUE_PRIORITY_SUB_WIZARD).click()

    def set_venue_priority(self, value):
        self.set_combobox_value(ExecutionStrategiesConstants.VENUE_PRIORITY_FIELD_AT_VENUE_PRIORITY_SUB_WIZARD, value)
    ...
    def is_post_mode_values_exist(self, value: List[str]):
        for item in range(4):
            try:
                self.set_combobox_value(ExecutionStrategiesConstants.VALUE_FIELD_AT_PARAMETERS_SUB_WIZARD, value[item])
            except NameError:
                return False
        return True
